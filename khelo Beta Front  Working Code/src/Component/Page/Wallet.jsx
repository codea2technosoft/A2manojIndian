...
export default function Wallet() {
  ...
  const handleDeposit = () => {
    ...
    try {
      if (inputValuedeposit) {
        setLoading(true);
        const apiUrl = `https://khelobeta.com/api/deposit.php?name=${userData.name}&userid=${user_id}&amount=${inputValuedeposit}&contact=${userData.mob}&getaway=razorpay&type=web`;
  ...
  const handleUTRPyament = () => {
    try {
      // setLoading(true);
      const apiUrl = `https://khelobeta.com/api/mdeposit.php?name=${userData.name}&userid=${user_id}&contact=${userData.mob}`;
      window.location.href = apiUrl;
    } catch (error) {
      console.error("There was a problem with the fetch operation:", error);
      setLoading(false);
