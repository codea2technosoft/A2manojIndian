...
function Pending() {
  ...
  const fetchPendingBets = async (page = 1) => {
    ...
    try {
      ...
      const apiUrl = `${process.env.REACT_APP_API_URL}/gameloadbet-list?page=${page.toString()}&limit=${limit.toString()}&username=${FilterUsername}&markettype=${FilterMarkettype}&marketname=${FilterMarketname}&gametype=${FilterGameType}&session=${FilterSession}&startdate=${startDateForFetch}&enddate=${endDateForFetch}${statusQuery}`;

      console.log("Bet Pending API URL:", apiUrl);
      ...
      const data = await res.json();
      console.log("Bet Pending API Response:", data);
