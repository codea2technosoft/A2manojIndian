...
function SuccessBetHistory() {
  ...
  // Date change functions में changes:
  const setSelectedStartDate = (e) => {
    const value = e;
    setselectedStartDate(value);
  ...
  const fetchSuccessBets = async (page = 1) => {
    ...
    try {
      ...
      const apiUrl = `${process.env.REACT_APP_API_URL}/gameloadbet-success-list?page=${page.toString()}&limit=${limit.toString()}&username=${FilterUsername}&markettype=${FilterMarkettype}&marketname=${FilterMarketname}&gametype=${FilterGameType}&session=${FilterSession}&startdate=${startDateForFetch}&enddate=${endDateForFetch}${statusQuery}`;

      console.log("Bet Success API URL:", apiUrl);
      ...
      const data = await res.json();
      console.log("Bet Success API Response:", data);
