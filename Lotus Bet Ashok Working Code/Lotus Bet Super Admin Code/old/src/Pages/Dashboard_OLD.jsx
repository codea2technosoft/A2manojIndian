...
const cardConfig = [
  {
    ...
    modalLinks: [
      { label: "Master", key: "total_master", route: "/masters_list", icon: <FaUserTie size={16} /> },
      { label: "Super Agent", key: "total_super_agent", route: "/agent_lists", icon: <FaUserTie size={16} /> },
      { label: "Agent User", key: "total_agent", route: "/AgentMasternew", icon: <FaUsers size={16} /> },
      { label: "User", key: "total_user", route: "/Mastermyuser", icon: <FaUsers size={16} /> },
    ],
  },
  {
    bg: '#fa0e0e',
  ...
  },
  {
    ...
    modalLinks: [
      { label: "Profit / Loss", route: "/profitloss", icon: <FaChartLine size={16} /> },
      { label: "My Ledger", route: "/my-ledger", icon: <FaBook size={16} /> },
      { label: "Master", route: "/Master-ledger", icon: <FaUserTie size={16} /> },
      { label: "Super Agent", route: "/super-agent-ledger", icon: <FaUsers size={16} /> },
  ...
  },
  {
    ...
    modalLinks: [
      { label: "Master", route: "/agent_master-transaction", icon: <FaUserTie size={16} /> },
      { label: "Super Agent ", route: "/Superagenttransaction", icon: <FaUsers size={16} /> },
      { label: "Agent ", route: "/Agenttransaction", icon: <FaUserTie size={16} /> },
    ],
  },
  {
    bg: '#009d9a',
...
const SportSection = ({ sportId, sportName, games, color, icon }) => {
  const navigate = useNavigate();

  if (!games || games.length === 0) return null;

  const handleMatchClick = (market_id, event_id, e) => {
    e.preventDefault();
  ...
  return (
    <div className="sport-section mb-4">
      <div className="sport-header" style={{ borderLeft: `4px solid ${color}` }}>
        <div className="sport-icon-wrapper" style={{ backgroundColor: color }}>
          <span className="sport-icon">{icon}</span>
        </div>
        <h3 className="sport-title">{sportName}</h3>
        {/* <span className="match-count">{games.length} matches</span> */}
      </div>
              ...
              <tr key={game._id}>
                ...
                <td>
                  <button
                    ...
                    onClick={(e) => handleMatchClick(game.market_id, game.event_id, e)}
                  >
                    <FaEye />
                  </button>
                </td>
...
export default function Dashboard() {
  ...
  const sortedSportIds = SPORT_ORDER.filter(id => 
    groupedGames[id] && groupedGames[id].length > 0
  );
