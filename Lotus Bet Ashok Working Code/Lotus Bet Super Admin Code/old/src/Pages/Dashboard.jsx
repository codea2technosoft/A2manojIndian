...
const SportSection = ({ sportId, sportName, games, color, icon, onToggleStatus }) => {
        ...
        response.data.forEach(market => {
          if (market.marketId && market.runners && market.runners.length > 0) {
            ...
            runners.forEach((runner, index) => {
              if (runner.ex) {
                const backPrice = runner.ex.availableToBack && runner.ex.availableToBack.length > 0 
                  ? runner.ex.availableToBack[0].price 
                  : null;
                const layPrice = runner.ex.availableToLay && runner.ex.availableToLay.length > 0 
  ...
  const handleMatchClick = (eventId, seriesId, e) => {
    ...
    navigate(`/viewmatch-fancy/series_idd/${seriesParam}/event_id/${eventParam}`);
  };
              ...
              return (
                ...
                >
                  <td className="event_id w-100">
                    <div className="d-flex justify-content-between gap-1">
                      <div className="d-flex justify-content-between align-items-center gap-1">
                        ...
                        <div 
                          onClick={(e) => handleToggleStatus(game._id, game.status || 0, e)}
                          style={{ cursor: 'pointer' }}
                  ...
                  <td
                    ...
                    style={{ padding: "2px" }}
                  >
                    <div className="odds_btns_div" style={{ display: 'flex', gap: '4px' }}>
                      {oddsDisplay.map((value, index) => (
                        <button 
                          ...
                          style={{
                            ...
                            backgroundColor: value !== null ? (index % 2 === 0 ? '#d4edda' : '#f8d7da') : '#e9ecef',
                            color: value !== null ? '#000' : '#999',
                            minWidth: '40px',
                            fontSize: '12px',
                            cursor: 'pointer'
                          }}
              ...
              sortedSportIds.map((sportId) => {
                ...
                return (
                  <SportSection
                    ...
                    onToggleStatus={() => fetchEvents(1, pagination.itemsPerPage)}
                  />
                );
              })
            )}
