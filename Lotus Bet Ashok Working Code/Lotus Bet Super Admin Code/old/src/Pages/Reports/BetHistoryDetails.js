          ...
          {roundInfo && (
            <div className="row mb-3">
              ...
              <div className="col-md-3">
                ...
                <p className="fw-bold">
                  <span className={`badge ${roundInfo.status === 1 ? 'bg-success' : 'bg-danger'}`}>
                ...
                <table className="table table-bordered table-hover table-striped">
                  <thead className="table-dark">
                    <tr>
                      ...
                      <th style={{ textAlign: 'right' }}>STAKES</th>
                    ...
                    {betData.map((item, index) => {
                      ...
                      return (
                        <tr key={item.id || index}>
                          ...
                          <td style={{ fontSize: '12px' }}>{item.user?.username || '-'}</td>
                          <td style={{ fontSize: '12px' }}>{item.runner_name || '-'}</td>
                          <td style={{ fontSize: '12px' }}>{item.round_id || '-'}</td>
                          ...
                          <td style={{ textAlign: 'right', fontSize: '12px' }}>
                            <span className={`badge ${isWin ? 'bg-success' : isLoss ? 'bg-danger' : 'bg-secondary'}`}>
                              {formatNumber(resultValue)}
                            </span>
                          </td>
                          <td style={{ fontSize: '12px' }}>
                            <span className={`badge ${item.status === 1 ? 'bg-success' : 'bg-warning'}`}>
