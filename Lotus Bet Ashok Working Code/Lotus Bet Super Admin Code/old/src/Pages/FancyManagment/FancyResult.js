...
const FancyResult = () => {
  ...
  const handleDeleteFancyOddWise = async () => {
    if (!currentFancy) {
      toast.error("Fancy not selected ❌");
      return;
    }
    const ok = await confirmAction(
      "Delete Bets?",
    ...
    try {
      setBtnLoader("delete_oddwise", true);
      ...
      const res = await deleteFancyBetOddwise(payload);
      ...
      setBtnLoader("delete_oddwise", false);
    }
  };
  const fetchFullFancyList = async () => {
    try {
      const payload = {
        fancy_id: currentFancy.fancy_id,
        event_id: currentFancy.event_id,
      ...
      toast.error("Error loading full list ❌");
    }
  };
  // // SETTLE
  ...
  const handleBetDelete = async (f) => {
    ...
    try {
      ...
      const res = await deleteFancyBets({
        fancy_id: f.fancy_id,
      });
              ...
              <div className="modal-body p-1">
                <div className="row g-2 p-2">
                  ...
                  {/* <div className="col-md-4">
                    <label className="form-label mb-0"><small>Bet Type</small></label>
                    <select
                      className="form-control form-control-sm"
                      value={betType}
                  ...
                  {/* Odd Input */}
                  {/* <div className="col-md-4">
                    <label className="form-label mb-0"><small>Odd</small></label>
                    <input
                      type="number"
                      className="form-control form-control-sm"
                      placeholder="Odd"
                  ...
                  <div className="col-md-3 d-flex gap-2 align-items-end">
                    ...
                    <button
                      className={`refreshbutton ${!(startDate && endDate) ? "disabled-btn" : ""}`}
                      // disabled={!(betType && odd && startDate && endDate)}
                      disabled={!(startDate && endDate)}
                      onClick={handleDeleteFancyOddWise}
                    >
                      <AiFillDelete />
                    </button>
                  </div>
