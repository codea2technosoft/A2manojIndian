...
import {
  getMatchBetCompletedHistory,
  fetMatchExposerMyBook,
} from "../../Server/api";
...
function MatchBet() {
  ...
  const getEventBets = async (page = 1) => {
    try {
      ...
      const res = await getMatchBetCompletedHistory(payload);
  ...
  return (
    <div className="card">
      <div className="card-header bg-primary-yellow d-flex justify-content-between align-items-center">
        <h3 className="mb-0 card-title">MATCH BETS</h3>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((bet, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        {formatNumber(
                          bet.bet_amount || bet.stake || bet.amount,
                        )}
                      </td>
                    </tr>
                  ))
