...
const CreateSuperAgentAdmin = () => {
  ...
  // ✅ ONLY PAYLOAD MODIFICATION - Bas ye part change karna hai
...
const handleSubmit = async (e) => {
  ...
  try {
    const payload = {
      // ✅ NAYA FIELD ADD KAREN - super_agent_id
      super_agent_id: formData.admin_id,  // ← YE ADD KAREN
      super_admin_id: formData.super_admin_id || "admin",
      name: formData.name.trim(),
      password: formData.password,
      reference: formData.reference,
      master_admin_id: formData.admin_id,  // ← Yeh "admin_id" super agent ka hai
      coins: parseInt(formData.coins) || 0,
      match_share: parseFloat(formData.agentMatchShare) || 0,
      commission_type: formData.agentCommissionType,
      
      match_comm: formData.agentCommissionType === "0" 
        ? 0 
        : parseFloat(formData.agentMatchComm) || 0,
      
      football_comm: formData.agentCommissionType === "0" 
        ? 0 
        : parseFloat(formData.agentFootballComm) || 0,

      tennis_comm: formData.agentCommissionType === "0" 
        ? 0 
        : parseFloat(formData.agentTennisComm) || 0,

      horse_racing_comm: formData.agentCommissionType === "0" 
        ? 0 
        : parseFloat(formData.agentHorseRacingComm) || 0,

      greyhound_racing_comm: formData.agentCommissionType === "0" 
        ? 0 
        : parseFloat(formData.agentGreyhoundRacingComm) || 0,

      politics_comm: formData.agentCommissionType === "0" 
        ? 0 
        : parseFloat(formData.agentPoliticsComm) || 0,

      casino_comm: formData.agentCommissionType === "0" 
        ? 0 
        : parseFloat(formData.agentCasinoComm) || 0,

      session_comm: formData.agentCommissionType === "0" 
        ? 0 
        : parseFloat(formData.agentSessionComm) || 0,
    ...
    <>
      ...
      <div className="card">
        <div className="card-header bgHeader d-flex bg-primary-yellow justify-content-between align-items-center">
          <h3 className="card-title mb-0"><IoCreateSharp className="fs-2" />Create New master</h3>
