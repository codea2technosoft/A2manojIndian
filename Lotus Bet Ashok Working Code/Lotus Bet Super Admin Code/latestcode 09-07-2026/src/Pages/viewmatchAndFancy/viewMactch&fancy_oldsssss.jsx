  ...
  ({
    ...
    const getExposureAmount = (runnerName) => {
      ...
      if (matchesName && Array.isArray(matchesName)) {
        ...
        if (
          team &&
          bookmakerExposureData[team.team_id?.toString()] !== undefined
        ) {
...
function Cricket() {
  ...
  // useEffect(() => {
  //   const parts = location.pathname.split("/");
  //   let marketIdx = parts.indexOf("market_id");
  //   if (marketIdx === -1) {
  //     marketIdx = parts.indexOf("series_idd");
  //   }
  //   const extractedMarketId = (marketIdx !== -1 && parts[marketIdx + 1]) 
  //     ? parts[marketIdx + 1] 
  //     : "";
  //   const sportIdx = parts.indexOf("sport_id");
  //   const extractedSportId = (sportIdx !== -1 && parts[sportIdx + 1]) 
  //     ? parts[sportIdx + 1] 
  //     : "";

  //   setMarketId(extractedMarketId);
  //   setSportId(extractedSportId);
  //   alert(`Market ID: ${extractedMarketId || "Not found"}\nSport ID: ${extractedSportId || "Not found"}`);
    ...
    try {
     ...
     // alert(event_id);
      ...
      const response = await axios.get(
        ...
        // `https://cricketapinew.shyammatka.co.in/get-match-odds-list?id=${series_idd}&sport_id=${sportId}`,
  ...
  const getBookmakerList = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        ...
        `https://cricketapinew.shyammatka.co.in/get-book-maker-list?id=${marketId}&sport_id=${sportId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    ...
    const intervalTime =
     showLoginModal || isLadderOpen ? 600000 : 2000000;
