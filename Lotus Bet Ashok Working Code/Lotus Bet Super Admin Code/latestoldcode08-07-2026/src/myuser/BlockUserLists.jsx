...
import { blockUserList, blockUnblockMasteruser } from "../Server/api";
...
function BlockedUserLists() {
  ...
  const fetchAgentData = async (
    ...
    try {
      ...
      const res = await blockUserList(payload);
      if (res.data.success) {
        const data = res.data;
  ...
  const handleBlockUnblockConfirm = async () => {
    ...
    try {
      ...
      const res = await blockUnblockMasteruser(
  ...
  const handleNext = () =>
    ...
    fetchAgentData(currentPage + 1, itemsPerPage, searchTerm);
  const handlePrev = () =>
    currentPage > 1 &&
    fetchAgentData(currentPage - 1, itemsPerPage, searchTerm);
  const handlePageClick = (page) =>
    page >= 1 &&
    page <= totalPages &&
    fetchAgentData(page, itemsPerPage, searchTerm);
    ...
    <>
      <div className="card agentmaster">
        <div className="card-header bg-primary-yellow d-flex justify-content-between align-items-center">
          <h5 className="mb-0">Blocked User Lists</h5>
                    ...
                    <tr key={row.admin_id || index}>
                      ...
                      <td>
                        <div className="d-flex align-items-center">
                          <input
                            type={
                              showPassword[row.admin_id] ? "text" : "password"
                            }
                            value={row.password || ""}
                            readOnly
