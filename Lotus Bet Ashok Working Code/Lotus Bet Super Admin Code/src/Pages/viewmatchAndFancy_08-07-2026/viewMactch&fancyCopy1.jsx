...
function Cricket() {
  ...
  useEffect(() => {
    if (isInitialLoad && series_idd && event_id) {
      const fetchAllData = async () => {
        try {
          await Promise.all([matchlist(), getfancylist(), getBookmakerList(), fetchMatchTotalExposure()]);
        } catch (error) {
        } finally {
  ...
  useEffect(() => {
    ...
    const intervalTime =
      showLoginModal || isLadderOpen ? 600000 : 200000;
        ...
        <div className="modal-overlay" onClick={closeLadder}>
          <div className="ladder-modal modal_client" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h4 className="text-light">Client Session PL</h4>
