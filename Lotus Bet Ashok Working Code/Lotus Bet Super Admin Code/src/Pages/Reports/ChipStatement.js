...
const ChipStatement = () => {
  ...
  }, [adminId, currentPage, searchTerm, sport, fromDate, toDate, transactionType]);
  ...
  const fetchChipStatementData = async (page = currentPage) => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        const errorMsg = response.data?.message || "Failed to fetch chip statement";
        toast.error(errorMsg);
      }
    } catch (error) {
      console.error("Error fetching chip statement:", error);
      const errorMsg = error.response?.data?.message || error.message || "Failed to fetch chip statement";
      toast.error(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  const hasActiveFilters = searchTerm || sport !== "ALL" || fromDate || toDate || transactionType !== "ALL";
                  ...
                  <table className="table table-bordered table-hover">
                    ...
                    <tbody>
                      {statementData.map((item, index) => {
                        ...
                        const isCredit = item.type === "credit" || item.dc === "Credit";
                        ...
                        return (
                          <tr key={item._id || index}>
                            ...
                            <td className={isCredit ? "text-success" : "text-danger"}>
                              {isCredit ? "Credit" : "Debit"}
                            </td>
                            <td className="text-end fw-bold">
                              {formatNumber(item.wallet_amount || 0)}
                            </td>
                          </tr>
                        );
                      })}
