...
import { getChildList_list, addUserNewTransaction, getMyLedgerTxnUser, UserdeleteLedgerTxn } from "../../Server/api";
...
function Usertransaction() {
    ...
    const { admin_id: urlAdminId } = useParams();
    ...
    const loginId=localStorage.getItem("admin_id")
    ...
    const fetchClients = async () => {
        try {
            ...
            const response = await getChildList_list(5);
    ...
    useEffect(() => {
        if (urlAdminId && clients.length > 0) {
            const user = clients.find(c => c.admin_id === urlAdminId);
            if (user) {
                setFormData(prev => ({
                    ...prev,
                    client: user.admin_id
                }));
                setSelectedUserData(user);
                setShowTable(true);
                setCurrentPage(1);
                fetchLedgerData(user.admin_id, 1, paymentFilter);
            }
        }
    }, [urlAdminId, clients]);
    ...
    const handleDeleteTxn = async (ledger_id) => {
        ...
        try {
            const res = await UserdeleteLedgerTxn({
    ...
    const fetchLedgerData = async (adminId, pageNo = 1, filter = "all") => {
        ...
        try {
            ...
            const response = await getMyLedgerTxnUser({
                admin_id: adminId,
                payment_type: filter,
                page: pageNo,
                limit: itemsPerPage
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await addUserNewTransaction(transactionData);
            if (response.data && response.data.success) {
                await Swal.fire({
                    icon: "success",
                            ...
                            <table className="table table-bordered table-striped">
                                ...
                                <tbody>
                                    {ledgerData.map((transaction, index) => {
                                        ...
                                        const diya = Number(transaction.credit) || 0;
                                        const liya = Number(transaction.debit) || 0;
                                        // const balance = parseFloat(transaction.balance) || 0;
                                        runningBalance = runningBalance + liya - diya;
                                        ...
                                        return (
                                            <tr key={transaction.id || transaction.lenden_uuid || index}>
                                                ...
                                                <td className={transaction.balance >= 0 ? "text-success" : "text-danger"}>
                                                    <strong>{Number(transaction.balance).toFixed(2)}</strong>
                                                </td>
                            ...
                            <div className="paginationall d-flex align-items-center gap-1">
                                ...
                                <div className="d-flex gap-1">
                                    {getPageNumbers().map((currentPage) => (
                                        <div
                                            key={currentPage}
                                            className={`paginationnumber ${currentPage === currentPage ? "active" : ""
                                                }`}
                ...
                <div className="card">
                    <div className="card-header bg-primary-yellow">
                        <div className="d-flex align-items-center justify-content-between">
                            <h3 className="card-title  mb-0"> User  Transaction</h3>
