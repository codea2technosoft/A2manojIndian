...
//       fetchMasters();
//     }
//   }, [masterIdFromURL, fromDate, toDate]);
...
//       setIsFiltering(true);
//       const masterId =
//         masterIdFromURL || localStorage.getItem("selectedMasterId");
//       const params = {
//         role: 3,
...
//         <div className="card-header border-0 bg-primary-yellow d-flex justify-content-between align-items-center flex-wrap gap-2">
//           <h3 className="card-title mb-0">Mah7's Balance Sheet</h3>
...
//                   }`}
//                 >
//                   <h5 className="modal-title">Super Master Transaction</h5>
//                   <button
//                     type="button"
//                     className="btn-close"
//                     onClick={() => {
...
//                     <input
//                       type="text"
//                       value={selectedAgent?.username || "ramrajgg"}
...
function SuperAgentLedger() {
 ...
 const getUTType = (adminId) => {
  ...
  // ✅ CHANGE 2: TOPCLIENTS ki tarah - SIRF EK USEFFECT, BINA CONDITION KE
  useEffect(() => {
    if (masterIdFromURL) {
      localStorage.setItem("selectedMasterId", masterIdFromURL);
    }
    // ✅ SIRF EK BAR FETCH - BINA DATES KE
    fetchMasters("", "");
  }, []); // ✅ EMPTY DEPENDENCY ARRAY - SIRF EK BAR CHALEGA

  // ✅ CHANGE 3: fetchMasters - params leta hai
  const fetchMasters = async (from = "", to = "") => {
    try {
      setLoading(true);
      setIsFiltering(true);
      const masterId =
      ...
      const params = { role: 3 };
      if (masterId) params.admin_id = masterId;
      
      if (from && from.trim()) params.from_date = from;
      if (to && to.trim()) params.to_date = to;
  ...
  // ✅ CHANGE 4: Handle Filter - ab alert nahi
  const handleFilter = () => {
    fetchMasters(fromDate, toDate);
  };
