...
//     // ✅ DYNAMIC admin_id - navigationPayload se, nahi toh localStorage se
//     const adminId = navigationPayload.admin_id || loggedInAdminId || "admin";
    
//     const payload = {
//       admin_id: adminId, // ✅ ab dynamic hai
...
const ProfitLossBetHistory = () => {
  ...
  // ✅ Casino check - data se detect karo (sport filter "ALL" hone par bhi)
  const isCasino = statementData.length > 0 && statementData.some(item => 
    item.sport_name === "All Casino" || item.bet_type === "casino"
  );
      ...
      return (
        <tr key={item._id || serialNo}>
          ...
          <td className={`fw-bold ${item.total >= 0 ? "text-success" : "text-danger"}`}>
            {formatNumber(item.win || item.total || 0)}
          </td>
          <td>
              {item.win_loss || "-"}
