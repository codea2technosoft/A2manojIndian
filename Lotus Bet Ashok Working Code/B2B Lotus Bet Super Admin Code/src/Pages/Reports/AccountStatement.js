...
const AccountStatement = () => {
  ...
  const fetchStatementData = async (page = currentPage) => {
    try {
      ...
      if (fromDate && fromDate.trim()) payload.from_date = fromDate;
      if (toDate && toDate.trim()) payload.to_date = toDate;
      if (transactionType && transactionType !== "ALL") payload.tr_type = transactionType;
  ...
  // ✅ FIXED: Bet History navigate function
  // const handleBetHistoryClick = (item) => {
  //   const eventId = item.event_id || item.round_id || "";
  //   const marketId = item.market_id || item.event_type_id || "";
  //   const sportId = item.sport_id || item.sport || "";
  ...
  //   // ✅ Casino (sport_id = 10) - sirf sport_id bhejo
  //   if (sportId === "10" || sportId === 10) {
  //     navigate(`/reports/bet-history/${sportId}`, {
  //       state: {
  //         payload: {
  ...
  //   // ✅ Baaki sports - event_id, sport_id, market_id sab bhejo
  //   if (eventId && sportId) {
  //     navigate(`/reports/bet-history/${eventId}/${sportId}/${marketId || ""}`, {
  //       state: {
  //         payload: {
          ...
          <div className="row mb-3 align-items-center gy-2">
            ...
            <div className="col-md-2 col-6">
              ...
              >
                ...
                {/* <option value="PROFIT_LOSS">Profit & Loss</option> */}
                {/* <option value="FREE_CHIPS">Free Chips</option> */}
                <option value="coins">Free Chips</option>
                 <option value="Bet">Profit & Loss</option> 
                {/* <option value="coins">Cash(0)</option> */}
              </Form.Select>
            </div>
