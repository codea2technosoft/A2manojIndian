...
function AgentMasterTransction() {
  ...
  const selectedSuperAgentId = master_id;
  ...
  const fetchClients = async () => {
    try {
      ...
      const response = await SelectMastertList({ role: 3 });

      if (response.data?.success) {
        const apiData = response.data.data || [];
        console.log("API Data:", apiData);
        const role3Users = apiData.filter((user) => user.role === 3);
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        setSummary({
          ...
          balance: Number(res.balance) || 0,
          balanceType: res.balance >= 0 ? "LENA" : "DENA",
        });
      }
    } catch (err) {
    ...
    return (
      <div className="card mt-4">
        <div className="card-header bg-primary-yellow">
          <h5 className="text-white mb-0">Super agent Details</h5>
        </div>
              ...
              <table className="table table-bordered table-striped">
                ...
                <tbody>
                  {ledgerData.map((transaction, index) => {
                    ...
                    const dr = transaction.debit || 0;
                    const cr = transaction.credit || 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                        // className={balanceType === "CREDIT" ? "text-success" : "text-danger"}
                        >
                          <strong>{Math.abs(balance).toFixed(2)}</strong>
                        </td>
                        <td>
                          {/* {transaction.payment_type === "cr" ? "CREDIT" : "DEBIT"} */}
                          {/* {cr > 0 ? "LENA" : dr > 0 ? "DENA" : "-"} */}
        ...
        <div className="card">
          <div className="card-header bg-primary-yellow">
            <div className="d-flex align-items-center justify-content-between">
              <h3 className="card-title mb-0"> Super agent Transaction</h3>
              <button
