...
const Statementmasterlist = () => {
  ...
  // ============================
  // UTILITY FUNCTION: UTC to IST with 12-hour format
  // ============================
  const convertUTCToIST12Hour = (utcDateString) => {
  ...
  const fetchStatementData = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/get-statement`,
        {
      ...
      if (response.data.success) {
        ...
        const formattedData = data.map((item, index) => {
          ...
          // Format date using UTC to IST conversion with 12-hour format
          const formattedDate = convertUTCToIST12Hour(item.created_at);
