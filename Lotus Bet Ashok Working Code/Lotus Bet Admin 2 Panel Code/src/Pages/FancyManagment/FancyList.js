  ...
  const fetchFancyList = async () => {
    try {
      ...
      const response = await getFancySecondList(eventId);
       console.log("fdfdfdfdf",response)
  ...
  const handleFancyStatusChange = async (item) => {
    try {
      ...
      if (response.data.success) {
        ...
        setFancies((prev) =>
          prev.map((f) =>
            f.SelectionId === item.SelectionId ? { ...f, status: newStatus } : f
          )
        );
      } else {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        fetchFancyList(); // लिस्ट रिफ्रेश करें
      } else {
        showToast(response.data.message || "Failed to update result", "error");
      }
    } catch (error) {
      ...
      <div className="card">
        <div className="card-header bg-dark text-white d-flex  justify-content-between align-items-center">
          ...
          <button
            ...
            onClick={fetchFancyList}
          >
            <MdFilterListAlt /> Refresh
          </button>
              ...
              {fancies.length > 0 ? (
                fancies.map((item, index) => (
                  <tr key={item.SelectionId || index}>
                    ...
                    <td>{getStatusBadge(item.GameStatus)}</td>
                    <td>
                   <button
  className={`btn ${item.status === 1 ? "btn-danger" : "btn-success"}`}
