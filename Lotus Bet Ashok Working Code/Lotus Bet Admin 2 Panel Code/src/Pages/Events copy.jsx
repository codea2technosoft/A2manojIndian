...
const BookmakerTable = memo(
  ({
    ...
    matchesName,
    onMyBookClick, // Add this prop
    onMyBookClicktotel, // Add this prop
    isFetchingExposure, // Add this prop
  }) => {
    ...
    const getExposureAmount = (runnerName) => {
      ...
      if (matchesName && Array.isArray(matchesName)) {
        ...
        if (team) {
          // सबसे पहले team_id से check करें
          if (bookmakerExposureData[team.team_id] !== undefined) {
            return bookmakerExposureData[team.team_id];
          }

          // string team_id से check करें
          if (bookmakerExposureData[team.team_id?.toString()] !== undefined) {
            return bookmakerExposureData[team.team_id.toString()];
          }

          // team_name से check करें (backup)
          if (bookmakerExposureData[team.team_name] !== undefined) {
            return bookmakerExposureData[team.team_name];
          }
        }
      }
...
function Cricket() {
  ...
  // Add loading state for exposure API
  ...
  // Updated fetchExposuremy function
  const fetchExposuremy = useCallback(async () => {
    try {
      if (!event_id) {
      ...
      console.log("✅ Fetching match total exposure for event_id:", event_id);
      ...
      console.log("🔍 Bookmaker data type:", typeof response.data.bookmaker);
      console.log("🔍 Bookmaker data:", response.data.bookmaker);
  ...
  // Updated totalexposermy function
  const totalexposermy = useCallback(async () => {
    try {
      ...
      if (response.data?.status_code === 1) {
        ...
        console.log("✅ Final MY Bookmaker exposure map:", bookmakerMap);
        setBookmakerExposureData(bookmakerMap);
      ...
      console.error("❌ Error fetching MY exposure:", error);
    } finally {
      setIsFetchingExposure(false);
    }
  }, [event_id, role, admin_id]);
  ...
  const handleMyBookClicktolet = useCallback(() => {
    console.log("📘 Ttl Book button clicked");
    fetchExposuremy();
  }, [fetchExposuremy]);
  ...
  const fetchFancyExposureData = useCallback(async () => {
    try {
      ...
      const role = localStorage.getItem("role");
      const admin_id = localStorage.getItem("admin_id");
      const token = localStorage.getItem("token") || sessionStorage.getItem("token"); // Add token here
  ...
  }, [event_id, fancylist]); // Remove token from dependencies since it's defined inside
  ...
  useEffect(() => {
    ...
    const intervalTime =
      showLoginModal || isLadderOpen ? 600000 : 6000;
