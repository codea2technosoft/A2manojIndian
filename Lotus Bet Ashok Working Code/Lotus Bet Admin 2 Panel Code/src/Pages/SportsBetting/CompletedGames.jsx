...
export default function Dashboard() {
  ...
  const fetchMatchResultList = async (pageNo = page) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setProfitLoss(res.data.summary.pl);
      }
    } catch (err) {
  ...
  const handlecancelhistory = (agent) => {
    navigate(`/Cancelhistory/${agent.event_id}`);
  };

  const handleCompletedSession = (agent) => {
    navigate(`/Completedsession/${agent.event_id}`);
  };
  const handleMatchSessionall = (agent) => {
    navigate(`/MatchSessionall/${agent.event_id}`);
  };
            ...
            <div className="row mb-3 align-items-center">
              <div className="col-md-6">
                <div className="d-flex">
                  <button
                    className={`btn ${ProfitLoss < 0 ? "btn-danger" : "btn-success"}`}
                  >
                    Profit & Loss ₹{Number(ProfitLoss || 0).toFixed(2)}
                  </button>
                  ...
                  {gamesLoading ? (
                    ...
                    marketData.map((game, index) => (
                      <tr key={game._id || game.event_id}>
                        ...
                        <td
                          style={{
                            color: Number(game.pl) < 0 ? "red" : "green",
                            fontWeight: "bold"
                          }}
                        >
                          {game.pl ? Number(game.pl).toFixed(2) : "0.00"}                        </td>
                        {/* <td>
                          <button
