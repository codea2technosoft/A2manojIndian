...
import { getAllMatches, deleteMatch, toggleMatchStatus, refreshMatches, importEvent } from "../../Server/api";
import { FaEye, FaTrashAlt, FaDownload, FaSpinner, FaList } from "react-icons/fa";
...
function Cricketlist() {
  ...
  const fetchGames = async (page = pagination.page, limit = pagination.limit) => {
    try {
      setLoading(true);
      const params = {
        page,
  ...
  // Fetch matches for a specific series - integrated directly in component
  const getSeriesMatches = async (seriesId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://apileo.leobook.in/get-matches?series_id=${seriesId}`, {
        headers: {
  ...
  const fetchSeriesMatches = async (seriesId, seriesName) => {
    try {
      ...
      // Assuming API returns data in the format you mentioned
  ...
  const handleRefresh = async () => {
    try {
      ...
      if (data.success) {
        ...
        showToast(
          `${data.addedCount} new matches added`,
          "success"
        );
      } else {
  ...
  const handleImportEvent = async (game) => {
    try {
      ...
      if (response.data.success) {
        showToast(response.data.message || "Events imported successfully!", "success");
                ...
                <tbody>
                  {seriesMatches.map((match, index) => (
                    <tr key={match.event?.id || index}>
                      ...
                      <td>
                        {match.event?.openDate ? formatDate(match.event.openDate) : 'N/A'}
                      </td>
                      <td>
                      ...
                      <td>
                        <code className="small">{match.event?.id || 'N/A'}</code>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {games.length > 0 ? games.map((game, index) => (
                ...
                <tr>
                  <td colSpan="5" className="text-center py-4">
                    ...
                    <button className="btn btn-primary mt-2" onClick={fetchGames}>
                      Refresh
                    </button>
                  </td>
                </tr>
          ...
          <div className="card-footer">
            <div className="d-flex justify-content-between align-items-center">
              <span className="text-muted">
                Page {pagination.page} of {pagination.totalPages} • Total: {pagination.total} matches
              </span>
