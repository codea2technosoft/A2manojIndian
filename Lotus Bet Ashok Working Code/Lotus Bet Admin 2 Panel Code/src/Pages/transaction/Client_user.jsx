...
import { getChildList_listnewUser, addNewTransactionuser, getMyLedgerTxn_user, deleteLedgerTxn_user } from "../../Server/api";
...
function AgentMasterTransction() {
    ...
    const handleAutoSelectClient = async (adminId) => {
        try {
            setLoading(true);
            const response = await getChildList_listnewUser(5, admin_id);
            ...
            if (response.data && response.data.success) {
                ...
                const processedClients = clientData.map(user => {
                    // Note: For clients, total_amount seems to be positive in the example
                    // But we need to check the actual meaning - usually positive means client owes money (DENA)
                    let amount = Math.abs(user.total_amount || 0);
                    let type = "CLEAR"; // Default type

                    // IMPORTANT: Need to confirm with business logic
                    // Usually for clients:
                    // - Positive total_amount = Client owes you (DENA for you, LENA for client)
                    // - Negative total_amount = You owe client (LENA for you, DENA for client)

                    if (user.total_amount > 0) {
                        type = "DENA"; // Client owes you money
                    } else if (user.total_amount < 0) {
                        type = "LENA"; // You owe client money
                    }
    ...
    const confirmDelete = async (txnId, adminId) => {
        try {
            ...
            const res = await deleteLedgerTxn_user({
                admin_id: adminId,
                ledger_id: txnId,
            });
    ...
    const fetchLedgerData = async (adminId, pageNo = 1, filter = "all") => {
        ...
        try {
            ...
            const response = await getMyLedgerTxn_user(params);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const transactionData = {
                admin_id: formData.client,  // The client we're transacting with
            ...
            setLoading(true);
            const response = await addNewTransactionuser(transactionData);
        ...
        return (
            <div className="card mt-4">
                <div className="card-header bg-color-black">
                    <h3 className="card-title text-white mb-0">Client Details</h3>
                </div>
                <div className="card-body">
                    <div className="row mb-4">
                        <div className="col-md-12">
                            <div className="text-center border-danger">
                                <div className="d-flex align-items-end gap-2 master-txn justify-content-between">
  ...
  transaction?.post_date
    ...
    : "No Date"
                                                    ...
                                                    <td>
                                                        ...
                                                        {/* {!transaction.is_delete_admin_id && (
                                                            <button
                                                                className="btn btn-sm btn-outline-danger"
                                                                onClick={() =>
                    ...
                    <div className="card">
                        <div className="card-header bg-color-black">
                            <div className="d-flex align-items-center justify-content-between">
                                <h3 className="card-title text-white mb-0">Client Transaction</h3>
