...
//                         ) : deposits.length === 0 ? (
//                           <tr>
//                             <td colSpan="13" className="text-center">No Record Found.</td>
//                           </tr>
//                         ) : (
//                           deposits.map((deposit, idx) => {
//                             const imageUrl = getImageUrl(deposit.screenshot);
...
//                             const isOnlinePayment = deposit.payment_type === 'online';
//                             const gatewayName = deposit.getway_name || deposit.gateway_name || '';
...
//                                 </td>
//                                 <td>{formatDateTime(deposit.createdAt || deposit.requestDate)}</td>
//                                 <td>{deposit.agent?.username || deposit.agent_name || "-"}</td>

//                                 {/* ============= ACTION COLUMN ============= */}
//                                 <td>
//                                   {/* ✅ Agar Online Payment hai toh "Online" + Gateway Name Show karo (Pure Text Style) */}
//                                   {isOnlinePayment ? (
...
//                                     </div>
//                                   ) : deposit.status === 'pending' ? (
//                                     /* ✅ Agar Pending hai toh Approve/Decline Buttons (Original Style) */
//                                     <div className="d-flex gap-1">
//                                       <button
//                                         className="btn btn-sm btn-success"
//                                         onClick={() => handleOpenStatusModal(deposit._id, 'approve')}
...
//                                     </div>
//                                   ) : (
//                                     /* ✅ Agar Status Approved/Rejected hai toh Badge */
//                                     <span className={`badge ${deposit.status === 'approved' || deposit.status === 'success' ? 'bg-success' : 'bg-danger'}`}>
//                                       {deposit.status || "pending"}
//                                     </span>
//                                   )}
//                                 </td>
...
//   // 🔥 REFS FOR SOUND MONITORING
//   const prevIds = useRef([]);
//   const isFirstRun = useRef(true);

//   // ============= 🎵 100% WORKING SOUND (Mobile + Desktop) =============
//   const playSound = () => {
...
//       audio.src = "data:audio/wav;base64,UklGRlQAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQAAAACAgICAf39/f3+AgICAf39/f3+AgICAf39/f3+AgICAf39/f3+AgICAf39/f3+AgICAf39/f38AAAA=";
//       audio.volume = 1.0;
//       audio.play()
//         .then(() => console.log("✅ Sound played!"))
//         .catch(err => console.log("⚠️ Auto-play blocked:", err.message));
//     } catch (e) {}
...
//       //     body: 'Check admin panel for pending deposits',
//       //     icon: '🔔',
//       //     vibrate: [200, 100, 200],
...
//     // 🔥 4. Alert (Last resort - 100% guaranteed)
//     setTimeout(() => {
//       alert("🔔 New Deposit Request!\nCheck the list.");
//     }, 300);
//   };
//   // ===================================================
...
//       if (response?.data?.success) {
//         const newDeposits = response.data.data || [];
        
//         // 🔥 SOUND CHECK
//         if (!isFirstRun.current) {
//           const currentIds = prevIds.current;
//           const newIds = newDeposits.map(d => d._id);
//           const addedIds = newIds.filter(id => !currentIds.includes(id));
          
//           if (addedIds.length > 0) {
//             console.log("🔔 NEW DEPOSIT FOUND!", addedIds);
//             playSound(); // 🎵 SOUND + VIBRATION + NOTIFICATION + ALERT
//           }
          
//           prevIds.current = newIds;
//         } else {
//           prevIds.current = newDeposits.map(d => d._id);
//           isFirstRun.current = false;
//           console.log("🟢 Monitoring started:", prevIds.current.length, "deposits");
//         }
...
//   // 🔥 USE EFFECT - AUTO REFRESH EVERY 5 SECONDS
//   useEffect(() => {
...
const AdminDepositList = () => {
  ...
  // ============= 🔥 USE EFFECT 1: Initial Load + Auto Refresh =============
  useEffect(() => {
    ...
    // 🔥 Auto-refresh every 5 seconds (background mein)
    const interval = setInterval(fetchDeposits, 120000);
    
    return () => clearInterval(interval);
  }, []); // ✅ EMPTY ARRAY — Sirf ek baar run hoga

  // ============= 🔥 USE EFFECT 2: Filter/Page Change Par Fetch =============
  useEffect(() => {
    fetchDeposits();
  }, [currentPage, activeFilters]); // ✅ Sirf page/filter change par run hoga
