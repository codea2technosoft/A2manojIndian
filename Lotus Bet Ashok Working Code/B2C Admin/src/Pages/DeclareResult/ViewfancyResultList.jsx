...
import {
  ...
  MdOutlineKeyboardArrowRight, MdRemoveRedEye
} from "react-icons/md";

import { useParams } from "react-router-dom";
import { useNavigate } from "react-router-dom";
import { getfancyrollbacklist, rollbackFancyNow1 } from "../../Server/api";
...
function ViewfancyResultList() {
  ...
  const fetcfancyResultList = async () => {
    try {
      ...
      const res = await getfancyrollbacklist(payload);
  ...
  return (
    <div className="card">
      <div className="card-header d-flex justify-content-between align-items-md-center">
              ...
              fancyResultList.map((item, index) => (
                <tr key={item._id}>
                  ...
                  <td style={{ width: "260px" }}>
                    ...
                    >
                      {btnLoading["rollback_" + item.fancy_id]
                        ? "..."
                        : "Rollback"}
                    </button>
                  </td>
