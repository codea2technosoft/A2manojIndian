  ...
  const fetchRiskManagementData = async () => {
    ...
    try {
      ...
      if (response?.data?.data) {
        ...
        const titles = {};
        if (
          response.data.data.match_odds &&
          response.data.data.match_odds.length > 0
        ) {
          titles.matchOdds = "Match Odds";
        }
        if (
          response.data.data.book_maker &&
          response.data.data.book_maker.length > 0
        ) {
          titles.bookMaker = "Book Maker";
        }
        if (
          response.data.data.fancy_bet &&
          response.data.data.fancy_bet.length > 0
        ) {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        if (fancyItem.SelectionId) {
          matchedData = response.data.filter(
            (item) =>
        ...
        if (matchedData.length > 0) {
          ...
          transformedData.forEach((item) => {
            ...
            if (odd > 0) {
              [-1, 0, 1].forEach((offset) => {
                ...
                if (existing) {
                  existing.profitLoss =
                    parseFloat(existing.profitLoss) + parseFloat(pl);
                } else {
          ...
          let displayName =
            fancyItem.fancy_name ||
            fancyItem.RunnerName ||
            fancyItem.event_name ||
            "Fancy Bet";
          if (fancyItem.SelectionId) {
          ...
          setSelectedUser([
            {
              run: "No Data",
              profitLoss:
                "No match found for Selection ID: " + fancyItem.SelectionId,
            },
          ]);
      ...
      if (response.data) {
        ...
        if (data.rows && Array.isArray(data.rows) && data.rows.length > 0) {
          ...
          const rows = data.rows.map((item) => {
            const row = {};
            row["Downline"] =
              item.downline || item.downline_raw || item.agent_id || "Unknown";
    ...
    try {
      ...
      if (response.data) {
        ...
        if (data.rows && Array.isArray(data.rows) && data.rows.length > 0) {
          ...
          titleText =
            data.data.title || data.title || "Client Wise Downline P/L";
        }
        ...
        if (rowsData.length > 0) {
          ...
          if (!headers.includes("Client")) {
            ...
            if (clientFieldFound) {
              headers = [
                clientFieldFound,
                ...headers.filter((h) => h !== clientFieldFound),
              ];
            } else {
  ...
  const fetchMatchOddsData = async (marketId, eventId, index) => {
    try {
      ...
      try {
        ...
        if (
          teamsResponse.data?.status_code === 1 &&
      ...
      if (
        ...
        if (
          ...
          const transformedMatches = marketData.runners.map((runner, index) => {
            ...
            if (
              runner.runnerName &&
              teamNameMap[runner.runnerName.toLowerCase()]
            ) {
              teamName = teamNameMap[runner.runnerName.toLowerCase()];
            } else if (
              teamNamesFromApi.length > index &&
              teamNamesFromApi[index]
            ) {
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          const transformedBookMaker = marketData.runners.map(
            (runner, index) => {
    ...
    return dataArray.map((item, index) => {
      ...
      const bookPL1 =
        teamEntries.length > 0
          ? { team: teamEntries[0][0], value: teamEntries[0][1] }
          : null;
      const bookPL2 =
        teamEntries.length > 1
          ? { team: teamEntries[1][0], value: teamEntries[1][1] }
          : null;
      ...
      if (
        type === "match_odds" &&
        item.book_pl?.formatted &&
        item.book_pl.formatted.length > 0
      ) {
                    ...
                    <table className="table">
                      <tbody>
                        ...
                        <tr>
                          <td
                            width="8%"
                            className="bg-light-yellow border-0 text-center"
                          >
                            <strong>1</strong>
                          </td>
                          <td
                      ...
                      {matchOddsDataFinal.map((item, index) => {
                        const oddsKey = `match-${index}`;
                        const currentOdds =
                          oddsData[oddsKey] || item.selections;
                          ...
                          <tbody key={item.id || index} className="match-tbody">
                            <tr>
                              <td className="bg-yellow border-0">
                                <a className="d-flex align-items-center">
                                  <button
                                    onClick={() =>
                                      toggleMatchOdds(index, item.marketId)
                                    }
                                    type="button"
                              ...
                              <td className="border-0 bg-yellow text-center"></td>
                              <td className="border-0 bg-yellow text-center">
                                {item.bookPL2 && (
                                  <div className="d-flex justify-content-between flex-column gap-1">
                                    <p className="fw-bold">
                            ...
                            {activeMatchIndex === index && (
                              <tr>
                                <td
                                  colSpan="4"
                                  className="px-0 gray-inner_table"
                                >
                                  {isLoading ? (
                                            ...
                                            <tr key={selIndex}>
                                              ...
                                              >
                                                <div className="light-blue rounded-0">
                                                  <strong>
                                                    {selection.back3.odds}
                                                  </strong>
                                                  <span className="d-block">
                                                    {selection.back3.amount}
                                                  </span>
                                                </div>
                                              </td>
                                              <td
                                                className="back-2 p-0"
                                                style={{
                                                  cursor: "not-allowed",
                                                }}
                                              >
                                                <div className="light-blue rounded-0">
                                                  <strong>
                                                    {selection.back2.odds}
                                                  </strong>
                                                  <span className="d-block">
                                                    {selection.back2.amount}
                                                  </span>
                                                </div>
                                              </td>
                                              <td
                                              ...
                                              >
                                                <div className="light-blue rounded-0">
                                                  <strong>
                                                    {selection.back1.odds}
                                                  </strong>
                                                  <span className="d-block">
                                                    {selection.back1.amount}
                                                  </span>
                                                </div>
                                              </td>
                                              <td
                            ...
                            <tr>
                              <td className="bg-yellow border-0">
                                <a className="d-flex align-items-center">
                                  <button
                                    onClick={() =>
                                      toggleBookmaker(index, item.eventId)
                                    }
                                    type="button"
                                    className="angle-up down-up btn btn-primary me-1"
                                  >
                              ...
                              <td className="border-right-0 text-center">
                                <a
                                  onClick={(e) =>
                                    handleBookmakerDownlinePLClick(
                                      item.eventId,
                                      item.apiType,
                                      e,
                                    )
                                ...
                                <a
                                  onClick={(e) => {
                                    ...
                                    const fancyData = {
                                      ...
                                      fancy_name:
                                        item.fancy_name || item.marketType,
                              ...
                              <tr>
                                ...
                                >
                                  {isLoading ? (
                                    ...
                                    <div className="text-center py-3">
                                      <span className="text-muted">
                                        No fancy odds data available for this
                                        selection
                                      </span>
                                    </div>
                                  )}
            ...
            >
              <div className="modal-content">
                ...
                <div className="modal-body p-0">
                  ...
                  ) : downlineData.headers &&
                    downlineData.headers.length > 0 &&
                    downlineData.rows &&
                    downlineData.rows.length > 0 ? (
                    <div className="table-responsive account-table risk-pop">
                      <table className="table table-bordered">
                        ...
                        <tbody>
                          ...
                          {downlineData.totalValues &&
                            downlineData.totalValues.length > 0 && (
        ...
        <Modal.Body>
          <section className="account-table">
            <div className="container-fluid">
              ...
              ) : clientDownlineData.rows &&
                clientDownlineData.rows.length > 0 ? (
                <div className="responsive">
                  <table className="table table-bordered">
                    <thead>
                      <tr>
                        {clientDownlineData.headers &&
                          clientDownlineData.headers.map((header, index) => (
                            <th key={index} scope="col">
                    ...
                    <tbody>
                      {clientDownlineData.rows.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                          {clientDownlineData.headers &&
                            clientDownlineData.headers.map(
                              (header, colIndex) => (
                                <td key={colIndex}>
                                  {header === "Client" ? (
                      ...
                      {clientDownlineData.totalValues &&
                        clientDownlineData.totalValues.length > 0 && (
                          <tr style={{ fontWeight: 600 }}>
                            {/* <td className="text-start"><strong>Total</strong></td> */}
                            {clientDownlineData.totalValues.map(
                              (value, idx) => (
                                <td key={idx}>
                                  <span
                                    className={
            ...
            {loadingEventBets ? (
              ...
              <div className="responsive">
                <table className="table table-bordered">
                  <thead>
                    <tr>
                      {eventBetsData.headers &&
                        eventBetsData.headers.map((header, index) => (
                          <th key={index} scope="col">
                            {header}
                          </th>
                        ))}
                  ...
                  <tbody>
                    {eventBetsData.rows.map((row, rowIndex) => (
                      <tr key={rowIndex}>
                        {eventBetsData.headers &&
                          eventBetsData.headers.map((header, colIndex) => (
                            <td key={colIndex}>
                              {header === "Client" ? (
                                <strong>{row[header] || "N/A"}</strong>
                              ) : header === "Total Stake" ||
                                header === "Total Liability" ||
                                header === "Total Bet Win" ||
                                header === "Total" ? (
                                <span
                                  className={
                                    parseFloat(row[header] || 0) < 0
                    ...
                    {eventBetsData.totalValues &&
                      eventBetsData.totalValues.length > 0 && (
                        <tr
                          style={{
                            fontWeight: 600,
