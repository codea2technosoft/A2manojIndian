...
export default function RegisteruserdashboardRecent() {
  ...
  const fetchRecentRegistrations = async (page = 1) => {
    setLoading(true);
    try {
      const params = {
        page: page,
      ...
      if (response?.data?.success) {
        ...
        if (data.recent_registrations) {
          setRecentData(data.recent_registrations.data || []);

          if (data.recent_registrations.pagination) {
            setPagination({
              page: data.recent_registrations.pagination.page || 1,
              limit: data.recent_registrations.pagination.limit || 50,
              total: data.recent_registrations.pagination.total || 0,
              totalPages: data.recent_registrations.pagination.totalPages || 1
            });
          }
        }
      }
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      const response = await getRegistrationStats(params);
      if (response?.data?.success) {
        const data = response.data.recent_registrations?.data || [];
        if (data.length === 0) {
        ...
        link.setAttribute("download", `recent_users_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="allcommon">
      <section className="main-inner-outer py-4">
        <div className="db-sec">
          <h2 className="common-heading">Recent Users</h2>
        </div>
