...
const Banking = () => {
  ...
  // Calculate totals
  // const totals = members.reduce((acc, member) => ({
  //   balance: acc.balance + member.coins,  // <-- coins use karo
  //   availableDW: acc.member.coins + acc.balance,
  //   exposure: acc.exposure + member.exposure,
  //   creditReference: acc.creditReference + member.creditReference,
  //   referencePL: acc.referencePL + member.referencePL
  // }), { balance: 0, availableDW: 0, exposure: 0, creditReference: 0, referencePL: 0 });
  ...
  const handleDWToggle = (id, type) => {
    ...
    // Set fullbutton based on type
    if (type === 'withdraw') {
      setfullbutton(prev => ({
        ...prev,
        [id]: false  // W click - Full button active (false = enabled)
      }));
    } else if (type === 'deposit') {
      setfullbutton(prev => ({
        ...prev,
        [id]: true   // D click - Full button inactive (true = disabled)
      }));
    }
    ...
    // Show error message immediately on click
    setWithdrawErrors(prev => ({
      ...prev,
  ...
  const handleDWValueChange = (id, value) => {
    ...
    if (member && selectedTransactionType[id] === 'withdraw' && numValue > 0) {
      ...
      // Deposit - clear error if amount is valid
      setWithdrawErrors(prev => ({
        ...prev,
        [id]: ''
      }));
  ...
  //   // Check for withdrawal errors before submitting
  //   let hasError = false;
  //   const transactions = [];
  //   members.forEach(member => {
  //     const amount = parseFloat(dwValues[member.id]);
  //     if (amount && amount > 0) {
  //       const type = selectedTransactionType[member.id] || 'withdraw';

  //       // Validate withdrawal
  //       if (type === 'withdraw' && amount > member.availableDW) {
  //         hasError = true;
  //         setWithdrawErrors(prev => ({
  //           ...prev,
  //           [member.id]: 'Withdrawal amount should not be greater than available balance'
  //         }));
  //         return;
  //       }
  ...
  //       text: 'Please fix withdrawal errors before submitting',
  ...
  //     // Direct success alert - NO OK button, auto close
  //     Swal.fire({
  //       icon: 'success',
  //       title: 'Success!',
  //       text: successMessage,
  ...
  //     // Reset form and refresh data immediately (without waiting for alert)
  //     setDwValues({});
  //     setRemarks({});
  //     setPassword('');
  ...
  //     if (!dwValues[member.id] || dwValues[member.id] === '' || amount <= 0) {
  //       hasError = true;
  //       setWithdrawErrors(prev => ({
  //         ...prev,
  //         [member.id]: 'Amount should be greater than 0'
  //       }));
  //       return;
  //     }
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    // If type selection error exists
    if (hasTypeError) {
      Swal.fire({
        icon: 'error',
  ...
  // Handle log click for individual row - navigate to agentwise banking log page
  const handleLogClick = (adminId) => {
    console.log('View logs for:', adminId);
    //navigate(`/agentwisebanking-debit-credit-log?agentId=${adminId}`);
    window.open(`/agentwisebanking-debit-credit-log?agentId=${adminId}`, '_blank');
  };

  // Handle header Logs button click - navigate to Agentbankingtransctionhistory
  // const handleHeaderLogsClick = () => {
  //   navigate('/Agentbankingtransctionhistory', '_blank');
  // };
              ...
              <div className="common-container">
                ...
                {/* <div className="bet_status bank_balance_detail d-sm-flex align-items-center my-1 my-sm-3">
                  ...
                  <strong>
                    ...
                    {summaryData?.formatted?.available_balance || '0.00'}
                  </strong>
                </div> */}
                  ...
                  <div className="responsive">
                    <table className="banking_detail_table table-color table">
                      ...
                      <tbody>
                        ...
                        {/* Total Row - Show only if members exist */}
                        {members.length > 0 && (
