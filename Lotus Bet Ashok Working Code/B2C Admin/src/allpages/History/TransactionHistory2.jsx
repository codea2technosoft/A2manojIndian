...
const TransactionHistory2 = () => {
  ...
  const fetchUserProfile = async () => {
    ...
    try {
      ...
      // 🔥 FIX: Response mein "user_profile" hai, "admin_profile" nahi
      ...
      if (userIdFromProfile) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Could not find user ID. Please check console.",
          confirmButtonText: "OK",
        });
      }
  ...
  const fetchTransactions = async (page = 1, userIdParam = null) => {
    ...
    console.log("📤 fetchTransactions - userIdToUse:", userIdToUse);
    console.log("📤 fetchTransactions - adminId:", adminId);
    ...
    try {
      const payload = {
        admin_id: adminId,
        user_id: userIdToUse,  // 🔥 YEH IMPORTANT HAI
        type: "all",
        page: page,
        limit: itemsPerPage,
        start_date: null,
  ...
  useEffect(() => {
    if (userId) {
      console.log("🔄 userId changed, fetching transactions for page:", currentPage);
      fetchTransactions(currentPage, userId);
    }
  }, [currentPage, userId]);
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {transactions && transactions.length > 0 ? (
                        transactions.map((transaction, index) => (
                          <tr key={transaction._id || index}>
                            <td>{formatDate(transaction.date || transaction.created_at)}</td>
                            <td>
                              {transaction.credit && transaction.credit > 0 && transaction.type !== 'withdraw' ? (
                                <span className="text-success">{transaction.credit.toFixed(2)}</span>
                              ) : "-"}
                            ...
                            <td>-</td>
                            <td>{transaction.balance || transaction.wallet_amount ? (transaction.balance || transaction.wallet_amount).toFixed(2) : '0.00'}</td>
                            <td>
                              {transaction.description || transaction.remark || '-'}
                            </td>
                            <td>{getFromToLabel()}</td>
                          </tr>
                        ))
