                      ...
                      <tr key={user._id || index}>
                        ...
                        <td className="text-end">
                          <Link onClick={() => handleExposureClick(user._id, user.username || user.name)}>
                            <span className="status-suspend1">{user.exposure || '0.00'}</span>
                          </Link>
                        </td>
                        {/* ====================================== */}
                        ...
                        <tbody>
                          {exposureData && exposureData.length > 0 ? (
                            exposureData.map((item, index) => (
                              <tr key={index}>
                                <td className="text-center">{item.match_name || item.event_name || '-'}</td>
                                <td className="text-center">{item.market_name || item.fancy_name || '-'}</td>
                                <td className="text-center">{item.type || item.bet_type || '-'}</td>
                                <td className="text-center text-danger fw-bold">
                                  ₹ {item.exposure || item.amount || 0}
                                </td>
                              </tr>
                            ))
                          ) : (
