...
import { getAlltwithdrawList } from "../../Server/api";

const AdminWithdrawReport = () => {
  ...
  const calculateTodaysDeposit = async () => {
    try {
      ...
      const res = await getAlltwithdrawList(params);
  ...
  const fetchReport = useCallback(
    async (
      ...
      try {
        ...
        setTodaysDeposit(0);
        setError(
          error.response?.data?.message || "Failed to load deposit records",
        );
      } finally {
        setLoading(false);
      }
      ...
      <div className="row mb-3">
        <div className="col-md-3 mb-3">
          <div className="card text-center shadow-sm border-success summary-card">
            <div className="card-body">
              <h5 className="text-success mb-1">Total Withdraw</h5>
        ...
        <div className="col-md-3 mb-3">
          <div className="card text-center shadow-sm border-primary summary-card">
            <div className="card-body">
              <h5 className="text-primary mb-1">Today's Withdraw</h5>
                    ...
                    <tbody>
                      {data.map((item, index) => {
                        const depositAmount = Number(
                          item.amount || item.depositAmount || 0,
                        );
                        const itemIsToday = isToday(
                          item.createdAt || item.date,
                        );

                        return (
                          <tr
                            key={`${item.id || item._id || index}-${currentPage}`}
                          >
        ...
        {data.length >= 0 && (
          <div className="card-footer d-flex justify-content-between align-items-center">
            <span className="text-muted small">
              ...
              {Math.min(currentPage * limit, data.length)} of {data.length}
            </span>
