...
function AgentLists() {
      ...
      if (response.data && response.data.success) {
        ...
        if (response.data.summary) {
          setSummary({
            ...
            total_available_balance: response.data.summary.total_available_balance || '0.00',
            balance: response.data.summary.balance || '0.00',
            available_balance: response.data.summary.available_balance || '0.00',
            total_player_balance: response.data.summary.total_player_balance || '0.00'
          });
        }
      } else if (Array.isArray(response.data)) {
        setAgentList(response.data);
  ...
  const handleCreateAgent = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        Swal.fire("Error", response.data?.message || "Failed to create agent", "error");
      }
    } catch (error) {
  ...
  const fetchMarketList = async (agent) => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        Swal.fire("Error", response.data?.message || "Failed to fetch market list", "error");
      }
    } catch (error) {
      Swal.fire("Error", error.response?.data?.message || "Something went wrong", "error");
    } finally {
  ...
  const confirmMarketStatusUpdate = async () => {
    ...
    try {
      const payload = {
        agent_id: selectedAgentForMarket?.admin_id || selectedAgentForMarket?._id,
      ...
      if (response.data && response.data.success) {
        Swal.fire(
          ...
          response.data.message || `Market ${actionType === "active" ? 'activated' : 'inactivated'} successfully!`,
          "success"
        );
        ...
        Swal.fire("Error", response.data?.message || "Failed to update market status", "error");
      }
    } catch (error) {
      Swal.fire("Error", error.response?.data?.message || "Something went wrong", "error");
    }
  ...
  // ===== OLD FUNCTION (COMMENTED - NOT DELETED) =====
        ...
        <section className="total-balance-sec d-flex justify-content-center align-items-center">
          <ul className="list-unstyled detail-header mb-2" style={{ width: "98.5%" }}>
            ...
            <li>
              ...
              <strong>INR {summary.total_player_balance}</strong>  {/* ✅ Correct */}
            </li>
          </ul>
        </section>
                    ...
                    <tr key={index}>
                      ...
                      <td className="text-end">
                        <Link to="#" className="text-primary">
                          {agent.credit_ref || '0.00'} <i className="fas fa-pen ps-1" />
                        </Link>
                      </td>
                      <td className="text-end">
                        <Link to="#" className="text-primary">
                          {agent.withdraw_limit || '0.00'} <i className="fas fa-pen ps-1" />
                        </Link>
                      </td>
                      ...
                      <td className="text-end ul-t">
                        <span style={{ color: "red" }}>{agent.reference_pl || '0.00'}</span>
                      </td>
                      <td className="text-end">
                        <span style={{ color: "green" }}>{agent.current_pl || '0.00'}</span>
                      </td>
                      ...
                      <td className="action_link text-end">
                        ...
                        <a title="Change Status" className="btn" onClick={() => openChangeStatusModal(agent)}>
                ...
                <div className="modal-body">
                  <div className="test-status">
                    <div className="status-row d-flex justify-content-between align-items-center">
                      <h6 className="mb-0">
                        <span className="me-1">Agent </span> {selectedAgentForPassword?.name || selectedAgentForPassword?.username || 'Agent'}
                      </h6>
              ...
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="common-heading">
                    Block Market - {selectedAgentForMarket?.name || selectedAgentForMarket?.username || 'Agent'}
                  </h5>
                  <button
                    type="button"
                    className="btn-close"
                          ...
                          {marketList && marketList.length > 0 ? (
                            marketList.map((item, index) => (
                              <tr key={index}>
                                ...
                                <td className="text-center">{item.betfair_id || item.market_id || '-'}</td>
                                <td className="text-center">{item.name || item.market_name || '-'}</td>
                                <td className="text-center">
                                  <span className={item.status === 1 ? 'text-success' : 'text-danger'}>
                                    {item.status === 1 ? `${item.name || 'Market'} is ON` : `${item.name || 'Market'} is OFF`}
                                  </span>
                                </td>
                                <td className="text-center">
                                  <div className="form-check form-check-inline form-switch ps-0">
                                    <input
                                      ...
                                      onChange={() => handleMarketStatusUpdate(item, item.status)}
                                    />
              ...
              <div className="modal-content">
                ...
                <div className="modal-body">
                  ...
                  <div className="text-center">
                    <h6 className="mb-3">
                      You Want to {actionType === "active" ? "Active" : "InActive"} This Match?
                    </h6>
          ...
          >
            ...
            >
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <form className="super-admin-form" onSubmit={handleCreateAgent}>
                    <div className="row">
                      {/* Username */}
                    ...
                    <div className="mt-3 text-center">
                      <button type="submit" className="theme_dark_btn px-5 btn btn-primary" disabled={agentLoading}>
