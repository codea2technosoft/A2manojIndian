...
function Bettingprofitloss() {
        ...
        try {
            ...
            const mappedData = data.map((item) => {
                const baseData = {
                    ...
                    sport_name: item.sport_name || '',           // ✅ ADD
                    market_name: item.market_name || item.market || '',  // ✅ ADD
        ...
        try {
            ...
            const mappedDetails = details.map((item) => ({
                ...
                profit_loss: item.profit_loss || item.pl || 0,
                  sport_id: item.sport_id || '',  // ✅ ADD THIS
            }));
    ...
    //                                                     <dt className="me-2">Profit Amount</dt>
    //                                                     <dd className="mb-0">
    //                                                         {summary.backSubtotal >= 0 ? (
    //                                                             summary.backSubtotal
    //                                                         ) : (
    ...
    //                                                     <dt className="me-2">Loss Amount</dt>
    //                                                     <dd className="mb-0">
    //                                                         {summary.laySubtotal >= 0 ? (
    //                                                             summary.laySubtotal
    //                                                         ) : (
    ...
    //                                                     </dd> */}
    //                                                     <dt className="me-2">Commission</dt>
    //                                                     <dd className="mb-0 text-success">
    //                                                         {summary.commission || 0}
    //                                                     </dd>
    ...
    //                                                     <dt className="me-2">Profit & Loss </dt>
    //                                                     <dd className="mb-0">
    //                                                         {summary.marketSubtotal >= 0 ? (
    //                                                             summary.marketSubtotal
    //                                                         ) : (
...
//     // ✅ CHECK IF CASINO TAB
//     const isCasino = activeTab === 'casino' || activeTab === 'Indian Casino';
...
//                                     <tr>
//                                         {isCasino ? (
//                                             // ✅ CASINO TAB - Only 4 columns (Type removed)
//                                             <>
//                                                 <th>User Name</th>
//                                                 <th>Bet ID</th>
//                                                 <th>Placed</th>
...
//                                     ) : (
//                                         <tr>
//                                             <td colSpan={isCasino ? 4 : 8} className="text-center">
//                                                 No details available
//                                             </td>
//                                         </tr>
//                                     )}
...
//                                     {summary && (
//                                         <tr>
//                                             <td colSpan={isCasino ? 4 : 8} className="sum-pl">
//                                                 <dl className="d-flex flex-wrap justify-content-between m-0">
//                                                     <dt className="me-2">Total Stakes</dt>
//                                                     <dd className="mb-0">{summary.totalStakes || 0}</dd>
...
const renderDetailView = () => {
    ...
    // ✅ CHECK - Casino tab OR sport_id === "10"
    const isCasino = activeTab === 'casino' || activeTab === 'Indian Casino' || 
        (bets.length > 0 && bets[0]?.sport_id === "10");
                                    ...
                                    <tr>
                                        <td colSpan={isCasino ? 4 : 8} className="sum-pl">
                                            <dl className="d-flex flex-wrap justify-content-between m-0">
                                                ...
                                                <dt className="me-2">Profit Amount</dt>
                                                <dd className="mb-0">
                                                    {summary.backSubtotal >= 0 ? summary.backSubtotal : 
                                                        <span className="text-danger">({Math.abs(summary.backSubtotal)})</span>}
                                                </dd>
                                                <dt className="me-2">Loss Amount</dt>
                                                <dd className="mb-0">
                                                    {summary.laySubtotal >= 0 ? summary.laySubtotal : 
                                                        <span className="text-danger">{Math.abs(summary.laySubtotal)}</span>}
                                                </dd>
                                                <dt className="me-2">Commission</dt>
                                                <dd className="mb-0 text-success">{summary.commission || 0}</dd>
                                                <dt className="me-2">Profit & Loss</dt>
                                                <dd className="mb-0">
                                                    {summary.marketSubtotal >= 0 ? summary.marketSubtotal : 
                                                        <span className="text-danger">{Math.abs(summary.marketSubtotal)}</span>}
                                                </dd>
                                            </dl>
                                        </td>
                                    </tr>
                                        ...
                                        <React.Fragment key={index}>
                                            <tr style={rowStyle}>
                                                ...
                                                {displayFields.map((field, idx) => (
                                                    <td key={idx}>
                                                        {field === 'market' ? (
                                                            item.sport_name && item.market_name
                                                                ? `${item.sport_name}▸${item.market_name}`
                                                                : (item.market || '-')
                                                        ) : field === 'profit_loss' || field === 'pl' ? (
                                                            Number(item[field]) >= 0 ? Number(item[field]) : `(${Math.abs(Number(item[field]))})`
