...
//       'BookMaker': { sport: '4', market_type: 'bookmaker', bet_status: 'pending' },
//       'casino': { sport: '10', market_type: 'casino', bet_status: 'pending' },
...
//         <span style={{ color: bet.match_status == "3" ? "red" : "transparent" }}>
//           {bet.match_status == "3" ? "Pending" : ""}
//         </span>
//       </td>
//           </tr>
//         ));
...
//   // ✅ Get tab specific params - EXACT MATCH market_type
//   const getTabParams = (tabKey) => {
//     const paramsMap = {
//       'Exchange': { sport: '', market_type: '', bet_status: '' },
//       'FancyBet': { sport: '4', market_type: 'fancy', bet_status: 'pending' },
//       'BookMaker': { sport: '4', market_type: 'bookmaker', bet_status: 'pending' },
//       'casino': { sport: '10', market_type: 'casino', bet_status: 'pending' },
//       'Greyhound': { sport: '8', market_type: 'Greyhound', bet_status: 'pending' }, // ✅ Exact match
//       'HorseRacing': { sport: '7', market_type: 'Horse Racing', bet_status: 'pending' }, // ✅ Exact match
//       'Toss': { sport: '', market_type: 'toss', bet_status: 'pending' },
//       'Tie': { sport: '', market_type: 'tie', bet_status: 'pending' },
//       'lottery': { sport: '', market_type: 'lottery', bet_status: 'pending' },
//     };
...
//       // ✅ Client-side filtering based on sport (backup)
//       if (sport) {
//         bets = bets.filter(bet => bet.sport_id === parseInt(sport));
//         console.log(`✅ Filtered to ${bets.length} bets for sport_id: ${sport}`);
//       }
...
const getTabParams = (tabKey) => {
  const paramsMap = {
    ...
    'BookMaker': { sport: '4', market_type: 'bookmaker', bet_status: 'pending' },
    'casino': { sport: '10', market_type: 'casino', bet_status: 'pending' },
    'Greyhound': { sport: '8', market_type: '', bet_status: 'pending' },  // ✅ SPORT HATAYA
    'HorseRacing': { sport: '7', market_type: '', bet_status: 'pending' }, // ✅ SPORT HATAYA
    'Toss': { sport: '', market_type: 'toss', bet_status: 'pending' },
    'Tie': { sport: '', market_type: 'tie', bet_status: 'pending' },
    'lottery': { sport: '', market_type: 'lottery', bet_status: 'pending' },
  };
...
//       // Client-side filtering based on sport (BACKUP FILTER)
//       if (sport) {
//         bets = bets.filter(bet => bet.sport_id === parseInt(sport));
//         console.log(`✅ Filtered to ${bets.length} bets for sport_id: ${sport}`);
//       }
...
//     console.log(`📊 Total bets before filtering: ${bets.length}`);
...
//     } else if (tabKey === 'FancyBet') {
//       filteredBets = bets.filter(bet => bet.bet_type === 'fancy');
//       console.log(`✅ Filtered to ${filteredBets.length} Fancy bets`);
//     } else if (tabKey === 'BookMaker') {
//       filteredBets = bets.filter(bet => bet.bet_type === 'bookmaker');
//       console.log(`✅ Filtered to ${filteredBets.length} BookMaker bets`);
//     } else if (tabKey === 'casino') {
//       filteredBets = bets.filter(bet => parseInt(bet.sport_id) === 10);
//       console.log(`✅ Filtered to ${filteredBets.length} Casino bets`);
//     } else if (tabKey === 'Toss') {
//       filteredBets = bets.filter(bet => bet.market?.toLowerCase() === 'toss');
//       console.log(`✅ Filtered to ${filteredBets.length} Toss bets`);
//     } else if (tabKey === 'Tie') {
//       filteredBets = bets.filter(bet => bet.market?.toLowerCase() === 'tie');
//       console.log(`✅ Filtered to ${filteredBets.length} Tie bets`);
//     } else if (tabKey === 'lottery') {
//       filteredBets = bets.filter(bet => bet.market?.toLowerCase() === 'lottery');
//       console.log(`✅ Filtered to ${filteredBets.length} Lottery bets`);
//     }
//     // Exchange - no filter, show all

//     setBetData(filteredBets);
...
const fetchBets = async (tabKey) => {
  ...
  try {
    const params = {
      sport_id: sport,  // ✅ AB YE BHEJEGA
      market_type: market_type, // ✅ AB YE BHEJEGA
      bet_status: bet_status,
      from_date: '',
      to_date: '',
      bet_on: '',
    ...
    // ✅ Client-side backup filter
    let filteredBets = bets;
    if (sport && bets.length > 0) {
      filteredBets = bets.filter(bet => parseInt(bet.sport_id) === parseInt(sport));
      console.log(`✅ Filtered to ${filteredBets.length} bets for sport_id: ${sport}`);
    }
        ...
        return betData.map((bet, index) => (
          <tr key={bet.bet_id || index}>
            ...
            <td>
              ...
              //   ? `${bet.market} - ${bet.selection || ""}`
              //   : (bet.selection || "-")
               bet.game_name && bet.team && bet.bet_type
                    ? `${bet.game_name}▸${bet.team}▸${bet.bet_type}`
                    : bet.game_name && bet.team
                    ? `${bet.game_name}▸${bet.team}`
                    : (bet.market || bet.game_name || '-')}
