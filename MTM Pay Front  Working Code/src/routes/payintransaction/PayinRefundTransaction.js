...
// request
import { getOrders, getPayinRefundreport, exportOrders } from 'requests/order';

const { RangePicker } = DatePicker;

const titles = [{ title: 'All Transaction' }];

function PayinFailedTransaction() {
    ...
    const getRecords = async (query) => {
        try {
            ...
            const response = await getPayinRefundreport(query);
        ...
        try {
            ...
            if (query.created_at_date_min && query.created_at_date_max) {
                ...
                if (response && response.filepath) {
                    window.open(`https://api.mtmpay.in/files/${response.filepath}`, '_blank');
                } else {
    ...
    const columns = [
...

        {
            ...
            render: (payment_status) =>
                payment_status === 4 ? (
                    <span style={{ color: 'green', fontWeight: 'bold' }}>Refunded</span>
                ) : (
                    payment_status
                ),
        },
...
export default PayinFailedTransaction;
