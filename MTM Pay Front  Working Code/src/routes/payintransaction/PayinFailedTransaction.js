...
// request
import { getOrders, getPayinFaieldreport, exportOrders } from 'requests/order';
...
function PayinFailedTransaction() {
    ...
    const getRecords = async (query) => {
        try {
            ...
            const response = await getPayinFaieldreport(query);
    ...
    const columns = [
      ...
      
        {
            ...
            key: 'name',
            render: (text, record) => (
                <div>{record.name}<br></br>{record.email}</div>
            )
