...
function SuccessBetHistory() {
  ...
  const [limit, setLimitPages] = useState(100);
  const fillterdata = () => {
    ...
    async (type) => {
      ...
      try {
        const res = await fetch(
          `${process.env.REACT_APP_API_URL}/${endpoint}?page=1&limit=${limit}&user_id=${userid}`,
          {
            method: "GET",
            headers: {
        ...
        // alert(data.pagination.pageSize);
        setLimitPages(data.pagination.pageSize);
        if (type === "success") {
          setAllBets(betsData);
