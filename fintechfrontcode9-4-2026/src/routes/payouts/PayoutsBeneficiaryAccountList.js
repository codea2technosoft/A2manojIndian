...
// request
import { getUsers, Paymenttranferbankedit, Paymenttranferbankupdate } from 'requests/user';
import {
...
const PayoutsBeneficiaryAccountList = () => {
    ...
    const onOpenUpdateForm = async (record) => {
        try {
            const response = await Paymenttranferbankedit(record.id);
            console.log("API Response:", response);
            
            // FIX: response.data is an object, not an array
            const beneficiaryData = response.data;
            setCurrentRecord(beneficiaryData);
            ...
            Modal.error({
                title: 'Error',
                content: 'Failed to fetch beneficiary data',
            });
        }
    };
    ...
    const onUpdate = async (data) => {
        try {
            ...
            await Paymenttranferbankupdate(updateData);
            Modal.success({
                title: 'Success',
                content: 'Beneficiary updated successfully',
            });
            // refresh list
            navigate({
                pathname: location.pathname,
            ...
            Modal.error({
                title: 'Error',
                content: 'Failed to update beneficiary',
            });
        }
    };
    ...
    const onDelete = async (id) => {
        try {
            ...
            Modal.error({
                title: 'Error',
                content: 'Failed to delete beneficiary',
            });
        }
    };
