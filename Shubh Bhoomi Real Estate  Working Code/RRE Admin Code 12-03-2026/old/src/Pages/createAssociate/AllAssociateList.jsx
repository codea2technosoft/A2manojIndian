...
const dontallowedDomains = [
  "master.bahikhatas.com"
];
...
function AllAssociateList() {
  ...
  const fetchCurrentParentId = async (userId) => {
    try {
      ...
      const response = await fetch(`${API_URL}/get-current-parent-from-history`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const fetchAssociates = async (page = 1, username = "", mobile = "", location = "") => {
    setError(null);
    try {
      const token = getAuthToken();
      if (!token) {
    ...
    showCustomMessageModal(
      ...
      async () => {
        ...
        try {
          ...
          if (!response.ok) {
            const errorData = await response.json();
            showCustomMessageModal("Error", errorData.message || "Failed to update associate status.", "error");
            throw new Error(errorData.message || "Failed to update associate status.");
          }

          const result = await response.json();
          if (result.success = '1') {
            showCustomMessageModal("Success", "Associate status has been updated successfully!", "success");
            fetchAssociates(currentPage);
          } else {
            showCustomMessageModal("Error", result.message || "Something went wrong", "error");
          }
  ...
  const handleSearchClick = () => {
    setCurrentPage(1);
    fetchAssociates(1, searchName.trim(), searchMobile.trim(), searchLocation.trim());
  };
    ...
    showCustomMessageModal(
      ...
      async () => {
        ...
        try {
          ...
          if (!response.ok) {
            const errorData = await response.json();
            showCustomMessageModal("Error", errorData.message || "Failed to delete associate.", "error");
            throw new Error(errorData.message || "Failed to delete associate.");
          }

          const result = await response.json();
          if (result.status == '1') {
            showCustomMessageModal("Success", "Associate has been deleted successfully!", "success");
            fetchAssociates();
          } else {
            showCustomMessageModal("Error", result.message || "Something went wrong", "error");
          }
  ...
  return (
    <>
          ...
          {isFilterActive && (
            <div className="d-flex gap-2 mb-3 mt-2 flex-wrap-mobile expencelistwrap">
              ...
              <div className="d-flex gap-2 justify-content-end">
                ...
                <button className="exportexcel" onClick={exportAllToExcel} disabled={exporting}>
                  {exporting ? (
                    <>
                      <span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
                      Exporting All...
                ...
                {associates.length > 0 ? (
                  associates.map((associate, i) => (
                    <tr key={associate.id}>
                      ...
                      <td>
                        {associate.kyc
                          ? associate.kyc.charAt(0).toUpperCase() + associate.kyc.slice(1).toLowerCase()
                          : ""}
                      </td>
                      <td>
                        {associate.parent_name
                          ? associate.parent_name.charAt(0).toUpperCase() + associate.parent_name.slice(1)
                          : "NA"}
                      </td>
                      <td>{associate.parent_id}</td>
                      <td>{associate.self_sqyd || 0.00}</td>
                      <td>{associate.team_sqyd || 0.00}</td>
                      <td>{associate.self_gift_sqyd || 0.00}</td>
                      <td>{associate.team_gift_sqyd || 0.00}</td>
                      <td>{associate.total_sqyd || 0.00}</td>
                      <td>
                        <span className={`badge ${associate.status === "active" ? "bg-success" : "bg-danger"}`}>
                          {associate.status}
                        </span>
                      </td>
                        ...
                        <div className="dropdown">
                          ...
                          <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <li className="dropdown-item">
                              <button className="btn view_btn btn-sm" onClick={() => handleViewAssociate(associate.id)}>
                                <FaEye /> View
                              </button>
                            </li>
                            <li className="dropdown-item">
                              <button className="btn view_btn btn-sm" onClick={() => handleEditAssociate(associate.id)}>
                                <FaEdit /> Edit
                              </button>
                            </li>
                            <li className="dropdown-item">
                              <button
                                ...
                                onClick={() => handleStatusUpdate(associate.id, associate.status)}
                              >
                                {associate.status === "active" ? <MdAirplanemodeInactive /> : <MdAirplanemodeActive />}
                            ...
                            <li className="dropdown-item">
                              <button
                                ...
                                onClick={() => handleDeleteAssociate(associate.id, associate.username)}
                                style={{ width: '100%', textAlign: 'left', backgroundColor: 'transparent' }}
                              >
                                <RiDeleteBin5Fill className="text-danger" /> <span className="text-danger">Delete</span>
                              </button>
                            </li>
                            ...
                            <li className="dropdown-item">
                              <Link to={`/team-list/${associate.mobile}?name=${encodeURIComponent(associate.username)}`} className="btn view_btn btn-sm">
                                <FaEye /> View Teams
                              </Link>
                            </li>
                          </ul>
                  ...
                  <tr>
                    <td colSpan="15" className="text-center">No associates found.</td>
                  </tr>
                )}
              </tbody>
            </Table>
      ...
      <Modal show={showViewModal} onHide={handleCloseViewModal} centered size="lg">
            ...
            <Row>
              <div className="col-md-12">
                <table className="table">
                  <tbody>
                    <tr><th>Associate Name</th><td>{selectedAssociate.username}</td></tr>
                    <tr><th>Associate Email</th><td>{selectedAssociate.email}</td></tr>
                    <tr><th>Associate Mobile</th><td>{selectedAssociate.mobile}</td></tr>
                    <tr><th>Associate WhatsApp No</th><td>{selectedAssociate.whatsapp_number}</td></tr>
                    <tr><th>Associate Parent ID</th><td>{selectedAssociate.parent_id}</td></tr>
                    <tr>
                      <th>Associate State</th>
                      <td>
                        {selectedAssociate.state
                          ? states.find(s => s.id === Number(selectedAssociate.state))?.name || selectedAssociate.state
                          : 'NA'}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        {(() => {
                          ...
                          return cityObj ? cityObj.name : `City ID: ${selectedAssociate.city}`;
                        })()}
                      </td>
                    </tr>
                    <tr><th>Associate Area</th><td>{selectedAssociate.area}</td></tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td>
                        {selectedAssociate.marriage_anniversary_date
                          ? (() => {
                            const date = new Date(selectedAssociate.marriage_anniversary_date);
                            if (!isNaN(date.getTime())) {
                              const day = String(date.getDate()).padStart(2, '0');
                              const month = String(date.getMonth() + 1).padStart(2, '0');
                              const year = date.getFullYear();
                    ...
                    <tr><th>Associate Rera Number</th><td>{selectedAssociate.rera_number}</td></tr>
                    <tr><th>Associate Address</th><td>{selectedAssociate.address}</td></tr>
                    <tr><th>Associate Pincode</th><td>{selectedAssociate.pincode}</td></tr>
                    <tr><th>Associate Pan Number</th><td>{selectedAssociate.pan_number}</td></tr>
                    <tr><th>Associate AAdhar Number</th><td>{selectedAssociate.adhar_number}</td></tr>
                    <tr>
                      <th>Associate AAdhar Front Image</th>
                      <td>
                        {selectedAssociate.adhar_fornt_image && (
                          <img src={`${profileImage}${selectedAssociate.adhar_fornt_image}`}
                            alt="Adhar Front"
                            style={{ width: '100px', height: 'auto', cursor: 'pointer' }}
                            onClick={() => window.open(`${process.env.profileImage}/${selectedAssociate.adhar_fornt_image}`, '_blank')}
                          />
                        )}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        {selectedAssociate.adhar_back_image && (
                          <img src={`${profileImage}${selectedAssociate.adhar_back_image}`}
                            alt="Adhar Back"
                            style={{ width: '100px', height: 'auto', cursor: 'pointer' }}
                            onClick={() => window.open(`${process.env.profileImage}/${selectedAssociate.adhar_back_image}`, '_blank')}
                          />
                        )}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        {selectedAssociate.pan_card_image && (
                          <img src={`${profileImage}${selectedAssociate.pan_card_image}`}
                            alt="Pan Card"
                            style={{ width: '100px', height: 'auto', cursor: 'pointer' }}
                            onClick={() => window.open(`${process.env.profileImage}/${selectedAssociate.pan_card_image}`, '_blank')}
                          />
                        )}
                      </td>
                    </tr>
        ...
        <Modal.Footer>
          <Button variant="danger" onClick={handleCloseViewModal}>Close</Button>
        </Modal.Footer>
      </Modal>
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="text" name="username" value={editFormData.username} onChange={handleEditFormChange} required />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="text" name="mobile" value={editFormData.mobile} onChange={handleEditFormChange} maxLength="10" required readOnly />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Associate WhatsApp Number</Form.Label>
                  <Form.Control type="tel" name="whatsapp_number" value={editFormData.whatsapp_number} maxLength="10" onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
            </Row>
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="tel" name="parent_id" value={editFormData.parent_id} maxLength="10" onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Associate Old Parent ID (From History)</Form.Label>
                  <Form.Control type="text" value={currentParentId || "Not Updated Till Date"} readOnly className="bg-light" />
                  <Form.Text className="text-muted">
                    <span style={{ color: "red" }}>This is the old parent ID from the previous update history.</span>
                  </Form.Text>
                </Form.Group>
              </Col>
            </Row>
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="text" name="area" value={editFormData.area} onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Associate Pin Code</Form.Label>
                  <Form.Control type="text" name="pincode" value={editFormData.pincode} onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
            </Row>
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="date" name="dob" value={editFormData.dob} onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Associate Marriage Anniversary Date</Form.Label>
                  <Form.Control type="date" name="marriage_anniversary_date" value={editFormData.marriage_anniversary_date} onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
            </Row>
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Select
                    ...
                    value={stateOptions.find(opt => opt.value === editFormData.state)}
                    onChange={(selectedOption) => setEditFormData({ ...editFormData, state: selectedOption?.value, city: "" })}
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Select
                    options={cityOptions}
                    value={cityOptions.find(opt => opt.value === Number(editFormData.city))}
                    onChange={(selectedOption) => setEditFormData({ ...editFormData, city: selectedOption?.value })}
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="text" name="rera_number" value={editFormData.rera_number} onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Associate PAN Number</Form.Label>
                  <Form.Control type="text" name="pan_number" value={editFormData.pan_number} onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
            </Row>
            ...
            <Row>
              <Col md={12}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="text" name="adhar_number" value={editFormData.adhar_number} onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
            </Row>
            ...
            <Row>
              <Col md={12}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control as="textarea" rows={3} name="address" value={editFormData.address} onChange={handleEditFormChange} />
                </Form.Group>
              </Col>
            </Row>
            ...
            <Row>
              <Col md={4}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'adhar_fornt_image')} />
                  {selectedAssociate?.adhar_fornt_image && !(editFormData.adhar_fornt_image instanceof File) && (
                    <div className="mt-2">
                      <img src={`${profileImage}${selectedAssociate.adhar_fornt_image}`} alt="Aadhar Front" style={{ width: '100px', cursor: 'pointer' }} className="img-thumbnail" onClick={() => openImageModal(`${profileImage}${selectedAssociate.adhar_fornt_image}`)} />
                    </div>
                  )}
                </Form.Group>
              </Col>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body text-center">
                <img src={selectedImage} alt="Full View" className="img-fluid" style={{ maxHeight: "80vh" }} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header d-flex justify-content-between align-items-center">
                ...
                <button type="button" className="btn-close" onClick={closeCustomMessageModal}></button>
              </div>
              ...
              <div className="modal-footer justify-content-center">
                {messageModalContent.confirmAction ? (
                  <>
                    <Button variant="primary" onClick={() => { messageModalContent.confirmAction(); closeCustomMessageModal(); }}>Confirm</Button>
                    <Button variant="danger" onClick={closeCustomMessageModal}>Cancel</Button>
                  </>
                ) : (
                  <Button variant={messageModalContent.type === 'success' ? 'success' : messageModalContent.type === 'error' ? 'danger' : 'primary'} onClick={closeCustomMessageModal}>OK</Button>
                )}
              </div>
            </div>
          </div>
