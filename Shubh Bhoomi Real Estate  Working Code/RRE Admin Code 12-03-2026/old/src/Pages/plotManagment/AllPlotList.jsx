...
function AllPlotList() {
  ...
  const fetchPlots = async (page = 1, query = "", projectFilter = "") => {
    ...
    try {
      ...
      if (!showMessageModal) {
        showCustomMessageModal("Error", err.message || "An unexpected error occurred while fetching Units.", "error");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleEditPlot = async (plotId) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Failed to fetch Units for editing.");
      }
      ...
      setError(err.message);
      showCustomMessageModal("Error", err.message || "An unexpected error occurred while fetching Units for editing.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdatePlot = async (e) => {
    ...
    try {
      ...
      if (result.success == '1') {
        showCustomMessageModal("Success", "Units updated successfully!", "success");
      ...
      setError(err.message);
      showCustomMessageModal("Error", err.message || "An unexpected error occurred while updating Units.", "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    showCustomMessageModal(
      ...
      async () => {
        ...
        try {
          ...
          if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || "Failed to update Units status.");
          }

          const result = await response.json();

          if (result.success == "1") {
            showCustomMessageModal("Success", "Units status updated successfully!", "success");
            fetchPlots(currentPage, searchQuery, selectedProject);
          }
    ...
    showCustomMessageModal(
      ...
      async () => {
        ...
        try {
          ...
          showCustomMessageModal("Success", "Units deleted successfully!", "success");
          fetchPlots(currentPage, searchQuery, selectedProject);
        } catch (err) {
          console.error("Delete Units error:", err);
          setError(err.message);
          showCustomMessageModal("Error", err.message || "An unexpected error occurred while deleting Units.", "error");
        } finally {
          setLoading(false);
        }
      }
        ...
        <Modal.Body>
          <Form onSubmit={handleUpdatePlot}>
            <div className="row">
              ...
              <div className="col-md-6">  <Form.Group className="mb-3" controlId="displayBlockName">
              ...
              <div className="col-md-6"> <Form.Group className="mb-3" controlId="editPlotSize">
              ...
              <div className="col-md-12">  <Form.Group className="mb-3" controlId="editPlotAddress">
