...
function AdminWalletLedger() {
    ...
    try {
      ...
      if (result.success === "1" || result.success) {
        ...
        if (result.data?.pagination) {
          ...
          const totalPagesCount = Math.ceil((paginationData.total_records || 0) / pageLimit);
  ...
  const handleDownloadFilteredExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/download-admin-wallet-ledger-excel?${params.toString()}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
                  ...
                  <Table striped bordered hover className="align-middle">
                    ...
                    <tbody>
                      {ledger.map((item, index) => {
                        ...
                        const transactionAmount = crAmount > 0 ? crAmount : drAmount;
                        ...
                        return (
                          <tr key={item.id || index}>
                            ...
                            <td>
                              <div className="table-cell-remark">{item.description || "NA"}</div>
                            </td>

                            <td>
                              <div className="table-cell-remark">{item.narration || "NA"}</div>
                            </td>
                            <td>
                              {(item.cr ?? 0).toFixed(2)}
                            </td>
                            ...
                            <td>
                              {item.balance != null ? Number(item.balance).toFixed(2) : "NA"}
                            </td>
                  ...
                  <>
                    <div className="d-flex justify-content-between align-items-center mt-3">
                      <div className="text-muted">
                        Showing {((currentPage - 1) * pageLimit) + 1} to {Math.min(currentPage * pageLimit, totalRecords)} of {totalRecords} entries
                      </div>
                    ...
                    <div className="text-end mt-2">
                      <small className="text-muted">
                        ...
                        {totalRecords > 0 && ` | Showing ${((currentPage - 1) * pageLimit) + 1} to ${Math.min(currentPage * pageLimit, totalRecords)} of ${totalRecords} records`}
                      </small>
                    </div>
                  </>
                )}
