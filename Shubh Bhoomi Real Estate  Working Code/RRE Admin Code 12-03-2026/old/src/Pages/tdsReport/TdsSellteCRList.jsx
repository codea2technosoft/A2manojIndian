...
function TdsSellteCRList() {
  ...
  const fetchTdsCRList = async (page = 1, name = "", mobile = "") => {
    ...
    try {
      ...
      let url = `${API_URL}/cr-settlement-report?page=${page}&limit=10`;
      ...
      if (data.status === "1") {
        ...
        showCustomMessageModal("Error", data.message || "Failed to fetch TDS CR list.", "error");
        setTransactions([]);
        setTotalPages(1);
        setCurrentPage(1);
      }
  ...
  const handleConfirmSettlement = async () => {
    ...
    try {
      ...
      if (data.status === "1") {
        showCustomMessageModal("Success", "TDS CR Settled Successfully", "success");
            ...
            <div className="col-md-6">
              <div className="card bg-light">
                <div className="card-body">
                  ...
                  <h4 className="card-text text-success">₹ {formatCurrency(summary.total_amount)}</h4>
                </div>
              </div>
            </div>
          </div>
                ...
                {transactions.length > 0 ? (
                  transactions.map((transaction, index) => (
                    <tr key={transaction.id}>
                      ...
                      <td className="fw-bold">{transaction.project_name || "-"}</td>
                      <td>{transaction.plot_shop_villa_no || "-"}</td>
                      <td>
                        <span className="badge bg-success">{transaction.category_name}</span>
                      </td>
                      <td className="text-primary fw-bold">
                        ₹ {formatCurrency(transaction.amount || 0)}
                      </td>
                      <td>{formatDate(transaction.settled_date)}</td>
                      <td>{transaction.payee || "-"}</td>
                       <td>{transaction.settled_by || "-"}</td>
                      <td>{transaction.expense_name || "-"}</td>
                      <td>{transaction.expense_mobile || "-"}</td>
                      <td>
          ...
          {totalPages > 1 && (
            <div className="d-flex justify-content-between align-items-center mt-3">
              <div className="text-muted">
                Showing {(currentPage - 1) * perPage + 1} to {Math.min(currentPage * perPage, summary.total_requests)} of {summary.total_requests} transactions
              </div>
              <div>
                <Pagination>
                  <Pagination.Prev
              ...
              <table className="table">
                <tbody>
                  ...
                  <tr>
                    <th>Category Name</th>
                    <td>
                      <span className="badge bg-success">{selectedTransaction.category_name}</span>
                    </td>
                  </tr>
                  <tr>
                    <th>Category Type</th>
                    <td className="text-uppercase">{selectedTransaction.category_type}</td>
                  </tr>
                  <tr>
                    <th>Amount</th>
                    <td className="text-primary fw-bold">₹ {formatCurrency(selectedTransaction.amount)}</td>
                  </tr>
                  <tr>
                    <th>Date</th>
...
export default TdsSellteCRList;