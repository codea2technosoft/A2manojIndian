...
function BookedPendingBookingList() {
  ...
  const fetchPendingBooking = async (page = 1, query = "") => {
    ...
    try {
      ...
      const url = new URL(`${API_URL}/booked-pending-book-list?${query}`);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to fetch Booked Pending Booking.");
      }
      ...
      console.error("Fetch Booked Pending booking error:", err);
      setError(err.message);
      if (!showMessageModal) {
        showCustomMessageModal("Error", err.message || "An unexpected error occurred while fetching Booked Pending Booking.", "error");
      }
    } finally {
      setLoading(false);
    }
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      ...
      if (response.ok) {
        ...
        showCustomMessageModal("Error", data.message || "Failed to update status", "error");
      }
    } catch (err) {
        ...
        <div className="card-header">
          <div className="d-flex align-items-center justify-content-between gap-3">
            <div className="titlepage">
              <h3>Booked Pending Booking List</h3>
            </div>
                ...
                {pendingBooking?.length > 0 ? (
                    ...
                    <tr key={pendingbooked.id}>
                      ...
                      <td>
                        <div className="dropdown">
                          ...
                          <ul className="dropdown-menu" aria-labelledby={`dropdownMenuButton-${pendingbooked.id}`}>
                            <li className="dropdown-item">
                              <button
                                className="btn btn-sm btn-primary w-100"
                                onClick={() => handleUpdateStatusClick(pendingbooked.id, "ongoing")}
                              >
                                Ongoing
                              </button>
                            </li>
                            <li className="dropdown-item">
                              <button
                                ...
                                onClick={() => handleUpdateStatusClick(pendingbooked.id, "cancelled")}
                              >
                                Cancelled
                              </button>
                            </li>
                  ...
                  <tr>
                    <td colSpan="8" className="text-center text-danger fw-bold">
                      No Booked Pending Booking found.
                    </td>
                  </tr>
                )}
              </tbody>
...
export default BookedPendingBookingList;