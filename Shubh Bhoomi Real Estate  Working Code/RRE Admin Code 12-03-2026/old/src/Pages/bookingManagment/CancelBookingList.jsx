...
function CancelBookingList() {
  ...
  const fetchPendingBooking = async (page = 1, query = "") => {
    ...
    try {
      ...
      const url = new URL(`${API_URL}/cancel-booking-list?${query}`);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to fetch Cancel Booking.");
      }
      ...
      console.error("Fetch Cancel booking error:", err);
      setError(err.message);
      if (!showMessageModal) {
        showCustomMessageModal("Error", err.message || "An unexpected error occurred while fetching Cancel Booking.", "error");
      }
    } finally {
      setLoading(false);
    }
