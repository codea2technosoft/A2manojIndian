...
function OngoingBookingList() {
  ...
  const fetchPendingBooking = async (page = 1, query = "") => {
    ...
    try {
      ...
      const url = new URL(`${API_URL}/ongoing-booking-list?${query}`);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to fetch Ongoing Booking.");
      }
      ...
      console.error("Fetch Ongoing booking error:", err);
      setError(err.message);
      if (!showMessageModal) {
        showCustomMessageModal("Error", err.message || "An unexpected error occurred while fetching Ongoing Booking.", "error");
      }
    } finally {
      setLoading(false);
    }
                          ...
                          <ul className="dropdown-menu" aria-labelledby={`dropdownMenuButton-${pendingbooked.id}`}>
                            <li className="dropdown-item">
                              <button
                                ...
                                onClick={() => handleUpdateStatusClick(pendingbooked.id, "complete")}
                              >
                                Complete
                              </button>
                            </li>
