...
function AllLeadList() {
  ...
  const showCustomMessageModal = (title, text, type, confirmAction = null, showDateInput = false, status = "") => {
    // Reset date when opening modal
    if (showDateInput && (status === "approved" || status === "unitisnotsold")) {
    ...
    const fetchPlotsForEdit = async () => {
      if (editFormData.block_id) {
        ...
        try {
          ...
          if (!response.ok) {
            const errorData = await response.json();
            showCustomMessageModal("Error", errorData.message || "Failed to fetch units for dropdown.", "error");
            setPlotsForEdit([]);
            setLoadingDropdowns(false);
            return;
          }
          ...
          showCustomMessageModal("Error", err.message || "An unexpected error occurred while fetching units for edit.", "error");
          setPlotsForEdit([]);
        } finally {
          setLoadingDropdowns(false);
        }
  ...
  const fetchLeads = async (page = 1, query = "", orderid = "", mobile = "", status = "", plotname = "") => {
    setLoading(true);
    setError(null);
    try {
      const token = getAuthToken();
      ...
      if (!showMessageModal) {
        showCustomMessageModal("Error", err.message || "An unexpected error occurred while fetching leads.", "error");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleViewLead = async (id) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        showCustomMessageModal("Error", errorData.message || "Failed to fetch lead details.", "error");
        setLoading(false);
        return;
      }
      ...
      showCustomMessageModal("Error", "An error occurred while fetching lead details.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitResponse = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        showCustomMessageModal("Success", "Response submitted successfully", "success");
  ...
  const validateForm = () => {
    ...
    if (!editFormData.customer_name || editFormData.customer_name.trim() === '') {
    ...
    if (editFormData.adharfront && typeof editFormData.adharfront !== 'string') {
      if (editFormData.adharfront.size > 2 * 1024 * 1024) {
        errors.adharfront = 'File size must be less than 2MB';
        isValid = false;
      }
      if (!['image/jpeg', 'image/png', 'application/pdf'].includes(editFormData.adharfront.type)) {
    ...
    try {
      ...
      for (const key in editFormData) {
        if (editFormData[key] !== null) {
          ...
          } else if (key !== 'adharfront' && key !== 'adharback' && key !== 'pancard') {
            formData.append(key, editFormData[key]);
          }
        }
      }

      const selectedProject = projectsForEdit.find(p => p.id === parseInt(editFormData.project_id));
      if (selectedProject) formData.append("project_name", selectedProject.name);

      const selectedBlock = blocksForEdit.find(b => b.id === parseInt(editFormData.block_id));
      if (selectedBlock) formData.append("block_name", selectedBlock.name);

      const selectedPlot = plotsForEdit.find(p => p.id === parseInt(editFormData.plot_id));
      if (selectedPlot) formData.append("plot_name", selectedPlot.plot_shop_villa_no);
      ...
      if (result.success == '1') {
        showCustomMessageModal("Success", result.message || "Lead updated successfully.", "success");
  ...
  showCustomMessageModal(
    "Confirm Status Change",
    `Are you sure you want to change the status to ${formatStatus(newStatus)} on ${lead.date} Lead Date?`,
    "confirmation",
    async (selectedDate) => {
      setLoading(true);
      try {
        ...
        if (socket && socket.readyState === WebSocket.OPEN) {
          const notificationPayload = {
            user_id: userId,
            type: "loan_lead",
            message: `Loan lead ${lead.order_id} status changed to ${newStatus}.`,
            statusremark: newStatus,
            remark: null,
    ...
    showCustomMessageModal(
      ...
      async () => {
        ...
        try {
          ...
          const result = await response.json();
          showCustomMessageModal("Success", result.message || "Lead deleted successfully.", "success");
          fetchLeads(currentPage, searchQuery);
        } catch (err) {
          showCustomMessageModal("Error", err.message || "An error occurred while deleting lead.", "error");
        } finally {
          setLoading(false);
          closeCustomMessageModal();
        }
  ...
  return (
    <div className="userlist mt-2">
      <div className="card">
        ...
        <div className="card-body">
          <div className="d-flex gap-2 justify-content-end align-items-center mb-3">
              ...
              <div className="d-flex gap-2 align-items-center">
                ...
                <div className="form-group">
                  ...
                  >
                    ...
                    {["new", "documentPending", "paymentPending", "notInterested", 'loanInprocess', "pattaInprocess", "termSheetPending", "agreementSignaturePending", "pattaApplicationFormPending", "approved", "unitisnotsold", 'rejected'].map((status) => (
            ...
            <div className="createnewadmin">
              <Link to="/create-lead" className="btn btn-success d-flex align-items-center">
                ...
                {leads?.length > 0 ? (
                  leads.map((lead, i) => (
                    <tr key={lead.id}>
                      ...
                      <td>
                        {lead.project_name
                          ? lead.project_name.charAt(0).toUpperCase() + lead.project_name.slice(1).toLowerCase()
                          : ""}
                      </td>

                      <td>
                        {lead.block_name
                          ? lead.block_name.charAt(0).toUpperCase() + lead.block_name.slice(1).toLowerCase()
                          : ""}
                      </td>

                      <td>
                        {lead.plot_name
                          ? lead.plot_name.charAt(0).toUpperCase() + lead.plot_name.slice(1).toLowerCase()
                          : ""}
                      </td>

                      <td>{lead.customer_name.charAt(0).toUpperCase() + lead.customer_name.slice(1).toLowerCase()}</td>
                      ...
                      <td>
                        <div className="w-100">
                          <select
                            ...
                            value={lead.status}
                            // onChange={(e) => handleStatusUpdate(lead.id, e.target.value, lead.order_id)}
                            onChange={(e) =>  handleStatusUpdate(lead, e.target.value)}
                            disabled={lead.status === "approved" || lead.status === "notInterested" || lead.status == "unitisnotsold"}
                          >
                            {["new", "documentPending", "paymentPending", "notInterested", 'loanInprocess', "pattaInprocess", "termSheetPending", "agreementSignaturePending", "pattaApplicationFormPending", "approved", "unitisnotsold", 'rejected'].map((status) => (
                              <option
                                key={status}
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => handleResponseLead(lead.id, lead.order_id)}
                          title="Remark Lead"
          ...
          <div className="d-flex justify-content-end">
            <Pagination>
              ...
              {/* Ellipsis if currentPage > 3 and totalPages > 4 */}
              {currentPage > 4 && totalPages > 4 && <Pagination.Ellipsis disabled />}

              {/* Current page if in middle */}
                ...
                <div className="col-md-12">
                  <div className="card mb-4">
                    ...
                    <div className="card-body p-2">
                      <Form.Group as={Row}>
                        <Form.Label column sm="3" xs='6' className="text-muted">Project Name:</Form.Label>
                        <Col sm="3" xs='6'>
                          <Form.Control plaintext readOnly defaultValue={selectedLead.project_name} className="fw-semibold" />
                        </Col>
                        <Form.Label column sm="3" xs='6' className="text-muted">Block Name:</Form.Label>
                        <Col sm="3" xs='6'>
                          <Form.Control plaintext readOnly defaultValue={selectedLead.block_name} className="fw-semibold" />
                        </Col>
                      </Form.Group>

                      <Form.Group as={Row}>
                        <Form.Label column sm="3" xs='6' className="text-muted">Unit Number:</Form.Label>
                        <Col sm="3" xs='6'>
                          <Form.Control plaintext readOnly defaultValue={selectedLead.plot_name} className="fw-semibold" />
                        </Col>
                        <Form.Label column sm="3" xs='6' className="text-muted">Customer Name:</Form.Label>
                        <Col sm="3" xs='6'>
                          <Form.Control plaintext readOnly defaultValue={selectedLead.customer_name} className="fw-semibold" />
                        </Col>
                      </Form.Group>

                      <Form.Group as={Row}>
                        <Form.Label column sm="3" xs='6' className="text-muted">Mobile Number:</Form.Label>
                        <Col sm="3" xs='6'>
                          <Form.Control plaintext readOnly defaultValue={selectedLead.mobile} className="fw-semibold" />
                        </Col>
                        <Form.Label column sm="3" xs='6' className="text-muted">Income Source:</Form.Label>
                        <Col sm="3" xs='6'>
                          <Form.Control plaintext readOnly defaultValue={selectedLead.income_source} className="fw-semibold" />
                        </Col>
                      </Form.Group>


                      <Form.Group as={Row}>
                        <Form.Label column sm="3" xs='6' className="text-muted">Aadhaar Card:</Form.Label>
                        <Col sm="3" xs='6'>
                          <Form.Control plaintext readOnly defaultValue={selectedLead.adhar_card_number} className="fw-semibold" />
                        </Col>
                        <Form.Label column sm="3" xs='6' className="text-muted">Pan Card:</Form.Label>
                        <Col sm="3" xs='6'>
                          <Form.Control plaintext readOnly defaultValue={selectedLead.pan_card_number} className="fw-semibold" />
                        </Col>
                      </Form.Group>
                    </div>
                  </div>
                          ...
                          <div className="document-card">
                            ...
                            {selectedLead.adhar_front_image ? (
                              <img
                                ...
                                onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/300x200/cccccc/000000?text=Aadhaar+Front"; }}
                              />
                            ) : (
                          ...
                          <div className="document-card">
                            ...
                            {selectedLead.adhar_back_image ? (
                              <img
                                ...
                                alt="Aadhaar Back"
                                className="img-thumbnail document-image"
                                onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/300x200/cccccc/000000?text=Aadhaar+Back"; }}
                              />
                            ) : (
                              <div className="document-placeholder">
                                <i className="bi bi-file-image fs-1 text-muted"></i>
                          ...
                          <div className="document-card">
                            ...
                            {selectedLead.pan_card_image ? (
                              <img
                                ...
                                alt="PAN Card"
                                className="img-thumbnail document-image"
                                onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/300x200/cccccc/000000?text=PAN+Card"; }}
                              />
                            ) : (
                              <div className="document-placeholder">
                                <i className="bi bi-file-image fs-1 text-muted"></i>
      ...
      {/* Edit Lead Modal */}
      <Modal size="lg" show={showEditModal} onHide={() => setShowEditModal(false)} className="formselectnewdesign">
        <Modal.Header closeButton>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group className="mb-3" controlId="editBlockId">
                  ...
                  {editFormData.block_id && loadingDropdowns && plotsForEdit.length === 0 && (
                    <small className="text-muted">Loading Unit...</small>
                  )}
                  {editFormData.block_id && !loadingDropdowns && plotsForEdit.length === 0 && (
                    <small className="text-danger">No Unit available for this Block.</small>
                  )}
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="text" name="customer_name" value={editFormData.customer_name} onChange={handleEditFormChange} required />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Mobile Number</Form.Label>
                  <Form.Control type="tel" name="mobile" value={editFormData.mobile} onChange={handleEditFormChange} maxLength={10} required />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-3">
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="tel" name="adhar_card_number" value={editFormData.adhar_card_number} onChange={handleEditFormChange} maxLength={12} isInvalid={!!validationErrors.adhar_card_number} required />
                  <Form.Control.Feedback type="invalid">
                    {validationErrors.adhar_card_number}
                  </Form.Control.Feedback>
                  <Form.Text muted>
                    Format: 123456789123 (4-4-4)
                  </Form.Text>
                </Form.Group>
              </Col>
              <Col md={6}>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="file" name="adharfront" onChange={handleEditFormChange} />
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="file" name="adharback" onChange={handleEditFormChange} />
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="file" name="pancard" onChange={handleEditFormChange} />
      ...
      {/* Response Lead Modal */}
      <Modal show={showResponceModal} onHide={() => {
        setShowResponceModal(false);
        setDocumentFile([]);
      }}>
        <Modal.Header closeButton>
      ...
      {/* Message Modal */}
      <Modal show={showMessageModal} onHide={() => {
        ...
        <Modal.Body>
          <Form>
            ...
            {messageModalContent.showDateInput &&
              (messageModalContent.status === "approved" || messageModalContent.status === "unitisnotsold" || messageModalContent.status === "rejected") && (
                <Form.Group className="mt-3">
                  <Form.Label>Status Date <span className="text-danger">*</span></Form.Label>
                  <Form.Control
                    type="date"
        ...
        <Modal.Footer>
          {messageModalContent.type === "confirmation" && (
            <Button
              variant="primary"
              onClick={() => {
                if ((messageModalContent.status === "approved" || messageModalContent.status === "unitisnotsold" || messageModalContent.status === "rejected") && !statusUpdateDate) {
