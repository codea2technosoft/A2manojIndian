...
import { Modal, Button, Table, Pagination, Row, Col, Form } from "react-bootstrap";
...
function AdminTDSReport() {
  ...
  const fetchTeamLedger = async (page = 1) => {
    ...
    try {
      ...
      if (result.success === "1") {
        ...
        showCustomMessageModal("Error", result.message || "Failed to fetch team records.");
      }
    } catch (error) {
  ...
  const fetchCustomersLedger = async (page = 1) => {
    ...
    try {
      ...
      if (result.success === "1") {
        ...
        showCustomMessageModal("Error", result.message || "Failed to fetch customers records.");
      }
    } catch (error) {
  ...
  const updateTeamGift = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/gift-team/${teamEditFormData.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const updateCustomerGift = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/gift-customer/${customerEditFormData.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    <div className="padding_15">
      ...
      <div className="userlist mb-5">
        <div className="card shadow-sm">
          ...
          <div className="card-body text-danger fw-bold">
            {loading ? (
              <p>Loading records...</p>
            ) : ledger.length === 0 ? (
              ...
              <>
                ...
                <div className="d-flex justify-content-end mt-3">
                  <Pagination>
                    ...
                    />
                    {getPaginationGroup(teamCurrentPage, teamTotalPages).map((item, index) => (
                      <Pagination.Item
                        key={index}
        ...
        <div className="card shadow-sm">
          ...
          <div className="card-body">
            {isCustomersFilterActive && (
              <Row className="mb-3 gy-2 mobilesed">
                ...
                <Col md={12} className="d-flex justify-content-end gap-2">
                  <Button variant="primary" onClick={() => fetchCustomersLedger(1)}>
                    <RiFilterFill /> Filter
                  </Button>
                  <Button
                    className="buttonpadding d-flex gap-2 border border-white"
          ...
          <div className="card-body text-danger fw-bold">
            {customersLoading ? (
              ...
              <p>No customers records found.</p>
            ) : (
              <>
                <Table striped bordered hover responsive>
                  <thead>
                  ...
                  <tbody>
                    {customersLedger.map((item, index) => (
                      <tr key={item.id}>
                        <td>{(customersCurrentPage - 1) * pageLimit + index + 1}</td>
                        <td>{(item.date_from) || "-"}</td>
                        <td>{(item.date_to) || "-"}</td>
                        <td>{item.closing_days}</td>
                        <td>
                ...
                <div className="d-flex justify-content-end mt-3">
                  <Pagination>
                    <Pagination.Prev
                      onClick={() => handleCustomersPageChange(customersCurrentPage - 1)}
                      disabled={customersCurrentPage === 1}
                    />
                    {getPaginationGroup(customersCurrentPage, customersTotalPages).map((item, index) => (
                      <Pagination.Item
                        key={index}
      ...
      <Modal show={showTeamEditModal} onHide={() => setShowTeamEditModal(false)} size="lg">
        <Modal.Header closeButton>
      ...
      <Modal show={showCustomerEditModal} onHide={() => setShowCustomerEditModal(false)} size="lg">
        ...
        <Modal.Footer>
          <Button variant="danger" onClick={() => setShowCustomerEditModal(false)}>
            Cancel
          </Button>
