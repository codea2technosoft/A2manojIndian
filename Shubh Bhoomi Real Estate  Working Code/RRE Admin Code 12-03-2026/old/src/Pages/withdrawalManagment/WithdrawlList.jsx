...
import { Modal, Button, Form, Table, Pagination, Row, Col, Spinner } from "react-bootstrap";
...
function WithdrawlList() {
  ...
  const handleStatusUpdateClick = (id, newStatus) => {
    if (newStatus === "success") {
      ...
      setPaymentUrl(''); // Reset payment URL
      setPaymentMethods(''); // Reset payment methods
      setSlipImage(null); // Reset slip image
  ...
  const handleDownloadFilteredExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/download-withdrawal-pending-excel?${params}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
          ...
          {isFilterActive && (
            <Row className="mb-3 gy-2 mobilesed">
              <Col md={3} style={{ position: "relative" }}>
                <div className="position-absolute" style={{ top: '16px', right: '20px' }}>
                  {!filterFromDate ? (
                    <FaCalendar
                ...
                <DatePicker
                  ...
                  isClearable={true} // enables keyboard or UI clear
                  showPopperArrow={false} // cleaner popup
                  showMonthDropdown
                  showYearDropdown
                  dropdownMode="select"
                />
              ...
              <Table striped bordered hover responsive className="shadow-sm">
                ...
                <tbody>
                  {withdrawals.length > 0 ? (
                          ...
                          {item.status === 'pending' && (
                            <div className="d-flex gap-2">
                              <Button
                                variant="success"
                                size="sm"
                                onClick={() => handleStatusUpdateClick(item.id, 'success')}
                              >
                                Approve
                              </Button>
                              <Button
                                variant="danger"
                                size="sm"
                                onClick={() => handleStatusUpdateClick(item.id, 'reject')}
                              >
                                Reject
                              </Button>
                            </div>
                    ...
                    <tr>
                      <td colSpan="19" className="text-center text-danger fw-bold">No withdrawal requests found.</td>
                    </tr>
                  )}
                </tbody>
              </Table>
          ...
          <Modal show={approveModalShow} onHide={() => {
            ...
            <Modal.Body>
              <Form>
                  ...
                  <Form.Select className="text-dark"
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.value === 'cash') {
                        setPaymentUrl(''); // Clear UTR when cash is selected
                        setSlipImage(null); // Clear slip image when cash is selected
                      }
                    }}
                    required
                  >
                ...
                {selectedPaymentMode === "online" && (
                  <>
                    <Form.Group className="mb-3">
                      <Form.Label>Payment UTR <span className="text-danger">*</span></Form.Label>
                      <Form.Control
                        type="text"
            ...
            <Modal.Footer>
              ...
              <Button
                ...
                disabled={loading || (selectedPaymentMode === "online" && !paymentUrl.trim()) || !selectedPaymentMode}
              >
