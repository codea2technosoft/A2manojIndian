...
function WithdrawalStatusRejectedLists() {
  ...
  const fetchVisits = async (page = 1, name = "", account = "", status = "") => {
    ...
    console.log("API URL:", `${API_URL}/withdrawal-requests-rejected-list?${query}`);

    try {
      const response = await axios.get(`${API_URL}/withdrawal-requests-rejected-list?${query}`, {
  ...
  const handleDownloadFilteredExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/download-withdrawal-reject-excel?${params}`, {
        method: "GET",
        headers: {
