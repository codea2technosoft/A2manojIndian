...
import { Modal, Button, Table, Pagination, Row, Col, Spinner } from "react-bootstrap";
...
function ALLPropertyUnitisnotsoldeList() {
  ...
  const INCOME_API_ENDPOINT = `${API_URL}/property-unitis-notsold-list`;
  ...
  const fetchIncomeRecords = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`${INCOME_API_ENDPOINT}?${params.toString()}`, {
        method: "GET",
        headers: {
  ...
  const handleDownloadExcel = async (income_id) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/export-property-unit-is-not-sold?id=${income_id}`,
        {
          method: "GET",
          headers: {
  ...
  const handleDownloadFilteredExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/export-property-commission-by-date?${params.toString()}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    <div className="card mt-2">
      <div className="card-header">
        <div className="d-flex justify-content-between align-items-center">
          ...
          <div className="titlepage">
            <h3 className="mb-0">Unit Is  Not Sold Commission List</h3>
          </div>
      ...
      <div className="card-body">
        ...
        <div className="d-none d-md-block card mb-3">
          ...
          <Row className="justify-content-between card-body gy-2">
                  ...
                  <tr key={record.income_id}>
                    ...
                    <td>
                      <Button
                        size="sm"
                        variant="info"
                        onClick={() => navigate(`/parent-unitis-notsold/${record.income_id}`)}
                      >
                        <FaEye />
                      </Button>
                    </td>
                    ...
                    <td>
                      {record.colony_name
                        ? record.colony_name.charAt(0).toUpperCase() + record.colony_name.slice(1).toLowerCase()
                        : ""}
                    </td>
                    <td>
                      {record.plot_no
                        ? record.plot_no.charAt(0).toUpperCase() + record.plot_no.slice(1).toLowerCase()
                        : ""}
                    </td>
...
export default ALLPropertyUnitisnotsoldeList;
