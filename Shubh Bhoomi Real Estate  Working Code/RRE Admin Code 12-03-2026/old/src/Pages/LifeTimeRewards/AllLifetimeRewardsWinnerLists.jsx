...
function AllLifetimeRewardsWinnerLists() {
  ...
  const fetchLedger = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/lifetime-rewards-winner-list?${params}`, {
        method: "GET",
        headers: {
              ...
              <Row className="mb-3 gy-2">
                ...
                <Col md={3}>
                  <input
                    ...
                    onChange={(e) => setSearchTerm({ ...searchTerm, mobile: e.target.value })}
                  />
                </Col>
                ...
                <Table striped bordered hover responsive className="align-middle">
                  ...
                  <tbody>
                    {ledger.length > 0 ? (
                          ...
                          return item.rewards.map((rewardItem, idx) => (
                            <tr key={`${item.user_id}-${idx}`}>
                              <td className="fw-bold">
                                {idx === 0 ? (currentPage - 1) * pageLimit + index + 1 : ""}
                              </td>
                               <td>{formatDate(rewardItem.achievementDate)}</td>

                              <td className="fw-semibold">{idx === 0 ? item.username || "N/A" : ""}</td>
                              ...
                              <td>{formatCurrency(rewardItem.reward.item_amount)}</td>
                      ...
                      <tr>
                        <td colSpan="9" className="text-center py-5 text-danger fw-bold">
