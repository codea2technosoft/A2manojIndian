...
function AddExpenses() {
    ...
    // ✅ NEW: Check if selected category requires mobile number field
    const shouldShowMobileNumberField = () => {
        const selectedCategory = categories.find(cat => cat.id.toString() === formData.category_id);
        
        // Check for these categories:
        // 1. Advance Payment To Associate (category_id = "1")
        // 2. Commission Paid with DR type
        // 3. Rewards with DR type
        return formData.category_id === "1" || 
               (selectedCategory?.name === "commission paid" && formData.category_type === "dr") ||
               (selectedCategory?.name === "rewards" && formData.category_type === "dr");
    };
    ...
    // ✅ NEW: Check if selected category requires project and plot selection
    ...
    // Fetch projects and plots when applicable categories are selected
    useEffect(() => {
        const fetchProjectsAndPlots = async () => {
            if (shouldShowProjectPlotSelection()) {
                ...
                // Clear projects and plots when not in relevant categories
                setProjects([]);
                setPlots([]);
                setFormData(prev => ({
                    ...prev,
    ...
    const handleChange = async (e) => {
        ...
        if ((name === "parent_id" && value.length === 10) && shouldShowMobileNumberField()) {
        ...
        } else if (name === "parent_id" && value.length !== 10 && shouldShowMobileNumberField()) {
            setParentName("");
            setErrors((prevErrors) => ({
                ...prevErrors,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // ✅ UPDATED: Prepare payload with parent_name for all cases
            let parent_name_value = "";
            
            if (shouldShowMobileNumberField()) {
                // For mobile number categories
            ...
            const payload = {
                ...
                ...(shouldShowMobileNumberField()
                    ...
                    : { parent_id: "" }),
                // Add project and plot data for relevant categories
            ...
            console.log("Should show mobile field?", shouldShowMobileNumberField());
            console.log("Should show project/plot?", shouldShowProjectPlotSelection());
                            ...
                            {dataLoading ? (
                                ...
                                <Form onSubmit={handleSubmit}>
                                    <Row>
                                        ...
                                        {/* Project and Plot selection for relevant categories */}
                                        {shouldShowProjectPlotSelection() && (
                                            <>
                                                <Col md={6}>
                                                    <Form.Group className="mb-3">
                                        ...
                                        {/* ✅ UPDATED: Show mobile number field for all mobile-required categories */}
                                        {shouldShowMobileNumberField() ? (
                                            <Col md={6}>
