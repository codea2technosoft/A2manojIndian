  ...
  const fetchVisits = async (
    ...
    try {
      const response = await axios.get(
        `${API_URL}/withdrawal-requests-completed-list?${query}`,
        {
      ...
      if (result.success === "1") {
        ...
        setError(result.message || "Failed to fetch Success Data.");
        setVisits([]);
        setTotalPages(1);
      }
    } catch (err) {
      setError("Failed to fetch Success Data.");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleDownloadFilteredExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/download-withdrawal-success-excel?${params}`,
        {
          method: "GET",
          headers: {
      ...
      link.href = url;
      link.setAttribute("download", `Filtered Withdrawal Success Report.xlsx`);
  ...
  return (
    <>
      <div className="card mt-2">
        <div className="card-header flex-wrap-mobile gap-2 d-flex justify-content-between align-items-center">
          <div className="titlepage">
            <h3 className="mb-0">Commission Success List Details</h3>
          </div>
        ...
        {isFilterActive && (
          <div className="card-body pb-0">
            <Row className="gy-2">
              ...
              <Col md={2} className="d-flex justify-content-start gap-2">
            ...
            <tbody>
              {visits.length > 0 ? (
                ...
                <tr>
                  <td
                    colSpan="20"
                    className="text-center text-gray-500 py-4 italic bg-gray-50 text-danger fw-bold"
                  >
                    No accounts found with status{" "}
