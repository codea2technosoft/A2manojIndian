...
function AssociatesBimaAchieverFormEntriesLists() {
  ...
  const downloadBima = async (id) => {
    ...
    try {
      ...
      const url = `${API_URL}/bima-download/${id}`;
      ...
      link.setAttribute('download', `bima_document_${id}.pdf`);
  ...
  const uploadPDF = async (id, file) => {
    ...
    try {
      const token = getAuthToken();
      const url = `${API_URL}/bima-upload-pdf/${id}`;
  ...
  const fetchUsers = async (page = 1) => {
    ...
    try {
      const token = getAuthToken();
      const url = `${API_URL}/admin-bima-reg-form-list?status=${searchTerm.status}&name=${searchTerm.customer_name}&mobile=${searchTerm.mobile}&orderid=${searchTerm.orderid}&page=${page}&limit=${LIMIT}`;
      const response = await fetch(url, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const toSentenceCase = (text) => {
    ...
    if (strText.length === 0 || strText === "null" || strText === "undefined") return "-";
    return strText.charAt(0).toUpperCase() + strText.slice(1).toLowerCase();
  };
...
export default AssociatesBimaAchieverFormEntriesLists;