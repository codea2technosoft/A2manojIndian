...
function CreatePropertyLead() {
  ...
  // ✅ ADD THIS STATE
  const [isBlockIdNull, setIsBlockIdNull] = useState(false);
  ...
  // ✅ CHECK IF BLOCK_ID IS NULL
  useEffect(() => {
    const urlBlockId = params.get("block_id");
    if (urlBlockId === "null") {
      setIsBlockIdNull(true);
      setFormData((prev) => ({
        ...prev,
        block_id: "",
        block_name: "",
  ...
  // ✅ MODIFIED fetchPlotsList - Project ID ke basis pe plots fetch karo jab block_id null ho
  useEffect(() => {
    const fetchPlotsList = async () => {
      // 🔥 CASE 1: Jab block_id null hai - Project ID ke basis pe plots fetch karo
      if (isBlockIdNull && formData.project_id) {
        setLoading(true);
        try {
          const token = getAuthToken();
          if (!token) {
          ...
          console.log("📊 Fetching plots by PROJECT ID:", formData.project_id);
          
          // ✅ Project ID ke basis pe plots fetch karo (backend ne null handle kar liya)
          ...
          // Auto-select plot from URL
          const matchedPlot = data.data.find((plot) => plot.id == urlPlotId);
          if (matchedPlot) {
            setFormData((prev) => ({
              ...prev,
              plot_name: matchedPlot.plot_shop_villa_no,
              plot_id: matchedPlot.id,
            }));
          }
      ...
      // 🔥 CASE 2: Jab block_id valid hai - Block ID ke basis pe plots fetch karo
      else if (formData.block_id && !isBlockIdNull) {
        setLoading(true);
        try {
          const token = getAuthToken();
          if (!token) {
          ...
          console.log("📊 Fetching plots by BLOCK ID:", formData.block_id);
    ...
    fetchPlotsList();
  }, [formData.block_id, formData.project_id, isBlockIdNull, urlPlotId]);
  ...
  const validateForm = () => {
    ...
    // ✅ Block sirf tab required hai jab null nahi hai
    if (!isBlockIdNull && !formData.block_id) {
    ...
    try {
      ...
      if (socket && socket.readyState === WebSocket.OPEN) {
        const notificationPayload = {
          ...
          remark: `${formData.project_name}, ${formData.block_name || "No Block"}, ${formData.plot_name}`,
          statusremark: "",
          order_id: "",
            ...
            <div className="card-body">
              <Form onSubmit={handleSubmit}>
                <Row>
                  ...
                  {/* ✅ BLOCK FIELD - Optional when block_id is null */}
                  <Col md={6}>
                    <Form.Group className="mb-3" controlId="formBlockId">
                      <Form.Label>
                        Select Block {!isBlockIdNull && <span className="text-danger">*</span>}
                        {isBlockIdNull && <span className="text-muted ms-2">(Optional - All Blocks)</span>}
                      </Form.Label>
                      {isBlockIdNull ? (
                        <Form.Control
                          type="text"
                          value="All Blocks (Viewing all units)"
                          disabled
                          className="bg-light"
                        />
                      ) : (
                  ...
                  {/* ✅ PLOTS DROPDOWN - Show all plots from state */}
                  <Col md={6}>
                    <Form.Group className="mb-3" controlId="formPlotId">
                      ...
                      >
                        <option value="">
                          {loading ? "Loading plots..." : "Select Unit"}
                        </option>
                        {plots.map((plot) => (
                          <option key={plot.id} value={plot.id}>
                            {plot.plot_shop_villa_no}
                            {isBlockIdNull && ` (${plot.block_name || "No Block"})`}
                          </option>
                        ))}
                      </Form.Control>
                      <Form.Control.Feedback type="invalid">
                      ...
                      {plots.length === 0 && !loading && (
                        <small className="text-danger">No plots found</small>
                      )}
                    </Form.Group>
                  </Col>
