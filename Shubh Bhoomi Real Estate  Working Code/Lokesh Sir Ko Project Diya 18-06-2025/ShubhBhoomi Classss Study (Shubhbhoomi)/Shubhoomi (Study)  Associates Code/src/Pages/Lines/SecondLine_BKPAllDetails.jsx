...
function SecondLine() {
  ...
  const [exportingAll, setExportingAll] = useState(false); // <-- Export loading state
  ...
  const fetchLineData = async (page = 1) => {
    ...
    try {
      ...
      if (result.status === "1") {
        setLineData(result);
        const allUsersData = result.line_info?.users || [];
  ...
  const handleExportAllToExcel = async () => {
    const allUsersList = lineData.line_info?.users || [];
    ...
    try {
      ...
      Swal.fire({
        ...
        html: `Processing 0/${allUsersList.length} users...<br>Please wait...`,
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        }
      });
      // Fetch data for each user - एक के बाद एक fetch करें
      for (let i = 0; i < allUsersList.length; i++) {
        ...
        Swal.update({
          html: `Processing ${i + 1}/${allUsersList.length} users...<br>
      ...
      // Create Excel/CSV with all data
      const headers = [
        'S.No',
        'User Name',
        'User Mobile',
      ...
      const summaryData = [
        ['Total Users in Line', allUsersList.length],
        ['Users with Records', successCount],
        ['Users without Records', allUsersList.length - successCount],
        ...
        [],
        ['NOTE', 'This file contains all property records for all users in Second Line']
      ];
      let csvContent = '';
      ...
      const date = new Date();
      const fileName = `Second_Line_All_Users_Records_${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getDate().toString().padStart(2, '0')}.csv`;
  ...
  // Get paginated users - अब lineData.line_info.users से लो
  const getPaginatedUsers = () => {
    const startIndex = (currentPage - 1) * LIMIT;
    const endIndex = startIndex + LIMIT;
    return (lineData.line_info?.users || []).slice(startIndex, endIndex);
  };

  const paginatedUsers = getPaginatedUsers();
  const totalUsers = lineData.line_info?.users?.length || 0;
  return (
    <div className="padding_15">
      <div className="card">
        <div className="card-header">
          <div className="d-flex align-items-center justify-content-between gap-2">
            <div className="titlepage">
              <h3>{lineData.line_info?.line_name || "Second Line"} Details</h3>
            </div>
        ...
        <div className="card-body">
              ...
              <div className="stat-card bg-primary text-white p-4 rounded shadow-sm">
                <div className="d-flex align-items-center justify-content-between">
                  <div>
                    ...
                    <h2 className="mb-0">{lineData.line_info?.total_members || 0}</h2>
                  </div>
                  <div className="icon-circle bg-white bg-opacity-25 p-3 rounded-circle">
                    <FaUsers size={30} />
                  </div>
                </div>
                <p className="mb-0 mt-2 small">All members in this line</p>
              </div>
            </div>
            <div className="col-xl-6 col-md-6 col-sm-12 mb-3">
              <div className="stat-card bg-warning text-white p-4 rounded shadow-sm">
                <div className="d-flex align-items-center justify-content-between">
                  <div>
                    ...
                    <h2 className="mb-0">{lineData.line_info?.total_buysqft || 0}</h2>
                  </div>
                  <div className="icon-circle bg-white bg-opacity-25 p-3 rounded-circle">
                    <FaChartArea size={30} />
                  </div>
                      ...
                      <div className="col-md-3 col-sm-6 mb-3">
                        <div className="d-flex align-items-center">
                          ...
                          <div>
                            ...
                            <h6 className="mb-0">{lineData.line_info.root_user.username}</h6>
                          </div>
                        </div>
                      </div>
                      <div className="col-md-3 col-sm-6 mb-3">
                        <div className="d-flex align-items-center">
                          ...
                          <div>
                            ...
                            <h6 className="mb-0">{lineData.line_info.root_user.mobile}</h6>
                          </div>
                        </div>
                      </div>
                      <div className="col-md-3 col-sm-6 mb-3">
                      ...
                      <div className="col-md-3 col-sm-6 mb-3">
                        <div className="d-flex align-items-center">
                          ...
                          <div>
                            ...
                            <h6 className="mb-0">₹{lineData.line_info.root_user.credit || 0}</h6>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          ) : !lineData.line_info?.users || lineData.line_info.users.length === 0 ? (
            <div className="text-center py-5">
            ...
            <>
              <div className="card">
                <div className="card-header d-flex justify-content-between align-items-center">
                  <h5 className="mb-0">Users List ({totalUsers} members)</h5>
                          ...
                          <tr key={user.id}>
                            ...
                            <td>
                              <Badge bg={
                                ...
                                {toSentenceCase(user.user_type || "user")}
                              </Badge>
                            </td>
                            ...
                            <td>
                              <div className="d-flex gap-2">
                                {/* Only View Details Button - Excel button नहीं */}
                                <button
                                  className="btn btn-sm btn-info"
                                  disabled={!user?.id}
                                  onClick={() => navigate(`/second-lines-details/${user?.id}`)}
                                  title="View Details"
                                >
                                  <FaEye />
                                </button>
