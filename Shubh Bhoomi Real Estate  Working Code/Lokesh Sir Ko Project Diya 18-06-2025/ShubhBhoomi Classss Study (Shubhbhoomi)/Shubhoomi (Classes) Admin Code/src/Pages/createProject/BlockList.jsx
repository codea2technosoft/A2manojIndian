...
function BlockList() {
  ...
  const fetchBlocks = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/block-list?page=${page}&limit=10`,
        {
          method: "GET",
          headers: {
  ...
  const handleViewBlock = async (blockId) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showCustomMessageModal(
          "Error",
          errorData.message || "Failed to fetch block details.",
          "error",
        );
  ...
  const handleEditBlock = async (blockId) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showCustomMessageModal(
          "Error",
          errorData.message || "Failed to fetch block for editing.",
          "error",
        );
        throw new Error(
          errorData.message || "Failed to fetch block for editing.",
  ...
  const handleUpdateBlock = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showCustomMessageModal(
          "Error",
          errorData.message || "Failed to update block.",
          "error",
        );
      ...
      async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            showCustomMessageModal(
              "Error",
              errorData.message || "Failed to update block status.",
              "error",
            );
            throw new Error(
              errorData.message || "Failed to update block status.",
      ...
      async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            showCustomMessageModal(
              "Error",
              errorData.message || "Failed to delete block.",
              "error",
            );
  ...
  //     let url = `${API_URL}/block-excel-download`;
  ...
  //     const data = await response.json();
  //     const allBloks = data.data || [];
  //     const worksheet = XLSX.utils.json_to_sheet(allBloks.map(bloks => ({
  //       "SL No.": allBloks.indexOf(bloks) + 1,
  //        "ID": bloks.id,
  //       "name": bloks.name,
  //       "status": bloks.status,
  //       "description": bloks.description,
  //     })));

  //     const workbook = XLSX.utils.book_new();
  //     XLSX.utils.book_append_sheet(workbook, worksheet, "Projects");
  //     XLSX.writeFile(workbook, `All_Bloks_${new Date().toISOString().split('T')[0]}.xlsx`);

  //   } catch (err) {
  //     console.error("Export error:", err);
  //     showCustomMessageModal("Error", "Failed to export Bloks.", "error");
  //   } finally {
  //     setExporting(false);
  //   }
  // };
  ...
  const exportAllToExcel = async () => {
    try {
      ...
      link.setAttribute("download", `block_${Date.now()}.csv`);
      link.click();
    } catch (error) {
            ...
            <div className="d-flex gap-2">
              ...
              <>
                {!allowedDomains.includes(currentDomain) && (
                  <div className="createnewadmin">
                    <Link
                      to="/create-bloks-by-uploading-csv"
                      className="btn btn-success d-inline-flex align-items-center"
                    >
                          ...
                          >
                            ...
                            <li className="dropdown-item">
                              <button
                                ...
                                onClick={() =>
                                  handleStatusUpdate(block.id, block.status)
                                }
                              >
                                {block.status === "active" ? (
                                  <MdAirplanemodeInactive />
      ...
      <Modal show={showViewModal} onHide={handleCloseViewModal} centered>
        ...
        <Modal.Body>
          {selectedBlock && (
            <div>
              <p>
                <strong>Project Name:</strong> {selectedBlock.project_name}
              </p>
              <p>
                <strong>Block Name:</strong> {selectedBlock.name}
              </p>
              <p>
