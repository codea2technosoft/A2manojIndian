  ...
  const fetchTdsDRList = async (page = 1, withdrawId = "", userId = "") => {
    ...
    try {
      ...
      if (data.status === "1") {
        ...
        showCustomMessageModal(
          "Error",
          data.message || "Failed to fetch TDS DR list.",
          "error",
        );
        setWithdrawals([]);
        setTotalPages(1);
                ...
                <div className="card-body">
                  ...
                  <h4 className="card-text">
                    ...
                    {formatCurrency(
                      withdrawals.reduce(
                        (sum, item) =>
                          sum + parseFloat(item.withdrawal_amount || 0),
                        0,
                      ),
                    )}
                  </h4>
                ...
                {withdrawals.length > 0 ? (
                  withdrawals.map((withdrawal, index) => (
                    <tr key={withdrawal.withdraw_id}>
                      ...
                      <td>
                        ₹ {formatCurrency(withdrawal.amount_after_tds || 0)}
                      </td>
                      ...
                      <td>
                        ₹ {formatCurrency(withdrawal.withdrawal_amount || 0)}
                      </td>
                    </tr>
                  ))
                ) : (
          ...
          {totalPages > 1 && (
            <div className="d-flex justify-content-between align-items-center mt-3">
              <div className="text-muted">
                ...
                {summary.total_requests} withdrawals
              </div>
              <div>
                <Pagination>
                  <Pagination.Prev
              ...
              <table className="table">
                <tbody>
                  <tr>
                    <th>Withdrawal ID</th>
                    <td className="fw-bold">
                      {selectedWithdrawal.withdraw_id}
                    </td>
                  </tr>
                  <tr>
                    <th>User ID</th>
                  ...
                  <tr>
                    ...
                    <td className="text-danger">
                      ₹ {formatCurrency(selectedWithdrawal.tds)}
                    </td>
                  </tr>
                  <tr>
                    <th>Amount After TDS</th>
                    <td>
                      ₹ {formatCurrency(selectedWithdrawal.amount_after_tds)}
                    </td>
                  </tr>
                  <tr>
                    <th>Net Payment</th>
                    <td className="text-success fw-bold">
                      ₹ {formatCurrency(selectedWithdrawal.net_payment)}
                    </td>
                  </tr>
                  <tr>
                    <th>Status</th>
                    <td>
                      <span
                        className={`badge ${getStatusBadgeClass(selectedWithdrawal.status)}`}
                      >
        ...
        <Modal.Body>
          {settleTransaction && (
            <>
              ...
              <p>
                <b>User:</b> {settleTransaction.username} (
                {settleTransaction.user_id})
              </p>
              <p>
              ...
              <p>
                <b>Net Payable:</b> ₹{" "}
                {formatCurrency(settleTransaction.net_payment)}
              </p>
