...
let globalStatesCache = null;
let isFetchingCitiesGlobal = false;
let isFetchingStatesGlobal = false;
...
// REPLACE the entire fetchAllCities function with this:

// Fetch ALL Cities directly (without looping through states)
const fetchAllCities = async () => {
  ...
  try {
    // ✅ Try to fetch all cities at once (if API supports without state_id)
    // Option 1: If your API has an endpoint to fetch all cities
    const res = await fetch(`${API_URL}/city-list-all`, {  // Change this endpoint name if needed
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
    ...
    if (res.ok && data.data) {
      ...
      console.log("City Map loaded directly:", cityMapData);
      setCityMap(cityMapData);
    } else {
      // Option 2: If API requires state_id, then use Promise.all with rate limiting
      console.warn("Direct city list not available, fetching with rate limiting...");
      await fetchCitiesWithRateLimiting();
    }
    
  } catch (error) {
    console.error("Error fetching all cities directly:", error);
    // Fallback to rate-limited approach
    await fetchCitiesWithRateLimiting();
  } finally {
    setIsLoadingCities(false);
  }
...
// Fallback function with rate limiting and Promise.all
const fetchCitiesWithRateLimiting = async () => {
  const token = getAuthToken();
  if (!token) return;
  ...
  try {
    ...
    console.log(`Fetching cities for ${allStates.length} states with rate limiting...`);
    
    // ✅ Use Promise.all instead of loop (parallel but controlled)
    const promises = allStates.map(state => 
      fetch(`${API_URL}/city-list`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
       ...
       .catch(err => ({ error: true, stateId: state.id }))
    );
    ...
    results.forEach((data, index) => {
      if (!data.error && data.data) {
        ...
        console.error(`Failed to fetch cities for state ${allStates[index]?.id}`);
      }
    });
    
    console.log(`Loaded ${Object.keys(cityMapData).length} cities`);
    setCityMap(cityMapData);
    
  } catch (error) {
    console.error("Error in fetchCitiesWithRateLimiting:", error);
  }
};
  ...
  const fetchUsers = async (page = 1) => {
    ...
    try {
      const token = getAuthToken();
      const url = `${API_URL}/associate-welcome-bonus-list?status=${searchTerm.status}&name=${searchTerm.customer_name}&mobile=${searchTerm.mobile}&orderid=${searchTerm.orderid}&page=${page}&limit=${LIMIT}`;
  ...
  const handleSubmitUpdate = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/associate-welcome-bonus-update`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
 ...
 const toSentenceCase = (text) => {
  ...
  // Create empty array for table structure when no data
          ...
          {selectedUser && (
            <div className="row">
              ...
              <div className="col-md-6 mb-3">
                ...
                <span className="badge" style={getStatusStyle(selectedUser.status) || "NA"}>
                  {toSentenceCase(selectedUser.status)|| "NA"}
                </span>
              </div>
              <div className="col-md-6 mb-3">
