  ...
  const fetchChannels = async (page = 1, name = "", location = "") => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/today-channel-list?page=${page}&limit=10&mobile=${name}&parent_id=${location}`,
        {
          method: "GET",
          headers: {
  ...
  const handleEditChannels = async (channelId) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showCustomMessageModal(
          "Error",
          errorData.message || "Failed to fetch Channel for editing.",
          "error",
        );
        throw new Error(
          errorData.message || "Failed to fetch Channel for editing.",
  ...
  const handleUpdateChannel = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showCustomMessageModal(
          "Error",
          errorData.message || "Failed to update Channel.",
          "error",
        );
      ...
      async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            showCustomMessageModal(
              "Error",
              errorData.message || "Failed to update Channel status.",
              "error",
            );
            throw new Error(
              errorData.message || "Failed to update Channel status.",
          ...
          if ((result.success = "1")) {
            showCustomMessageModal(
              "Success",
              "Channel status has been updated successfully!",
              "success",
            );
            fetchChannels(currentPage);
          } else {
                ...
                {channels.length > 0 ? (
                  channels.map((channel, i) => (
                    <tr key={channel.id}>
                      ...
                      <td>
                        {channel.username
                          ? channel.username.charAt(0).toUpperCase() +
                            channel.username.slice(1).toLowerCase()
                          : ""}
                      </td>
                      ...
                      >
                        {channel.kyc
                          ? channel.kyc.charAt(0).toUpperCase() +
                            channel.kyc.slice(1).toLowerCase()
                          : "-"}
                      </td>

                      <td>
                        {channel.parent_name
                          ? channel.parent_name.charAt(0).toUpperCase() +
                            channel.parent_name.slice(1).toLowerCase()
                          : "-"}
                      </td>
                          ...
                          >
                            ...
                            <li className="dropdown-item">
                              <button
                                ...
                                onClick={() =>
                                  handleStatusUpdate(channel.id, channel.status)
                                }
                              >
                                {channel.status === "active" ? (
                                  <MdAirplanemodeInactive />
                            ...
                            {channel.status !== "inactive" && (
                              <li className="dropdown-item">
                                <a
                                  href={`https://dashboard.rajasthanirealestates.in/login?mobile=${channel.mobile}&pssword=${channel.password}`}
                                  target="_blank"
