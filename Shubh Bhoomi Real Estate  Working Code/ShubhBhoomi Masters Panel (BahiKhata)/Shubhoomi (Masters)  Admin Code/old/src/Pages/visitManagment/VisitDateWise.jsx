...
function VisitDateWise() {
  ...
  const fetchExpenses = async (page = 1) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/visite-dateWaise?page=${page}&limit=10`,
        {
          method: "GET",
          headers: {
  ...
  const handleView = (date) => {
    // If date is already in DD-MM-YYYY format, convert it to YYYY-MM-DD
    const parts = date.split("-");
    if (parts.length === 3) {
      const [day, month, year] = parts;
      const isoDate = `${year}-${month}-${day}`;
      navigate(`/visit-by-date/${isoDate}`);
    } else {
      // Fallback for other formats
      const onlyDate = date.split("T")[0];
      navigate(`/visit-by-date/${onlyDate}`);
    }
  };
      ...
      <div className="card-header">
        <div className="d-flex justify-content-between align-items-center">
          <div className="titlepage">
            <h3 className="mb-0">Date-wise Visit Report</h3>
          </div>
        </div>
      </div>
      <div class="card-body">
              ...
              <tbody>
                {expenses.length > 0 ? (
                  expenses.map((expense, index) => {
                    const formattedDate = (() => {
                      const utcDate = new Date(expense.visit_date);
                      const [year, month, day] = utcDate
                        .toISOString()
                        .split("T")[0]
                        .split("-");
