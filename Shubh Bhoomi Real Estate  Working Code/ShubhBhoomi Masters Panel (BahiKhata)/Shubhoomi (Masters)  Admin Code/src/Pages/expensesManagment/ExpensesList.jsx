...
function ExpensesList() {
  ...
  const fetchPlotsByProject = async (projectId) => {
    ...
    try {
      const plotsResponse = await fetch(
        `${API_URL}/all-plot-list-by-id?project_id=${projectId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const exportAllToExcel = async () => {
    ...
    try {
      ...
      XLSX.writeFile(
        workbook,
        `All_Expenses_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
    } catch (err) {
  ...
  const handleEditFormChange = (e) => {
    ...
    setEditFormData((prevData) => {
      ...
      if (name === "plot_id") {
        ...
        newData.plot_name = selectedPlot
          ? selectedPlot.name || selectedPlot.plot_number
          : "";
      }
  ...
  const handleEditSubmit = async (e) => {
    ...
    const selectedProject = projects.find(
      (p) => p.id == editFormData.project_id,
    );
    const selectedPlot = filteredPlots.find(
      (p) => p.id == editFormData.plot_id,
    );
      ...
      <div className="card mt-2">
        ...
        {isFilterActive && (
          <div className="card-body pb-0">
            <div className="d-flex gap-2 flex-wrap-mobile expencelistwrap">
                      ...
                      {expenses.length > 0 ? (
                        expenses.map((expense, index) => (
                          <tr key={expense.id}>
                            ...
                            <td>{expense.category_type || "NA"}</td>
                            <td>{formatText(expense.payee)}</td>
                              ...
                              <div className="table-cell-remark">
                                {expense.description
                                  ? expense.description
                                      .charAt(0)
                                      .toUpperCase() +
                                    expense.description.slice(1).toLowerCase()
                                  : "NA"}
                              </div>
                            </td>
                            <td>
                                ...
                                >
                                  ...
                                  <li className="dropdown-item">
                                    <button
                                      ...
                                      onClick={() =>
                                        handleDeleteModalShow(expense.id)
                                      }
