...
const Dashboard = ({ userType }) => {
  ...
  const fetchRoyaltyRewardsList = async () => {
    try {
      ...
      const data = await response.json();
      console.log("Royalty Rewards List Response:", data);
      ...
      console.error("Royalty Rewards list fetch error:", error);
      setRoyaltyRewardsList([]);
    } finally {
  ...
  const fetchRoyaltyRewardsEligibility = async () => {
    try {
      ...
      const data = await response.json();
      console.log("Royalty Rewards Eligibility Response:", data);
      ...
      console.error("Royalty Rewards eligibility fetch error:", error);
      setRoyaltyEligibilityData([]);
    } finally {
  ...
  useEffect(() => {
    ...
    fetchRoyaltyRewardsList(); // Add this
    fetchRoyaltyRewardsEligibility(); // Add this
  }, []);
  ...
  const RoyaltyRewardsProgressBar = () => {
    ...
    // Static leg data for Royalty Rewards - exactly like Lifetime Rewards
    ...
    const eligibilityData =
      royaltyEligibilityData.length > 0 ? royaltyEligibilityData[0] : null;
    ...
    const maxSqyd =
      parseFloat(royaltyRewardsList[royaltyRewardsList.length - 1]?.area_sqyd) ||
      Math.max(...royaltyRewardsList.map((g) => parseFloat(g.area_sqyd)));
      ...
      <div className="gift-progress-wrapper">
        <div className="gift-progress-header">
          ...
          {eligibilityData ? (
            ...
            <NoDataMessage message="Sorry, no royalty Rewards eligibility data found" />
          )}
        </div>
              ...
              <div className="gift-steps-flex-container">
                ...
                {royaltyRewardsList.map((reward, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="gift-step-content-wrapper">
                        {/* Royalty Leg Data - Exactly like Lifetime Rewards */}
                        <div className="gift-step-target gift_content mb-2">
                          <ul className="leg_content">
        ...
        {royaltyRewardsList.length > 0 ? (
          ...
          <NoDataMessage message="Sorry, no royalty eligibility date data found" />
        )}
      </div>
    );
  };
