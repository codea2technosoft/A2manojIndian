...
function MasterTransction() {
  ...
  //     const response = await getChildList(2);
  ...
  const fetchClients = async () => {
    try {
      setClientLoading(true);
      const response = await SelectMastertList({
        role: 2,
      });
      ...
      if (response.data && response.data.success) {
        ...
        if (Array.isArray(clientData)) {
          ...
          clientsList = clientData.data;
        } else if (clientData.lena || clientData.dena || clientData.clear) {
          clientsList = [
            ...(clientData.lena || []),
            ...(clientData.dena || []),
            ...(clientData.clear || []),
        ...
        if (master_id && filteredClients.length > 0) {
          ...
          if (selectedClient) {
            ...
            Swal.fire({
              ...
              text: "The specified master ID was not found in the list.",
            });
          }
        }
  ...
  const handleDeleteTxn = async (ledger_id) => {
    ...
    try {
      const res = await MasterdeleteLedgerTxn({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await addNewTransactionmaster(transactionData);
      if (response.data && response.data.success) {
        await Swal.fire({
          icon: "success",
    ...
    return (
      <div className="card mt-4">
        <div className="card-header bg-color-black p-2">
          <h3 className="card-title text-white mb-0">MASTER LEDGER DETAILS</h3>
        </div>
              ...
              <div className="text-center border-danger">
                <div className="master-txn gap-2 d-flex align-items-end">
                  ...
                  <div
                    ...
                    onClick={() =>
                      navigate("/agent_master-deleted-transactions", {
              ...
              <table className="table table-bordered table-striped">
                ...
                <tbody>
                  {ledgerData.map((transaction, index) => {
                    ...
                    // const dr = transaction.payment_type === "credit" ? amount : 0;
                    // const cr = transaction.payment_type === "debit" ? amount : 0;
                    const diya = Number(transaction.debit) || 0;
                    const liya = Number(transaction.credit) || 0;
                    const balance = parseFloat(transaction.balance) || 0;
                    const balanceType = balance >= 0 ? "CREDIT" : "DEBIT";
                    return (
                      <tr
                        key={transaction.id || transaction.lenden_uuid || index}
                      >
  ...
  //   { label: "PAYMENT DIYA", value: "dr" },
  //   { label: "PAYMENT LIYA", value: "cr" },
                ...
                <div className="col-md-6 mb-3">
                  ...
                  >
                    ...
                    {clientLoading ? (
                      ...
                      clients.map((client) => (
                        <option key={client._id} value={client.admin_id}>
                          {client.username}
                        </option>
                      ))
                    ) : (
...
export default MasterTransction;
