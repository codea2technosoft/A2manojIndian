...
import { Modal, Button, Form, Table, Pagination, Spinner } from "react-bootstrap";
...
function DocumentList() {
  ...
  const fetchDocuments = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/realstate-website-document-list?page=${page}&limit=${itemsPerPage}`, {
        method: "GET",
        headers: headers,
      });
      ...
      if (response.ok) {
        ...
        showCustomMessageModal("Error", `Failed to fetch documents: ${errorData.message || response.statusText}`, "error");
      }
    } catch (error) {
  ...
  const handleUpdateDocument = async (event) => {
    ...
    if (!editDocumentName.trim()) {
      showCustomMessageModal("Validation Error", "Document name cannot be empty.", "warning");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        showCustomMessageModal("Success", "Document updated successfully!", "success");
        setShowEditModal(false);
        fetchDocuments(currentPage);
      } else {
        const errorData = await response.json();
        showCustomMessageModal("Error", `Update failed: ${errorData.message || response.statusText}`, "error");
      }
    } catch (error) {
      console.error("Network or unexpected error during update:", error);
      showCustomMessageModal("Error", `An unexpected error occurred during update: ${error.message}`, "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        showCustomMessageModal("Success", "Document deleted successfully!", "success");
        setShowDeleteConfirmModal(false);
        fetchDocuments(currentPage);
      } else {
        const errorData = await response.json();
        showCustomMessageModal("Error", `Deletion failed: ${errorData.message || response.statusText}`, "error");
      }
    } catch (error) {
      console.error("Network or unexpected error during delete:", error);
      showCustomMessageModal("Error", `An unexpected error occurred during delete: ${error.message}`, "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="card mt-2">
      ...
      <div className="card-body">
        {loading ? (
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
            <Table striped bordered hover className="min-w-full divide-y divide-gray-200">
      ...
      <Modal show={showViewModal} onHide={handleCloseViewModal} centered>
        ...
        <Modal.Body>
          {viewingDocument && (
            <>
              <p><strong>Document Name:</strong> {viewingDocument.name}</p>
              <hr />
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                  {viewingDocument.image.split(',').map((img, index) => (
                    <div key={index} style={{ textAlign: 'center' }}>
                      {img.toLowerCase().endsWith('.pdf') ? (
                        <div style={{ width: '100px', height: '100px', background: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #ddd' }}>
      ...
      <Modal show={showDeleteConfirmModal} onHide={handleCloseDeleteConfirmModal} centered>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={handleCloseDeleteConfirmModal}>Cancel</Button>
      ...
      <Modal show={showMessageModal} onHide={closeCustomMessageModal} centered>
        <Modal.Header closeButton>
          <Modal.Title className={messageModalContent.type === "success" ? "text-success" : messageModalContent.type === "warning" ? "text-warning" : "text-danger"}>
