...
function GameLoadBetLossLists() {
  ...
  // const fetchSuccessBets = async () => {
  //   setLoading(true);
  //   try {
  //     const res = await fetch(
  //       `${process.env.REACT_APP_API_URL}/gameloadbet-loss-list?page=1&limit=1000`,
  //       {
  //         method: "GET",
  //         headers: {
  ...
  useEffect(() => {
    fetchSuccessBets(currentPage);
  }, [currentPage, FilterUsername, FilterMarkettype, FilterMarketname, FilterGameType, FilterSession, selectedStartDate, selectedEndDate]);
  ...
  //       }/gameloadbet-loss-list?page=${page.toString()}&limit=${limit.toString()}&username=${FilterUsername}&markettype=${FilterMarkettype}&marketname=${FilterMarketname}&gametype=${FilterGameType}&session=${FilterSession}&startdate=${selectedStartDate}&enddate=${selectedEndDate}`,
  //       {
  //         method: "GET",
  //         headers: {
  //           "Content-Type": "application/json",
  ...
  const fetchSuccessBets = async (page = 1) => {
    ...
    try {
      ...
      const todaybet = urlParams.get("todaybet");
      const statusParam = urlParams.get("status");
      const allbet = urlParams.get("allbet");
      ...
      // यह logic change करो:
      if (todaybet === "yes" && !initialLoadDone && !isFromDashboard) {
      ...
      // Status parameter add करो
      let statusQuery = "";
      if (statusParam) {
        statusQuery = `&status=${statusParam}`;
      }

      const apiUrl = `${process.env.REACT_APP_API_URL}/gameloadbet-loss-list?page=${page.toString()}&limit=${limit.toString()}&username=${FilterUsername}&markettype=${FilterMarkettype}&marketname=${FilterMarketname}&gametype=${FilterGameType}&session=${FilterSession}&startdate=${startDateForFetch}&enddate=${endDateForFetch}${statusQuery}`;

      console.log("Bet Loss API URL:", apiUrl);
      ...
      const data = await res.json();
      console.log("Bet Loss API Response:", data);
      ...
      <div className="card">
        <div className="card-header bg-color-black">
          <div className="d-flex align-items-center justify-content-between">
            <h3 className="card-title text-white">Loss Bet List</h3>
...
export default GameLoadBetLossLists;
