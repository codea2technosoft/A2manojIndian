...
function UsersList() {
  ...
  const handleResetFilters = () => {
    ...
    setSelectedValueWinnerLosser("");
    setSelectedValuecolor("");
  ...
  const fetchUsers = async () => {
    ...
    // यह logic change करो:
    // Today users
    if (todayuser === "yes" && !initialLoadDone && !isFromDashboard) {
      const today = new Date();
      const formattedDate = today.toISOString().split("T")[0];
      startDateForFetch = formattedDate;
      endtDateForFetch = formattedDate;
    ...
    // Yesterday users
    if (yestardayuser === "yes" && !initialLoadDone && !isFromDashboard) {
    ...
    // Zero balance users
    if (zerobalance === "yes" && !isFromDashboard) {
      minForFetch = "0";
      setFilterMin("0");
    }

    // Clear dashboard date if user manually changes date
    if ((selectedStartDate || selectedEndDate) && isFromDashboard) {
      setIsFromDashboard(false);
    }
    ...
    try {
      ...
      if (result.success === "1") {
        ...
        setLimitPages(result.pagination.limit);
      } else {
        setUsers([]);
        setFilteredUsers([]);
      }
  ...
  // filterdata function में change:
  const fillterdata = () => {
    setFillter((prev) => !prev);
    // Filter button click पर automatically fetch नहीं करें
    // User को Filter button click करने का option दें
  };
  ...
  const [fillter, setFillter] = useState(true);
            ...
            <div className="row">
              <div className="col-md-12">
                <div className="form-design-fillter many_field gap-2 d-flex align-items-end flex-wrap">
                  ...
                  <div className="form_latest_design gap-2 d-flex">
                    <button
                            ...
                            <div style={{
                              ...
                              >
                                <svg
                                  ...
                                  fill="#ffffff"
                                >
                                  <path d="M16 .4C7.4.4.4 7.4.4 16c0 2.8.7 5.5 2.1 7.9L.2 31.8l8-2.1c2.3 1.3 4.9 2 7.8 2 8.6 0 15.6-7 15.6-15.6S24.6.4 16 .4zm0 28.5c-2.5 0-5-.7-7.1-2l-.5-.3-4.7 1.2 1.3-4.6-.3-.5C3.3 20.8 2.6 18.5 2.6 16 2.6 8.9 8.9 2.6 16 2.6S29.4 8.9 29.4 16 23.1 28.9 16 28.9zm7.2-9.8c-.4-.2-2.2-1.1-2.5-1.2-.3-.1-.5-.2-.7.2-.2.3-.9 1.2-1.1 1.4-.2.2-.4.3-.7.1-.3-.2-1.4-.5-2.7-1.7-1-1-1.7-2.3-1.9-2.7-.2-.3 0-.5.1-.7.1-.1.3-.4.5-.6.2-.2.2-.4.3-.6.1-.2 0-.5-.1-.7-.1-.2-.7-1.8-1-2.5-.3-.7-.6-.6-.8-.6h-.7c-.2 0-.6.1-.9.4-.3.3-1.1 1-1.1 2.4 0 1.4 1 2.7 1.2 2.9.1.2 2 3 4.8 4.2.7.3 1.2.5 1.6.6.7.2 1.3.2 1.8.1.6-.1 1.8-.7 2.1-1.4.3-.7.3-1.3.2-1.4-.1-.1-.3-.2-.6-.4z" />
                                </svg>
                              </a>
