      ...
      <div className="bet-slip-modal-content">
        ...
        <div className="modal-body">
          <div className="d-flex justify-content-between align-items-center mb-2 bettypeall">
            <span className="bettypetext">
              {selectedOdds?.marketName || selectedOdds?.eventName || "Match Odds"}
            </span>
            <span
          ...
          <div className="mb-2 p-2 bg-light rounded">
            <small className="text-muted">
              Selection ID: {selectedOdds?.selectionId || selectedOdds?.rowId}
              <br />
              Runner: {selectedOdds?.runnerName || selectedOdds?.eventName}
            </small>
          </div>
          ...
          <div className="bet-details mb-3" style={{
            ...
            {/* Show min/max for fancy bets */}
            {MatchType === "fancy" && (
              <div className="row mt-2">
                <div className="col-6">
                  <small className="text-muted">Min: {selectedOdds?.min || 100}</small>
                </div>
                <div className="col-6 text-end">
                  <small className="text-muted">Max: {selectedOdds?.max || 1000}</small>
                </div>
              </div>
            )}
          </div>
...
const BookmakerTable = memo(
  ({
    ...
    const getBookmakerExposure = (runnerName, selectionId) => {
      ...
      // Debug log
      console.log("Bookmaker Exposure Search:", {
        selectionId,
        runnerName,
        matchesNameCount: matchesName.length,
        availableExposureData: bookmakerExposureData
      });
      ...
      if (matchedTeam) {
        ...
        console.log("✅ Found team in matchesName:", matchedTeam.team_name, "team_id:", teamId);
      } else {
        console.log("❌ Runner not found in matchesName:", runnerName);
        ...
        if (matchesName.length > 0) {
          ...
          if (runners && Array.isArray(runners)) {
            ...
            if (runnerIndex !== -1 && matchesName[runnerIndex]) {
              ...
              console.log("✅ Matched by index position:", runnerIndex, "team_id:", teamId);
            }
          }
        }
      }
      ...
      if (teamId) {
        ...
        if (exposureItem) {
          console.log("✅ Found bookmaker exposure by team_id:", teamId, "amount:", exposureItem.amount);
          return exposureItem.amount;
        } else {
          console.log("❌ No exposure data found for team_id:", teamId);
        }
      }
      
      // Last resort: try to find by amount matching (if only one exposure item)
      if (bookmakerExposureData.length === 1) {
        console.log("⚠️ Using single exposure data item as fallback");
        return bookmakerExposureData[0].amount;
      }
      
      console.log("❌ Could not find bookmaker exposure for:", runnerName);
      return null;
    };
                                    ...
                                    {exposureAmount !== null && exposureAmount !== undefined && (
                                      <div
                                        style={{
                                          ...
                                          fontSize: "12px",
                                          fontWeight: "bold",
                                          backgroundColor: exposureAmount > 0 ? "#e6ffe6" : 
                                                         exposureAmount < 0 ? "#ffe6e6" : "#f5f5f5",
                                          padding: "2px 6px",
                                          borderRadius: "4px",
                                          border: exposureAmount > 0 ? "1px solid #108f10" : 
                                                 exposureAmount < 0 ? "1px solid #ff0000" : "1px solid #ddd"
                                        }}
...
function Cricket() {
  ...
  const fetchTeamExposureData = useCallback(async () => {
    try {
      ...
      console.log("Fetching exposure data for:", requestData);
      ...
      console.log("📊 Match Odds/Bookmaker Exposure API Response:", matchOddsResponse.data);
      console.log("📊 Fancy Exposure API Response:", fancyResponse.data);
      ...
      console.log("✅ Combined Exposure Data:", {
        match_odds: matchOddsExposure,
        bookmaker: bookmakerExposure,
        fancy: fancyExposure
      });
      
      // showNotification("Exposure data loaded successfully", "success");
    } catch (error) {
      console.error("Error fetching team exposure data:", error);
      showNotification("Failed to load exposure data", "error");
      setTeamExposureData({ match_odds: [], bookmaker: [], fancy: [] });
    } finally {
      setExposureLoading(false);
    }
  }, [event_id, baseUrl, showNotification]);
  ...
  }, [event_id, fetchTeamExposureData]);
      ...
      if (
        ...
        if (
          ...
          const transformedMatches = marketData.runners.map((runner, index) => {
            // Try multiple ways to match team name
            let teamName = `Team ${index + 1}`;
            
            // Method 1: Try to match by team_id from matchesName
            if (matchesName.length > 0) {
            ...
            // If still no match, use the runner's name if available
  ...
  const placeSessionBet = useCallback(
    async (betData) => {
      try {
        ...
        console.warn("📤 Original Fancy Bet Data:", completeBetData);
  ...
  const placeRegularBet = useCallback(
    async (betData) => {
      try {
        ...
        console.warn("📤 Original Regular Bet Data:", completeBetData);
...
const handlePlaceBet = useCallback(() => {
  ...
  console.log("🎯 Selected Odds Data for Bet:", {
    selectedOdds,
    matchType,
    marketName,
    stakeValue,
  ...
  // Prepare bet data based on match type
  if (matchType === "fancy") {
    ...
    console.log("📤 Fancy Bet Data to send:", fancyBetData);
    ...
    // FIXED: Get team_id from matchesName for bookmaker
    let teamId = null;
    
    // Try to find the team in matchesName
    if (matchesName.length > 0) {
      // First try to match by runner name
      const matchedTeam = matchesName.find(team => {
      ...
      if (matchedTeam) {
        ...
        // Fallback: If we have exactly 2 teams in matchesName, assign based on position
        // Assuming first runner is first team, second runner is second team
        if (matchesName.length >= 2) {
          // Check if this is the first or second runner in the bookmaker market
    ...
    // If still no teamId found, use selectionId as fallback
    if (!teamId) {
    ...
    const bookmakerBetData = {
      ...
      sport_id: "4",
      team_id: teamId, // ✅ Now properly set
      team: teamName,
      market_name: marketName,
      size: selectedOdds.size || 0
    };

    console.log("📤 Bookmaker Bet Data to send:", {
      ...bookmakerBetData,
      matchesNameCount: matchesName.length,
      foundTeamId: teamId
    });
    ...
    console.log("📤 Match Odds Bet Data to send:", matchOddsBetData);
  ...
  series_idd,
  matchesName,
  bookmakerList, // Added bookmakerList to dependencies
]);
  ...
  const handleOddsClick = useCallback(
    (oddsValue, type, market, oddType, rowId) => {
      ...
      console.log("Selected Odds Team Info:", {
        rowId,
        teamName,
        marketTeamName: market.team_name,
  ...
  const handleFancyOddsClick = useCallback(
    (oddsValue, type, market, clickedSide) => {
      ...
      console.log("📊 Fancy Odds Click Details:", {
        rowId,
        price,
        size,
        type,
      ...
      setSelectedOdds({
        ...
        rowId: rowId,
        selectionId: market.SelectionId, // ✅ यह जरूरी है
        runnerName: market.RunnerName,   // ✅ यह जरूरी है
        min: market.min,
        max: market.max,
  ...
  const FancyBetsSection = memo(() => {
    ...
    const getFancyExposure = (fancyId, sessionId) => {
      ...
      if (!exposureItem && fancyId) {
        // Try partial match (e.g., "35132021-11" matches "11")
      ...
      if (exposureItem) {
        console.log("✅ Found fancy exposure:", {
          fancyId,
          sessionId,
          exposure: exposureItem.exp,
          item: exposureItem
        });
        return exposureItem.exp;
      }
      
      console.log("❌ No fancy exposure found for:", { fancyId, sessionId });
      return null;
    };
        ...
        >
          ...
          >
            ...
            <div className="bet-history-body">
              <div className="bet-table-scroll">
                <Homepageslider />
              </div>
            </div>
          </div>
