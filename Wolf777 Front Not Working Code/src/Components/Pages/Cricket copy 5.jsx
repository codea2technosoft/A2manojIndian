...
import Ladder from '../Pages/Ledger'
import {
  Row,
  Col,
  InputGroup,
...
function Cricket() {
  ...
  // ✅ ADDED: Market Availability State
  const [marketAvailability, setMarketAvailability] = useState({
    bookmaker: false,
    fancy: false,
    matchOdds: true,
  });
  ...
  // ✅ ADDED: useEffect to check market availability
  useEffect(() => {
    const checkMarketAvailability = () => {
      try {
        // Check bookmaker availability
        const hasBookmaker = 
          Array.isArray(bookmakerList) && 
          bookmakerList.length > 0 && 
          bookmakerList.some(market => 
            market && 
            market.runners && 
            market.runners.length > 0 &&
            market.runners.some(runner => {
              const hasBackOdds = runner.ex?.availableToBack?.some(back => back.price > 0);
              const hasLayOdds = runner.ex?.availableToLay?.some(lay => lay.price > 0);
              return hasBackOdds || hasLayOdds;
            })
          );
        
        // Check fancy availability
        const hasFancy = 
          Array.isArray(fancylist) && 
          fancylist.length > 0 &&
          fancylist.some(market => 
            market && 
            (market.BackPrice1 > 0 || market.LayPrice1 > 0)
          );
        
        // 🔥 IMPORTANT BUSINESS RULE: Bookmaker न हो → Fancy न हो
        const finalFancyStatus = hasBookmaker ? hasFancy : false;
        
        setMarketAvailability({
          bookmaker: hasBookmaker,
          fancy: finalFancyStatus,
          matchOdds: true
        });
        
        console.log("Market Availability Check:", {
          bookmaker: hasBookmaker,
          fancy: finalFancyStatus,
          fancyRaw: hasFancy,
          bookmakerListLength: bookmakerList?.length,
          fancylistLength: fancylist?.length
        });
        
      } catch (error) {
        console.error("Error checking market availability:", error);
        setMarketAvailability({
          bookmaker: false,
          fancy: false,
          matchOdds: true
    ...
    const interval = setInterval(checkMarketAvailability, 3000);
    
    return () => clearInterval(interval);
  }, [bookmakerList, fancylist]);
    ...
    (value) => {
      if (MatchType === "match_odds") {
        if (bgColor === "#72bbef") {
          ...
          setteamexposercurrent2Name(otherTeamName);
