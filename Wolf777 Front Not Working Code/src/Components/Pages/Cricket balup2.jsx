  ...
  ({
    ...
    useEffect(() => {
      ...
      const walletInterval = setInterval(() => {
        fetchwallet_amount();
      }, 2000); // 👈 har 2 sec me call (aap change kar sakte ho)
...
function Cricket() {
  ...
  const intervalId = setInterval(() => {
    ...
    if (count == 5) {
      ...
      // 10 minute baad fir se 5 call
      setTimeout(() => {
        callOddsFiveTimes();
      }, 10 * 60 * 1000); // 10 minutes
    }
  }, 100); // 200ms × 5 = 1 second
}
  ...
  const checkAuthentication = useCallback(() => {
    ...
    // }
    if (!token) {
      navigate("/login"); // 👉 login page par navigate
      return false;
    }
    return true;
  }, [openLogin]);
