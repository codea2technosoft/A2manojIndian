...
function Adminchat() {
  ...
  useEffect(() => {
    ...
    if (!socketRef.current) {
      try {
        const socket = io("https://chatapi.demomayamatka.shyammatka.co.in", {
  ...
  const fetchChats = async (page = 1) => {
    try {
      const res = await fetch(
        `https://demomayamatkaadminapi.shyammatka.co.in/chat-list-admin?page=${page}&limit=20&username=${FilterUsername}&type=${Filtertype}`,
        {
          method: "GET",
          headers: {
