...
const Sidebar = ({ isOpen, onToggleSidebar, userType, permissions }) => {
  ...
  const [menuItems, setMenuItems] = useState([
    {
      ...
      items: [
...

        {
          id: "logout",
          title: "Logout",
          // onClick: handleLogout, // This will be handled by handleItemClick
          href: "#",
          icon: (
          ...
          checked: true // Logout always visible
        }
      ]
    }
  ]);
        ...
        if (section.section === "Menu") {
          return {
            ...
            items: section.items.map(item => {
              // Always show dashboard and logout for everyone
              if (item.id == "dashboard" || item.id === "logout") {
                return {
                  ...item,
                  checked: true,
              ...
              const hasParentPermission = permissions?.some(perm => {
                // Check if permission matches the parent route
              ...
              // Show parent if it has direct permission or any child has permission
  ...
  const renderMenuItem = (item, level = 0) => {
    ...
    if (!item.checked && userType?.type !== 'admin' && item.id !== "dashboard" && item.id !== "logout") {
      return null;
    }
    ...
    if (item.dropdown && item.children && userType?.type !== 'admin') {
      ...
      if (!hasVisibleChildren && item.id !== "dashboard" && item.id !== "logout") {
        return null;
      }
    }
