  ...
  const fetchLedgerData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/project_wise-ledger-report?${params}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
              ...
              <>
                ...
                <div className="mb-3">
                  <Card className="bg-light">
                    <Card.Body>
                      <h5 className="mb-0">
                        Project: {ledgerData.project.name}
                      </h5>
                      {ledgerData.filters.date_range_applied && (
                        <small className="text-muted">
                          Date Range: {ledgerData.filters.start_date || "Start"}{" "}
                          to {ledgerData.filters.end_date || "End"}
                        </small>
                      )}
                    </Card.Body>
                  </Card>
                        ...
                        <Card.Body>
                          <div className="summary-item">
                            ...
                            <h4 className="text-info mb-0">
                              {formatCurrency(
                                parseFloat(ledgerData.summary.total_balance),
                              )}
                            </h4>
                          </div>
                        </Card.Body>
                    ...
                    {ledgerData.plots_ledger.map((item, index) => (
                      <tr key={item.plot.id || index}>
                        ...
                        <td className="text-success">
                          {formatCurrency(
                            parseFloat(item.financials.total_credit),
                          )}
                        </td>
                        <td className="text-danger">
                          {formatCurrency(
                            parseFloat(item.financials.total_debit),
                          )}
                        </td>
                        <td
                          className={
                            parseFloat(item.financials.balance) >= 0
                              ? "text-primary"
                              : "text-danger"
                          }
                        >
