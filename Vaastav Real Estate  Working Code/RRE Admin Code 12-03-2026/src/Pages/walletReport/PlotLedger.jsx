    ...
    const fetchPlots = async () => {
      ...
      try {
        const plotsResponse = await fetch(
          `${API_URL}/all-plot-list-by-id?project_id=${formData.project_id}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  const fetchLedgerData = async () => {
    ...
    try {
      ...
      if (data.success === "1") {
        ...
        Swal.fire(
          "Error",
          data.message || "Failed to load ledger data",
          "error",
        );
      }
    } catch (err) {
      ...
      <div className="userlist">
        <div className="card shadow-sm mb-3">
          ...
          <div className="card-body">
            {isFilterActive && (
              <div className="mb-4">
                <div className="gy-2 d-flex align-items-center gap-2 flex-wrap-mobile">
                  ...
                  <Button className="mt-md-4"
                    ...
                    disabled={
                      !formData.project_id || !formData.plot_id || loading
                    }
                  >
                    ...
                    <Row>
                      <Col md={4}>
                        <div className="summary-item">
                          ...
                          <h4 className="text-success mb-0">
                            {formatCurrency(
                              parseFloat(ledgerData.summary.total_credit),
                            )}
                          </h4>
                        </div>
                      </Col>
                      <Col md={4}>
                        <div className="summary-item">
                          ...
                          <h4 className="text-danger mb-0">
                            {formatCurrency(
                              parseFloat(ledgerData.summary.total_debit),
                            )}
                          </h4>
                        </div>
                      </Col>
                      <Col md={4}>
                        <div className="summary-item">
                          ...
                          <h4
                            className={`mb-0 ${parseFloat(ledgerData.summary.closing_balance) >= 0 ? "text-primary" : "text-danger"}`}
                          >
                            {formatCurrency(
                              parseFloat(ledgerData.summary.closing_balance),
                            )}
                          </h4>
                        </div>
                      </Col>
                  ...
                  <tbody>
                    {ledgerData.ledger.map((entry, index) => (
                      <tr key={index}>
                        ...
                        <td>{getTransactionTypeBadge(entry.source_table)}</td>
                        ...
                        <td className="text-end text-success">
                          {parseFloat(entry.cr) > 0
                            ? formatCurrency(parseFloat(entry.cr))
                            : "-"}
                        </td>
                        <td className="text-end text-danger">
                          {parseFloat(entry.dr) > 0
                            ? formatCurrency(parseFloat(entry.dr))
                            : "-"}
                        </td>
                        <td
                          className={`text-end fw-bold ${parseFloat(entry.balance) >= 0 ? "text-primary" : "text-danger"}`}
                        >
