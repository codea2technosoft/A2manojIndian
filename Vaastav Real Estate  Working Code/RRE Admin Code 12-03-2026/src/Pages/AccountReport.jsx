  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const afterDebit = creditAmount - debitAmount;
        const walletTotal =
          (apiData.wallet_balance || 0) + (apiData.pending_wallet_balance || 0);
        const afterWallet = afterDebit - walletTotal;
        const tdsAmount = apiData.tds_balance || 0;
        ...
        const calculatedFinalAmount =
          afterTds - monthlyReward - lifetimeReward - royaltyAmount;
        setFinalAmount(calculatedFinalAmount);
        ...
        // Calculate balance amounts
        const cashBalance =
          (apiData.cr_cash?.total_amount || 0) -
          (apiData.dr_cash?.total_amount || 0);
        const onlineBalance =
          (apiData.cr_online?.total_amount || 0) -
          (apiData.dr_online?.total_amount || 0);
        const totalBalance =
          (apiData.cr_total?.total_amount || 0) -
          (apiData.dr_total?.total_amount || 0);
        ...
        const transformedData = [
          ...
          // Balance Section (Row 3)
          {
            ...
            numericValue: cashBalance,
            count:
              (apiData.cr_cash?.transaction_count || 0) +
              (apiData.dr_cash?.transaction_count || 0),
            icon: "FaMoneyBillTransfer",
            color: "primary",
            link: "/cash-balance-list",
            type: "balance",
          },
          {
            ...
            numericValue: onlineBalance,
            count:
              (apiData.cr_online?.transaction_count || 0) +
              (apiData.dr_online?.transaction_count || 0),
            icon: "FaMoneyBillTransfer",
            color: "info",
            link: "/online-balance-list",
            type: "balance",
          },
          {
            ...
            numericValue: totalBalance,
            count:
              (apiData.cr_total?.transaction_count || 0) +
              (apiData.dr_total?.transaction_count || 0),
            icon: "FaMoneyBillTransfer",
            color: "success",
            link: "/total-balance-list",
            type: "balance",
          ...
          {
            title:
              "Total Wallet Balance (After Withdrawal Request & Without Withdrawal Request)",
            value: formatCurrency(
              apiData.wallet_balance + apiData.pending_wallet_balance,
            ),
            numericValue:
              apiData.wallet_balance + apiData.pending_wallet_balance || 0,
            icon: "GiWallet",
            color: "success",
            link: "/success-wallet-balance",
            type: "wallet",
    ...
    <div className="col-xs-12 col-sm-6 col-md-4 col-lg-4">
      <div className="card bg_card_design">
        <div className="card-body d-flex align-items-center gap-3">
          ...
          <div>
            <div className="card-title text-dark">Final Profit / Loss</div>

            <div
              className={`fw-bold h4 ${
                finalAmount < 0 ? "text-danger" : "text-success"
              }`}
            >
              {finalAmount < 0
            ...
            <small className="text-muted">
              Credit − Debit − Wallet − TDS − Monthly Reward − Lifetime Reward −
              Royalty
            </small>
          </div>
        </div>
    ...
    <div className="padding_15">
      <div className="dashboard">
          ...
          {dashboardData
            ...
            .map((item, index) => {
              ...
              const isNegativeOrZero =
                item.numericValue < 0 || item.numericValue === 0;
                  ...
                  >
                    <div className="card-body gap-2 d-flex align-items-center justify-content-start">
                      ...
                      <div>
                        <div className="card-title text-dark">{item.title}</div>
                        <div
                          className={`card-text fw-bold h4 ${isNegativeOrZero ? "text-danger" : ""}`}
                        >
                          {item.value}
                        </div>
                        <small className="text-muted">
        ...
        <Row className="g-4 mt-4">
          ...
          {dashboardData
            .filter((item) => ["wallet", "tds"].includes(item.type))
            .map((item, index) => {
              const IconComponent = iconMap[item.icon];
              const isNegativeOrZero =
                item.numericValue < 0 || item.numericValue === 0;
        ...
        <Row className="g-4 mt-4">
          ...
          {dashboardData
            .filter((item) => ["reward", "royalty"].includes(item.type))
            .map((item, index) => {
              const IconComponent = iconMap[item.icon];
              const isNegativeOrZero =
                item.numericValue < 0 || item.numericValue === 0;
            ...
            <div className="card bg_card_design">
              <div className="card-body d-flex align-items-center gap-3">
                ...
                <div>
                  ...
                  <small className="text-muted">
                    Credit − Debit − Wallet − TDS - Monthly Reward Balance -
                    Lifetime Reward Balance - Royalty Balance
                  </small>
                </div>
              </div>
            </div>
                ...
                <div className="modal-header d-flex justify-content-between align-items-center">
                  <h5
                    className={`modal-title ${
                      messageModalContent.type === "error" ? "text-danger" : ""
                    }`}
                  >
