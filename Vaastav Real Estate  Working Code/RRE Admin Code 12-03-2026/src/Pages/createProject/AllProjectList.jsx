...
import { FaPlus, FaEye, FaEdit, FaUpload } from "react-icons/fa";
...
function AllProjectList() {
  ...
  useEffect(() => {
    ...
    fetchStates();
    fetchAmenities();
    fetchProjectCategories(); // Add this line
  }, []);
  ...
  const fetchProjects = async (page = 1, name = "", location = "") => {
    // setLoading(true);
    setError(null);
    try {
      const token = getAuthToken();
      ...
      const response = await fetch(
        `${API_URL}/project-list?page=${page}&limit=10&name=${name}&location=${location}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    fetchProjects(currentPage, searchName, searchLocation);
    return () => {
  ...
  const handleSearchChange = (e) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      setCurrentPage(1);
      fetchProjects(1, value, searchLocation);
    }, 500);
  };
  ...
  const handleSearchChangeLocation = (e) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      setCurrentPage(1);
      fetchProjects(1, searchName, value);
    }, 500);
  };
...
const handleViewProject = async (projectId) => {
  ...
  //     // Safe parser - checks if value is already an object
  //     const safeParse = (value) => {
  //       if (!value) return [];
  //       if (Array.isArray(value)) return value;
  //       if (typeof value === 'object') return value;
  ...
  //     // Handle amenities - could be array or could need parsing
  //     let parsedAmenities = [];
  //     if (projectData.aminities) {
  //       const amenitiesData = safeParse(projectData.aminities);
  //       parsedAmenities = amenitiesData.map((a) => String(a.id));
  //     }

  //     // Handle key transports
  ...
  //       category_id: projectData.category_id || "", // Add this
  ...
  const handleEditProject = async (projectId) => {
    ...
    try {
      ...
      setEditFormData({
        ...
        // category_name: categoryName, // Use the found category name
        newImages: [],
        images: imagesArray,
        pdfs: pdfsArray,
        singleImageFile: null,
  ...
  //     let url = `${API_URL}/project-excel-download`;
  ...
  //     const worksheet = XLSX.utils.json_to_sheet(allProjects.map(projects => ({
  //       "SL No.": allProjects.indexOf(projects) + 1,
  //       "ID": projects.id,
  //       "name": projects.name,
  //       "total_township_area": projects.total_township_area,
  //       "project_rera_no": projects.project_rera_no,
  //       "location": projects.location,
  //       "bussiness_volume": projects.bussiness_volume,
  //       "state": projects.state,
  //       "city": projects.city,
  //       "land_mark": projects.land_mark,
  //       "key_transport": projects.key_transport,
  //       "status": projects.status,
  //       "project_status": projects.project_status,
  //       "description": projects.description,
  //       "approve_authority": projects.approve_authority,
  //       "youtube_links": projects.youtube_links,
  //     })));

  //     const workbook = XLSX.utils.book_new();
  //     XLSX.utils.book_append_sheet(workbook, worksheet, "Projects");
  //     XLSX.writeFile(workbook, `All_Projects_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const exportAllToExcel = async () => {
    try {
      ...
      if (searchMobile) url += `?mobile=${searchMobile}`;
      if (searchName)
        url += searchMobile ? `&name=${searchName}` : `?name=${searchName}`;
          ...
          <div className="d-none d-md-block">
            <div className="d-flex gap-2 align-items-center">
              {/* <div className="createnewadmin">
                <Link to="/create-project-by-uploading-csv" className="btn btn-success d-inline-flex align-items-center">
                  <FaPlus className="me-1" /> Upload New Project
                </Link>
              </div> */}
            </div>
          </div>
          <div className="d-flex gap-2 align-items-center flex-wrap-mobile gap-2">
            <>
            ...
            <>
              {!allowedDomains.includes(currentDomain) && (
                <div className="createnewadmin">
                  ...
                  >
                    <FaUpload className="me-1" /> Upload New Project
                  </Link>
                </div>
              )}
            </>
            ...
            <div className="createnewadmin">
              <Link
                ...
                className="btn btn-success d-inline-flex align-items-center"
              >
                <FaPlus className="me-1" /> Add New Project
              </Link>
            </div>
            <button
                ...
                projects.map((project, i) => {
                  ...
                  return (
                    <tr key={project.id}>
                      ...
                      <td>
                        {project.project_category_name || "NA"}
                      </td>
                    ...
                    <Form.Select
                      ...
                      onChange={(e) => {
                        ...
                        setEditFormData((prevData) => ({
                          ...
                          category_name: selectedCategory ? selectedCategory.category_name : "", // Use category_name
                        }));
                      }}
                      required
                    >
                ...
                {editFormData.pdfs && editFormData.pdfs.length > 0 ? (
                  <ul className="list-unstyled">
                    {editFormData.pdfs.map((pdf) => (
                      ...
                      >
                        <a
                          href={`${imageAPIURL}/project/${pdf.pdf}`}
                          target="_blank"
                          rel="noopener noreferrer"
                        >
                          <img
                            src={`${imageAPIURL}/project/${pdf.thumbnail}`}
                            alt={pdf.pdf}
                            style={{
                              width: "80px",
                              height: "100px",
