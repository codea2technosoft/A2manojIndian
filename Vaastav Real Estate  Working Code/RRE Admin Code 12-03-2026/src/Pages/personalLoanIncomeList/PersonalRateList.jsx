...
function PersonalRateList() {
  ...
  const fetchPersonalRates = async () => {
    setLoading(true);
    setError(null);
    try {
      ...
      let url = `${API_URL}/personal-loan-rate-list`;
  ...
  const updatePersonalRate = async (e) => {
    e.preventDefault();
    setLoading(true);
    try {
      const token = getAuthToken();
      ...
      const url = `${API_URL}/personal-loan-rate-update`; 
      ...
      await fetchPersonalRates();
      setShowEditModal(false);
      showCustomMessageModal("Success", "Personal Rate updated successfully.", "success");
    } catch (err) {
  ...
  if (error) {
    return (
      <div className="alert alert-danger text-center m-5" role="alert">
        {error}
        <button className="btn btn-primary ms-3" onClick={() => fetchPersonalRates()}>
          Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="userlist mt-2">
        ...
        <div className="card-header">
          <div className="d-flex align-items-center justify-content-between">
            <div className="titlepage">
              <h3>Personal Loan Income List </h3>
            </div>
          </div>
        </div>
        ...
        <div className="card-body">
          <div className="table-responsive">
            <Table bordered>
              ...
              <tbody>
                {personalRate?.length > 0 ? (
                  personalRate.map((personalrate, i) => (
                    <tr key={personalrate.id}>
                      <td>{i + 1}</td>
                      <td>{personalrate.target}</td>
                      <td>{personalrate.direct_income}</td>
                      <td>{personalrate.differential_Income_after_achieve_target}</td>
                      <td>{personalrate.lavel}</td>
                      <td>
                        <Button onClick={() => handleEditClick(personalrate)} className="page-link">
                          <FaEdit />
                        </Button>
                      </td>
                    </tr>
      ...
      <Modal show={showEditModal} onHide={closeEditModal}>
        <Modal.Header closeButton>
          <Modal.Title>Edit Personal Loan Rate</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {currentRate && (
            <Form onSubmit={updatePersonalRate}>
...
export default PersonalRateList;