...
import { Form, Button, Container, Row, Col, Card, Modal, Tabs, Tab } from "react-bootstrap";
...
function CreatePlot() {
  ...
  const [activeTab, setActiveTab] = useState("csv"); // Default to CSV tab
  ...
  const validateCsvForm = () => {
    ...
    const requiredFields = ["project_id", "block_id"];
  ...
  const validateManualForm = () => {
    ...
    if (formData.plot_size && !dimensionPattern.test(formData.plot_size)) {
      newErrors.plot_size = "Dimensions must be in format like '50x40'.";
      isValid = false;
    }
  ...
  const handleCsvSubmit = async (e) => {
    ...
    try {
      ...
      uploadData.append("plotCsv", file);
      ...
      const response = await fetch(`${API_URL}/plot-create-csv`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!response.ok) {
        ...
        showCustomMessageModal(
          "Error",
          errorData.message || "Failed to create plots from CSV.",
          "error"
        );
        return;
      }
      ...
      showCustomMessageModal(
        "Success",
        result.message || "Plots created successfully from CSV!",
        "success"
      );
      ...
      console.error("Error creating plots from CSV:", err);
      showCustomMessageModal(
        "Error",
  ...
  // Common form fields for both tabs
    ...
    <Row>
      ...
      <Col md={6}>
        <Form.Group className="mb-3" controlId="formBlockId">
          ...
          {formData.project_id && !loading && blocks.length === 0 && (
            <small className="text-danger">
              No blocks available for this project. Please create one first.
            </small>
          )}
        </Form.Group>
      </Col>
            ...
            <div className="card-header">
              <div className="d-flex align-items-center justify-content-between">
                ...
                <div className="downloadtemplate">
                  <a href={callingleads} className="btn btn-primary" download="Plot Creation Format.csv">
                    Download Plot Creation Excel Format
                  </a>
                </div>
              </div>
            </div>
                  ...
                  <Form onSubmit={handleCsvSubmit}>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <Form.Text className="text-muted">
                        Please upload a CSV file with plot data in the correct format.
                      </Form.Text>
                    </Form.Group>
                    ...
                    <div className="submitbutton">
                      <Button
                        ...
                        disabled={loading}
                      >
                        {loading ? "Uploading..." : "Upload CSV & Create Plots"}
                      </Button>
                    </div>
                  </Form>
                </Tab>
