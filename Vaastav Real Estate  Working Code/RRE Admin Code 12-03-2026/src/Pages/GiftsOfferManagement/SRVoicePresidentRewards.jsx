...
function SRVoicePresidentRewards() {
  ...
  const fetchLedger = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/senior-vice-president-rewards-lists?${params}`,
        {
          method: "GET",
  ...
  const handleDownloadFilteredExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/download-team-sr-voice-president-rewards-achievements-lists?${params}`,
        {
          method: "GET",
          headers: {
      ...
      Swal.fire({
        ...
        text: "Please Check Entered From Date & To Date For Downloading 20001 SQYD From Your Team Sr.Voice President Achievements Report.",
      });
    }
  };
    ...
    <div className="padding_15">
      <div className="userlist">
        <div className="card shadow-sm">
          <div className="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap-mobile">
            <div className="titlepage">
              <h3 style={{ whiteSpace: "wrap" }}>
                20001 SQYD From Your Team Sr.Voice President Achievements Lists
              </h3>
            </div>
                    ...
                    {ledger.length === 0 ? (
                      ...
                      ledger.map((item, index) => (
                        <tr key={item.id || index}>
                          ...
                          <td>
                            {formatDate(
                              item.achievement_dates.current_achievement_date,
                            ) || "-"}
                          </td>
                          ...
                          <td>
                            {item.next_achievement.next_level_name || "NA"}
                          </td>
                          <td
                            style={{
                              color:
                          ...
                          <td>
                            {formatDate(
                              item.next_achievement.current_achievement_date,
                            )}
                          </td>
                          <td>
                            {item.user_id ? (
...
export default SRVoicePresidentRewards;
