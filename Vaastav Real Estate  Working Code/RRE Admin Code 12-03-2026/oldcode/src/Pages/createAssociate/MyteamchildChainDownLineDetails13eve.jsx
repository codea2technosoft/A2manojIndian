...
function MyTeamChildChainDownLineDetails() {
  ...
  const [otherLineStates, setOtherLineStates] = useState({}); // { root_mobile: treeWithExpanded }
  ...
  /* ------------------------------------------------------------------ */
  /*  TREE RENDERING (shared for main & other lines)                   */
  /* ------------------------------------------------------------------ */
  const treeRef = useRef(null);

  // ---------- Toggle for MAIN lines ----------
  const toggleNode = (node) => {
    ...
    setDownlineData((prev) => ({
      ...prev,
      summary: prev.summary.map((line) => ({
        ...line,
        tree: line.tree ? toggle(line.tree) : line.tree,
      })),
    }));
  };

  // ---------- Toggle for OTHER lines (per root_mobile) ----------
  const toggleOtherNode = (rootMobile, nodePath) => {
    const toggleAtPath = (tree, path) => {
      if (path.length === 0) {
        return {
          ...tree,
          expanded: !tree.expanded,
          children: tree.children?.map((c, i) => toggleAtPath(c, [...path, i])),
        };
      }
      ...
      return {
        ...
        children: tree.children?.map((c, i) =>
          i === head ? toggleAtPath(c, rest) : c
        ),
      };
    };

    setOtherLineStates((prev) => ({
      ...prev,
  ...
  const renderNode = (node, isRoot = false, rootMobile = null, nodePath = []) => (
    <li key={`${node.username}-${node.mobile || ""}`}>
      <span
        ...
        {node.children?.length > 0 && (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              if (rootMobile) {
                toggleOtherNode(rootMobile, nodePath);
              } else {
                toggleNode(node);
              }
            }}
  ...
  const OtherLineTreeView = ({ otherLine }) => {
    const rootMobile = otherLine.root_mobile;
    const tree = otherLineStates[rootMobile] || otherLine.tree || {};

    // If the API didn't give a tree, show a placeholder
    if (!tree.username) {
      return (
      ...
      <Accordion.Body className="p-0">
        ...
        {line.tree ? (
          ...
          <div className="p-3">
            ...
            {line.other_lines_summary.map((other, idx) => (
              <Card key={idx} className="mb-4 shadow-sm">
                <Card.Header className="bg-secondary text-white d-flex justify-content-between align-items-center">
                  <strong>{other.root_username}</strong>
                  <small>Phone: {other.root_mobile}</small>
                </Card.Header>

                <Card.Body className="p-0">
                  <div className="d-flex justify-content-between p-3 bg-light border-bottom">
                    <Badge bg="primary">Area: {other.area}</Badge>
                    <Badge bg="success">Members: {other.members}</Badge>
                  </div>

                  {/* FULL FAMILY TREE */}
                  <OtherLineTreeView otherLine={other} />
                </Card.Body>
              </Card>
            ))}
          </div>
