...
function ActivePlotList() {
  ...
  const fetchPlots = async (page = 1, query = "", projectFilter = "") => {
    ...
    try {
      ...
      const url = new URL(`${API_URL}/plot-list-active`);
  ...
  const handleEditPlot = async (plotId) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || "Failed to fetch unit for editing.",
        );
      }
      ...
      showCustomMessageModal(
        "Error",
        err.message ||
          "An unexpected error occurred while fetching unit for editing.",
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const handleUpdatePlot = async (e) => {
    ...
    try {
      ...
      showCustomMessageModal(
        "Error",
        err.message || "An unexpected error occurred while updating unit.",
        "error",
      );
    } finally {
      setLoading(false);
      ...
      async () => {
        ...
        try {
          ...
          if (!response.ok) {
            const errorData = await response.json();
            throw new Error(
              errorData.message || "Failed to update unit status.",
            );
          }
          ...
          showCustomMessageModal(
            "Error",
            err.message ||
              "An unexpected error occurred while updating unit status.",
            "error",
          );
        } finally {
          setLoading(false);
    ...
    showCustomMessageModal(
      ...
      async () => {
        ...
        try {
          ...
          showCustomMessageModal(
            "Error",
            err.message || "An unexpected error occurred while deleting unit.",
            "error",
          );
        } finally {
          setLoading(false);
  ...
  return (
    <div className="userlist mt-2">
          ...
          <div className="d-flex align-items-center justify-content-between">
            ...
            <div className="d-flex gap-2 align-items-center">
              <div className="createnewadmin">
                <Link
                  to="/create-plot"
                  className="fw-bold btn btn-success d-flex align-items-center"
                >
                  <FaPlus className="me-2" /> Create Unit
                </Link>
              </div>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Edit Available Unit</Modal.Title>
        </Modal.Header>
        <Modal.Body>
