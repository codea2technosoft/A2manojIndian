...
function AllPlotList() {
  ...
  //     link.setAttribute("download", `plot${Date.now()}.csv`);
  //     link.click();
  ...
  const exportAllToExcel = async () => {
    try {
      ...
      let filename = `plot${Date.now()}.csv`;
      if (selectedProject.trim() !== "") {

        const cleanProjectName = selectedProject.replace(/[^a-zA-Z0-9]/g, '_');
        filename = `plot_${cleanProjectName}_${Date.now()}.csv`;
      }
      ...
      showCustomMessageModal(
        ...
        selectedProject
          ...
          : "All plots data exported successfully",
        "success"
      );
        ...
        <div className="card-body">
          ...
          {isFilterActive && (
            <div className="d-flex gap-2 mb-3 mt-2 flex-wrap-mobile">
              ...
              <Form.Group controlId="searchPlot" className="d-flex">
                <input
                  type="text"
                  placeholder="Search by Unit No."
                  value={searchInput}
