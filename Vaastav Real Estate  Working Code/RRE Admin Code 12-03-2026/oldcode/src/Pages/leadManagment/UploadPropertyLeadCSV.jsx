...
function UploadPropertyLeadCSV() {
  ...
  const isNewStatusPage = () => {
    const params = new URLSearchParams(location.search);
    return params.get("status") === "New";
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `If you delete this lead, you can not revert it back , please verify and then delete ${selectedLeads.length} lead(s)`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        const res = await fetch(`${API_URL}/lead-delete-list-only-new`, {
          method: "DELETE",
          headers: {
        ...
        if (res.ok && data?.status == "1") {
          Swal.fire("Success", data?.message || "Leads deleted successfully!", "success");
          setSelectedLeads([]);
          setSelectAll(false);
          navigate("/calling-lead-report");
          fetchLeads(filters, page);
        } else {
          Swal.fire("Error", data?.message || "Failed to delete leads", "error");
        }
      } catch (err) {
        console.error(err);
        Swal.fire("Error", "Network error while deleting leads", "error");
      }
    }
  };
  ...
  return (
    <div className="card mt-2">
      <div className="card-header flex-mobile-wrap flex-direction-colunm-mobile d-flex justify-content-between align-items-center gap-2">
        <div className="titlepage ">
          <h3 className="m-0">Social Media Leads </h3>
        </div>
        <div className="d-flex gap-2 flex-wrap">
          <Link to="/calling-create-lead" className="btn btn-success d-flex align-items-center"
          >
            <FaPlus className="me-1"/>
            Add Calling Lead</Link>
          <Button onClick={() => setShowModal(true)}>   <FaFileCsv className="fs-5" />  Upload CSV</Button>
          <Button variant="success" className="d-flex gap-2 align-items-center fw-bold" onClick={handleDownloadExcel}>
            <BsFiletypeXlsx className="fs-5 " />
            Download Excel
          </Button>

          {/* Delete Selected Button - Only show when status=New and leads are selected */}
          {isNewStatusPage() && selectedLeads.length > 0 && (
          ...
          <>
            <Table striped bordered hover responsive>
              ...
              <tbody>
                {leads.map((lead, idx) => (
                  <tr key={lead.id || idx}>
                    {/* Checkbox cell - Only show when status=New */}
                    {isNewStatusPage() && (
                      <td>
                        <Form.Check
                          type="checkbox"
                    ...
                    <td>
                      {(() => {
                        if (!lead.lead_date) return "—"; // or "No date"
                    ...
                    <td>
                      <div className="table-cell-remark">
                        {lead.assign_by_subadmin_email ? lead.assign_by_subadmin_email : "-"}
                      </div>
                    </td>

                    <td>
                      <div className="table-cell-remark">
                        {lead.assign_to_subadmin_email ? lead.assign_to_subadmin_email : "-"}
                      </div>
                    </td>
                    <td>
                      <div className="table-cell-remark">
                    ...
                    <td>
                      {(() => {
                        if (!lead.next_follow_up_date) return "—"; // no date provided

                        let dateString = lead.next_follow_up_date;
