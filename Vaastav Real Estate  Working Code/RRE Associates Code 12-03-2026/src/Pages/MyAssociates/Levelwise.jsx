...
function Levelwise() {
        ...
        try {
            ...
            if (result.status === "1" && result.levelCounts) {
                ...
                for (let level = 1; level <= 11; level++) {
                    const levelData = result.levelCounts.find(item => item.level === level);
                    if (levelData) {
    ...
    const handleLevelClick = (level) => {
        ...
        // setShowDetails(true);
        window.location.href = `/levelwiselead?level=${level.level}`;
    };
    ...
    const getTotalUsers = () => {
        return levelCounts.reduce((total, level) => total + (level.user_count || 0), 0);
    };
            ...
            <div className="card">
                ...
                <div className="card-header">
                    <div className="d-flex align-items-center justify-content-between">
                        ...
                        <h2 className="stats-number">{getTotalUsers()}</h2>
                    ...
                    {loading ? (
                        ...
                        <div className="table-responsive">
                            <Table bordered hover>
                                <thead className="">
                                    <tr>
                                        <th className="headinglevel" style={{ fontWeight: "bold" }}>Level</th>
                                        <th className="headinglevel" style={{ fontWeight: "bold" }}>Team Size</th>
                                        <th className="headinglevel" style={{ fontWeight: "bold" }}>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {levelCounts.map((levelData) => (
                                        <tr key={levelData.level}>
                                            <td>
                                                <strong className="level_fonts">Level {levelData.level}</strong>
                                            </td>
                                            <td>
                                                <span className={`badge fs-6 ${levelData.user_count === 0 ? 'bg-secondary' : 'bg-primary'}`}>
                                                    {levelData.user_count || 0}
                                                </span>
                                            </td>
                                            <td>
                                                <Button
                                                    variant={levelData.user_count === 0 ? "" : "outline-info"}
                                                    size="sm"
                                                ...
                                                >
                                                    <FaEye />
                                                    {levelData.user_count === 0 ? 'No Data' : 'View Details'}
                                                </Button>
                                            </td>
                                        </tr>
                                    ))}
