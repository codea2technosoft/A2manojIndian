...
function ParentCommissionPage() {
  ...
  const fetchParentCommission = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/child-commission-list-for-parent?${params}`, {
        method: "GET",
        headers: {
  ...
  const handleDownloadFilteredExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/download-property-income-excel-by-associate-for-child?${params.toString()}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      <div className="card-header">
        <div className="d-flex justify-content-between align-items-center">
          ...
          <div className="d-block d-md-none">
            ...
            <button
              className={`btn btn-primary filter-toggle-btn ${isFilterActive ? "active" : ""}`}
                ...
                {commissionData.map((record, idx) => (
                  <tr key={record.id}>
                    ...
                    <td>
                      ₹{" "}
                      {(() => {
                        const advanceBalance = parseFloat(record.advance_balance);
