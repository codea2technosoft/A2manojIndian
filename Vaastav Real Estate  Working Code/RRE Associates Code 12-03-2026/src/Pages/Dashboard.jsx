...
const Dashboard = ({ userType }) => {
  ...
  const fetchmyteamLinesSummaryAssociate = async () => {
    try {
      ...
      if (data.status === 1) {  // status 1 hai na ki "1"
        ...
        const defaultData = {
          ...
          line3: { total_buysqft: 0, self_buysqft: 0, total_members: 0, line_name: "Others" },
          summary: { total_buysqft: 0, total_members: 0, total_lines: 0 }
        };
  ...
  const GiftProgressBar = () => {
    ...
    const eligibleGiftData = eligibilityData.find(item => item.status === "Eligible");
    const eligibleGift = giftList.find(gift =>
    ...
    const qualifyingSqyd = eligibleGift ? parseFloat(eligibleGift.area_sqyd) : 0;
              ...
              <div key={index} className="mt-2 w-50">
                <Row>
                  <Col sm={12} className="mb-3 text-end">
                    <div className="fs-5 d-block">
                      Current
                    </div>
                    <span className="text-success">{data.achieved_area || "0"} SQYD</span>
                  </Col>
                    ...
                    >
                      <div className="gift-step-content-wrapper">
                        ...
                        <div className="gift-step-content">
                          ...
                          <div className="gift-step-target">{gift.offer_item}</div>
        ...
        {!eligibleGift && eligibilityData.length === 0 && (
          <div className="text-center py-4">
            <div className="alert alert-warning">
              No eligible gift found. You need to achieve {giftList[0]?.area_sqyd || 0} SQYD to qualify for the first gift.
            </div>
          </div>
        )}
      </div>
                ...
                {giftTeamList.map((gift, index) => {
                  ...
                  return (
                    <>
                      ...
                      >
                        <div className="gift-step-content-wrapper">
                          <div className={`gift-step-marker ovel ${isCompleted ? 'gift-step-marker-completed bg-success text-white' :
                            isCurrent ? 'gift-step-marker-current bg-danger text-white' :
                          ...
                          <div className="gift-step-content">
                            ...
                            {isCompleted && (
                              <div className="gift-step-achieved">
                                ...
                                <div>{teameligibilityData?.[0]?.project_name || "-NA-"}</div>
                                {/* <div>{teameligibilityData?.[0]?.qualify_date || "-NA-"}</div> */}
                                <div>{formatDate(teameligibilityData?.[0]?.achieved_at)}</div>
                              </div>
                            )}
                            {isCurrent && (
  ...
  const LifetimeRewardsProgressBar = () => {
    ...
    const currentSqyd = eligibilityData?.rewards && eligibilityData.rewards.length > 0
      ? Math.max(...eligibilityData.rewards.map(r => parseFloat(r.totalArea || 0)))
      : 0;
    ...
    const eligibleRewardAreas = eligibilityData?.rewards
      ? new Set(eligibilityData.rewards.map(r => parseFloat(r.reward.area_sqyd)))
      : new Set();


    const highestEligibleReward = eligibilityData?.rewards && eligibilityData.rewards.length > 0
      ? eligibilityData.rewards.reduce((max, reward) =>
        parseFloat(reward.reward.area_sqyd) > parseFloat(max.reward.area_sqyd) ? reward : max
      ...
      <div className="gift-progress-wrapper">
              ...
              {eligibilityData.rewards && eligibilityData.rewards.length > 0 && (
                <div className="mt-2 p-2 bg-light rounded">
                  <h6 className="text-success mb-2">
                    🏆 You are eligible for {eligibilityData.rewards.length} Rewards!
                  </h6>
                  <div className="small">
                    {eligibilityData.rewards.map((reward, idx) => (
                      <div key={idx} className="d-flex justify-content-between border-bottom py-1">
                        ...
                        <span className="text-primary">₹{reward.reward.item_amount?.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="gift-steps-scroll-container">
              <div className="gift-steps-flex-container">
                ...
                {lifetimeRewardsList.map((reward, index) => {
                  ...
                  // Find matching reward details if eligible
                  const matchingReward = isEligible && eligibilityData?.rewards
                    ? eligibilityData.rewards.find(r => parseFloat(r.reward.area_sqyd) === rewardSqyd)
                    : null;
                  ...
                  return (
                    ...
                    >
                      <div className="gift-step-content-wrapper">
                        <div className={`gift-step-marker ovel ${isEligible ? 'bg-success' :
                        ...
                        <div className="gift-step-content">
                          ...
                          {reward.item_amount && (
                            <div className="gift-step-target text-primary">
                              <strong>₹{parseFloat(reward.item_amount).toLocaleString()}</strong>
                            </div>
                          )}
        ...
        {eligibilityData?.rewards && eligibilityData.rewards.length > 0 && (
          <div className="p-3 border-top bg-light">
            <h6 className="mb-3">🎁 Your Eligible Rewards ({eligibilityData.rewards.length})</h6>
            <Row>
              {eligibilityData.rewards.map((rewardItem, idx) => (
                <Col md={4} key={idx} className="mb-3">
                  <div className="border rounded p-2 h-100">
                    <div className="fw-bold text-success">{rewardItem.reward.offer_item}</div>
                    <div className="small">
                      <div><strong>Area:</strong> {rewardItem.reward.area_sqyd} SQYD</div>
                      <div><strong>Value:</strong> ₹{parseFloat(rewardItem.reward.item_amount).toLocaleString()}
                      </div>
                      ...
                      <div className="mt-1"><strong>Your Team Area:</strong> {rewardItem.totalArea.toFixed(2)} SQYD</div>
                    </div>
                  </div>
                </Col>
              ))}
