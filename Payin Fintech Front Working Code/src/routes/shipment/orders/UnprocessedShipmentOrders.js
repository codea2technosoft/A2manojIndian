...
import { Input, Table, Row, Alert, Modal, Button, Typography, Tag } from "antd";
import _ from 'lodash';
import { BaseSelect } from 'components/Elements';
import { toast } from "react-toast";
import { parseQueryParams, stringifyQueryParams } from 'utils/url';
// requests
import { getAvailableCarriers, rebookOrders } from "requests/shipment";

const { Title } = Typography;

const UnprocessedShipmentOrders = (props) => {
    ...
    // table columns for unprocessed tab
    const columns = [
        {
            title: "Order ID",
            fixed: true,
        ...
        },
        {
            ...
            render: (text, record) => {
                if (record.raw_data) {
                    let message = '';
                    if (record.raw_data?.message) message = record.raw_data.message;
                    if (record.raw_data?.error_message) message = record.raw_data.error_message;
                    if (record.raw_data?.err) message = record.raw_data.err;
                    if (record.raw_data?.payload?.error_message) message = record.raw_data.payload.error_message;
                    return (
                        <div>
                            {
                                message && <div className="text-error"><small>{message}</small></div>
                            }
                            {
                                record.raw_data.errors ? (
                                    <ul className="shipment-error-list">
                                        {
                                            Object.keys(record.raw_data.errors).map(key => {
                                                return (
                                                    <li className="text-error"><small>{record.raw_data.errors[key][0]}</small></li>
                                                )
                                            })
                                        }
                                    </ul>
    ...
    const onFetchCarriers = async (index) => {
        ...
        const selectedWarehouse = warehouses[record.service_id].find(item => item.value === record.warehouse_id);
    ...
    const onRebook = async () => {
        try {
            ...
            if (isValid) {
                await rebookOrders({ records: items });
                await onRefresh();
                setSelectedRows([]);
                setVisibleCarrierModal(false);
                setSelectedRecordId(null);
            } else {
                toast.error('Please fill out all fields before rebook shipments');
            }
        } catch (err) {
            console.log(err);
        } finally {
    ...
    return (
        <div>
            <div className="mb-16">
                <Button type="primary" disabled={!selectedRows.length} onClick={onRebook}>Re-book</Button>
            </div>
            <Alert
                className="mb-16"
...
export default UnprocessedShipmentOrders;